...
def generate_test_cases():
    ...
    # 1. Parse Bazel logs to find compilation failures
    ...
    bazel_failed_tests = get_bazel_failures()

    # 2. Process original test summaries
    for idx, test_file in enumerate(test_summaries_dir.glob("*.txt")):
        if "000_header.txt" in test_file.name:
            continue
    ...
    cnt = len(test_cases)
    # 3. Add Bazel compilation failures as distinct test cases
    for label, content in bazel_failed_tests.items():
        error_pattern = content.split("ERROR:")[0].strip()
        error_counts[error_pattern] = error_counts.get(error_pattern, 0) + 1
        test_cases.append(
            f"""
            <div class="card test-card mb-4" id="{cnt}" data-error="{error_pattern}">
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs mb-3">
                        <li class="nav-item">
                            <a class="nav-link active" data-bs-toggle="tab" href="#traceback-{cnt}">Info</a>
                        </li>
                    </ul>
                    
                    <div class="tab-content">
                        <div class="tab-pane show active" id="traceback-{cnt}">
                            <pre class="text-white">{content}</pre>
                        </div>
                    </div>
                </div>
