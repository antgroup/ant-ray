...
if command -v java >/dev/null; then
  if [ ! -f "$GOOGLE_JAVA_FORMAT_JAR" ]; then
    ...
    wget -q http://ray-streaming.oss-cn-hangzhou-zmf.aliyuncs.com/util%2Fgoogle-java-format-1.7-all-deps.jar -O "$GOOGLE_JAVA_FORMAT_JAR"
  fi
else
...
GIT_LS_EXCLUDES=(
  ':(exclude)python/ray/cloudpickle/'
  # ====== ANT-INTERNAL below =======
  # NOTE(kfstorm): DO NOT exclude a file or directory unless you are 100% sure
  # they are thirdparty files or machine generated. DO NOT exclude a file just
  # because it's Ant-internal only. Lint is not just about formatting, it's
  # also essential for detecthing Python script bugs such as:
  #
  # * F811 redefinition of unused 'test_local_scheduling_first' from line 71
  # * F821 undefined name 'generate_system_config_map'
  # * F901 'raise NotImplemented' should be 'raise NotImplementedError'
  #
  # If you still want to ignore some kind of error, you can add '# noqa: XXX'
  # to ignore it for once.
  #
  # Please think again before excluding something.
  ':(exclude)streaming/src/thirdparty/'
  ':(exclude)streaming/python/raystreaming/runtime/pangu_fs_lib.py'
  ':(exclude)dashboard/modules/job/clonevirtualenv.py'
)
