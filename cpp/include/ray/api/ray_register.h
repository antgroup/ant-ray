...
class Router {
    ...
    do {
      try {
        ...
        if (it == map_invokers_.end()) {
          result = PackArgsStr(error_code::FAIL, "unknown function: " + func_name);
          break;
        }
        ...
        result = PackArgsStr(error_code::FAIL, ex.what());
      }
    } while (0);
 ...
 private:
  ...
  Call(const F &f, std::string &result, std::tuple<Arg, Args...> tp) {
    ...
    result = PackArgsStr(error_code::OK);
  }
  ...
  Call(const F &f, std::string &result, std::tuple<Arg, Args...> tp) {
    ...
    result = PackArgsStr(error_code::OK, r);
  }
  ...
  struct invoker {
    static inline void Apply(const Function &func, const char *data, size_t size,
      ...
      try {
        ...
        result = PackArgsStr(error_code::FAIL, e.what());
  ...
  static std::string PackArgsStr(Arg arg, Args &&... args) {
