...
ObjectID NativeTaskSubmitter::Submit(InvocationSpec &invocation,
  ...
  if (invocation.task_type == TaskType::ACTOR_TASK) {
    RAY_UNUSED(core_worker.SubmitActorTask(invocation.actor_id,
                                           BuildRayFunction(invocation), invocation.args,
                                           options, &return_ids));
  } else {
    ...
    auto status =
        core_worker.SubmitTask(BuildRayFunction(invocation), invocation.args, options,
                               &return_ids, 1, false, scheduling_strategy, "");
    if (!status.ok()) {
...
ray::PlacementGroup NativeTaskSubmitter::CreatePlacementGroup(
  ...
  auto options = ray::PlacementGroupCreationOptions(
      create_options.name, (ray::PlacementStrategy)create_options.strategy,
