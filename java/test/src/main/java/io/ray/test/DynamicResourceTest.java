...
public class DynamicResourceTest extends BaseTest {
  ...
  public void testSetResource() {
    ...
    boolean resourceReady =
        TestUtils.waitForCondition(
            () -> {
              ...
              if (nodes.size() != 1 || !nodes.get(0).totalResources.containsKey(resourceName)) {
                return false;
              }
              return (0 == Double.compare(10.0, nodes.get(0).availableResources.get(resourceName)));
            },
            2000);
    ...
    Assert.assertEquals((double) nodes.get(0).totalResources.get("A"), 10.0);

    // Assert ray call result.
    result = Ray.wait(ImmutableList.of(obj), 1, 2000);
