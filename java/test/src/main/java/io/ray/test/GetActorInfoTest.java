...
public class GetActorInfoTest extends BaseTest {
  ...
  public void testExtendedProperties() {
    // Create an actor with the extended properies.sd
    Ray.actor(Echo::new).setExtendedProperties("TestKey", "TestValue").remote();
    List<ActorInfo> actorInfo = Ray.getRuntimeContext().getAllActorInfo();
    Assert.assertEquals(actorInfo.size(), 1);
    Assert.assertEquals(actorInfo.get(0).extendedProperties.size(), 1);
    String customerValue = actorInfo.get(0).extendedProperties.get("TestKey");
    Assert.assertTrue(customerValue != null && customerValue.equals("TestValue"));
  }
}
