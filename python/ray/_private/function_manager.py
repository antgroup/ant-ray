...
class FunctionActorManager:
    ...
    def fetch_and_register_remote_function(self, key):
        ...
        if job_id != self._worker.current_job_id:
            return
    ...
    def _load_function_from_local(self, function_descriptor):
        ...
        if object is not None:
            if isinstance(object, ray.remote_function.RemoteFunction):
        ...
        while True:
            ...
            if time.time() - start_time > timeout:
                warning_message = ("This worker was asked to execute a "
                                   ...
                                   "Ray. The function descriptor is "
    ...
    def load_actor_class(self, job_id, actor_creation_function_descriptor):
        ...
        if actor_class is None:
            # Load actor class.
            if actor_creation_function_descriptor.module_name.startswith(
                    "ray._private"):
                actor_class = self._load_actor_class_from_local(
                    actor_creation_function_descriptor)
                if actor_class is None:
                    raise RuntimeError(
                        "Please check ray version, we cannot import this "
                        "class from current ray, module name: "
                        f"{actor_creation_function_descriptor.module_name}, "
                        "class name: "
                        f"{actor_creation_function_descriptor.class_name}.")
            elif self._worker.load_code_from_local:
                # TODO(fyrestone): use job isolation solution.
    ...
    def _load_actor_class_from_local(self, actor_creation_function_descriptor):
        ...
        if object is not None:
            if isinstance(object, ray.actor.ActorClass):
                ...
                return ray.actor.modify_class(object)
        else:
            return None
