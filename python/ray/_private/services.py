...
def start_redis(node_ip_address,
    ...
    if external_redis_addresses is None:
        ...
        primary_redis_client = create_redis_client(
            "%s:%s" % (node_ip_address, port), password=password)
        ...
        redis_init_successful = primary_redis_client.get(
            "REDIS_INIT_SUCCESSFUL") == b"1"
        if redis_init_successful:
            logger.info("Redis is already initialized. Skip initialization.")
            redis_shards = external_redis_addresses[1:]
    ...
    for i in range(num_redis_shards):
        ...
        # TODO(qwang): `continue` if `external_redis_addresses is not None`
        # so that we should not reslove this conflict in the future.
        if external_redis_addresses is None:
            redis_executable = REDIS_EXECUTABLE
            ...
            # `redis_shard_port` and `last_shard_port` are not used in this
            # case. Here we add the assignment to avoid the "local variable
            # 'redis_shard_port' referenced before assignment" error.
            redis_shard_port = last_shard_port
    ...
    primary_redis_client.set("REDIS_INIT_SUCCESSFUL", 1)
...
def start_gcs_server(redis_address,
    ...
    Args:
        ...
        event_dir(str): Event log directory for the active ray process.
    Returns:
        ProcessInfo for the process that was started.
    """
...
def start_raylet(ray_params,
    ...
    try:
        jars_dir = get_ray_jars_dirs()
        if len(jars_dir) > 0 and os.path.exists(jars_dir[0]):
    ...
    if node_name:
        start_worker_command += [f"--node-name={node_name}"]
    ...
    command = [
        ...
        "--agent_command={}".format(subprocess.list2cmdline(agent_command)),
        f"--native_library_path={DEFAULT_NATIVE_LIBRARY_PATH}",
        f"--redis_password={redis_password or ''}",
        f"--temp_dir={temp_dir}",
        "--logs_dir={}".format(log_dir),
        "--event_dir={}".format(event_dir),
...
# Ant internal: support java in container
def _wrap_java_worker_command(setup_worker_path, worker_setup_hook,
                              session_dir, worker_command):
    if sys.platform == "win32":
...
def get_ray_jars_dirs():
    ...
    ray_dist_jar_dirs = [os.path.abspath(os.path.join(current_dir, "jars"))]
    try:
        import pytide.runtime.jars
        ray_dist_jar_dirs.append(pytide.runtime.jars.get_tide_jars_dir())
    except ImportError:
        pass
    return ray_dist_jar_dirs
...
def build_java_worker_command(
    ...
    ray_jars = ":".join(
        [os.path.join(jar_dir, "*") for jar_dir in get_ray_jars_dirs()])
    command += ["-cp", ray_jars]

    command += ["RAY_WORKER_DYNAMIC_OPTION_PLACEHOLDER"]
    command += ["io.ray.runtime.runner.worker.DefaultWorker"]

    return _wrap_java_worker_command(setup_worker_path, worker_setup_hook,
                                     session_dir, command)
