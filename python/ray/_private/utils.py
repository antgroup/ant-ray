...
def resources_from_resource_arguments(
    ...
    if memory is not None:
        # This is required for GCS tasks scheduling.
        if not ray_constants.is_multiple_of_memory_unit(memory):
            raise ValueError(
                "The memory resource must be positive integral multiple of"
                " memory units(50M).")
...
def estimate_available_memory():
    ...
    """
    # Reserve some memory as buffer to reduce the chance of OOM.
    # This is useful for the new resource management implementation.
    system_memory = get_system_memory()
    used_memory = get_used_memory()
    available_memory = system_memory - used_memory
    reserved_memory = 0.1 * available_memory

    reserved_system_memory_mb = os.environ.get("RESERVED_SYSTEM_MEMORY_MB",
                                               None)
    if reserved_system_memory_mb is not None:
        reserved_memory = min(
            float(reserved_system_memory_mb) * (1024**2),
            0.2 * available_memory)

    # If the user sets RESERVED_SYSTEM_MEMORY_RATIO then calculate a new
    # value to overwrite the old previous reserved_memory.
    # The recommended value for this parameter is [0, 0.2]
    reserved_system_memory_ratio = os.environ.get(
        "RESERVED_SYSTEM_MEMORY_RATIO", None)
    if reserved_system_memory_ratio is not None:
        reserved_memory = min(
            float(reserved_system_memory_ratio) * available_memory,
            0.2 * available_memory)
    ...
    return available_memory - reserved_memory + get_cached_memory() / 2
