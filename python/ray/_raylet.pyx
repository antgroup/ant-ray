...
from ray.includes.common cimport (
    ...
    CActorAffinitySchedulingStrategy,
    CActorAffinityMatchExpression,
    CActorAffinityOperator,
    CRayFunction,
    CWorkerType,
    CJobConfig,
    move,
    ...
    ACTOR_AFFINITY_OPERATOR_IN,
    ACTOR_AFFINITY_OPERATOR_NOT_IN,
    ACTOR_AFFINITY_OPERATOR_EXISTS,
    ACTOR_AFFINITY_OPERATOR_DOES_NOT_EXIST,
)
...
from ray.profiler import Profiler
import msgpack
    ...
    with core_worker.profile_event(b"task", extra_data=extra_data):
        try:
            ...
            if not (<int>task_type == <int>TASK_TYPE_ACTOR_TASK
                ...
                """ ANT-INTERNAL
                It's heavy to do memory check in executing thread. Any issue
                of the memory monitor will impact the ray critical path and
                lead to more serious issues from the user side. For example:
                https://github.com/ray-project/ray/issues/14929

                And because there is an independent memory monitor component
                inside Ant, our Ant internal code doesn't need this memory
                check in this execution thread any longer.
                """
            ...
            core_worker.report_event("ERROR", "pyTaskFailure",
                                     str(failure_object))
                ...
                if isinstance(e, RayActorError) and \
                   e.actor_init_failed:
                    ...
                    ray.worker.global_worker.core_worker.report_event(
                        "ERROR", "pyWorkerCrash", traceback_str)
                    ray._private.utils.push_error_to_driver(
...
cdef void profile_handler(c_bool start) nogil:
    with gil:
        Profiler.start() if start else Profiler.stop()
...
cdef class CoreWorker:
    ...
    def __cinit__(self, worker_type, store_socket, raylet_socket,
        ...
        options.profile_handler = profile_handler
    ...
    def shutdown(self, c_bool exit_with_error):
        with nogil:
            ...
            # driver.
            if self.is_driver:
                CCoreWorkerProcess.Shutdown(exit_with_error)
        ...
        if object_ref is None:
            with nogil:
                check_status(CCoreWorkerProcess.GetCoreWorker().CreateOwned(
                             ...
                             c_object_id, data, c_owner_address))
        else:
    ...
    def report_event(self, c_string severity, c_string label,
                     c_string message):
        CRayEvent.ReportEvent(severity, label, message)
    ...
    def put_serialized_object(self, serialized_object,
        ...
        object_already_exists = self._create_put_buffer(
            ...
            &c_object_id, &data, owner_address)
    ...
    cdef python_actor_affinity_match_expression_to_c(
            self, match_expression,
            CActorAffinityMatchExpression *c_match_expression):
        cdef:
            c_string c_key = match_expression.key
            c_bool c_soft = match_expression.soft
            CActorAffinityOperator c_operator
        c_match_expression[0].set_key(c_key)
        c_match_expression[0].set_soft(c_soft)

        operator = match_expression.operator
        if operator == ActorAffinityOperator.IN:
            c_operator = ACTOR_AFFINITY_OPERATOR_IN
        elif operator == ActorAffinityOperator.NOT_IN:
            c_operator = ACTOR_AFFINITY_OPERATOR_NOT_IN
        elif operator == ActorAffinityOperator.EXISTS:
            c_operator = ACTOR_AFFINITY_OPERATOR_EXISTS
        elif operator == ActorAffinityOperator.DOES_NOT_EXIST:
            c_operator = ACTOR_AFFINITY_OPERATOR_DOES_NOT_EXIST
        else:
            raise TypeError(operator)
        c_match_expression[0].set_actor_affinity_operator(c_operator)

        for value in match_expression.values:
            c_match_expression[0].add_values(value)
    ...
    cdef python_scheduling_strategy_to_c(
        ...
        cdef:
            ...
            CActorAffinitySchedulingStrategy \
                *c_actor_affnitiy_scheduling_strategy
        ...
        if python_scheduling_strategy == "DEFAULT":
            ...
            for node_id_hex in python_scheduling_strategy.nodes:
                c_node_affinity_scheduling_strategy.add_nodes(
                    NodeID.from_hex(node_id_hex).binary())
            c_node_affinity_scheduling_strategy[0].set_soft(
                python_scheduling_strategy.soft)
            c_node_affinity_scheduling_strategy[0].set_anti_affinity(
                python_scheduling_strategy.anti_affinity)
        elif isinstance(python_scheduling_strategy,
                        ActorAffinitySchedulingStrategy):
            c_actor_affnitiy_scheduling_strategy = \
                c_scheduling_strategy[0] \
                .mutable_actor_affinity_scheduling_strategy()
            for match_expression in \
                    python_scheduling_strategy.match_expressions:
                c_match_expression = c_actor_affnitiy_scheduling_strategy[0] \
                    .add_match_expressions()
                self.python_actor_affinity_match_expression_to_c(
                    match_expression, c_match_expression)
        else:
            raise ValueError(
                ...
                f" | ActorAffinitySchedulingStrategy]")
    ...
    def kill_job(self, JobID job_id):
        cdef:
            CJobID c_job_id = job_id.native()
            c_bool result

        with nogil:
            result = CCoreWorkerProcess.GetCoreWorker().KillJob(c_job_id)
        return result
    ...
    def put_kv(self, key: Union[str, bytes], value: Union[str, bytes],
               overwrite: bool = True, isGlobal: bool = False):
        cdef:
            pair[c_bool, CRayStatus] result_pair
        result_pair = CCoreWorkerProcess.GetCoreWorker().PutKV(
            key, value, overwrite, isGlobal)
    ...
    def get_kv(self, key: Union[str, bytes], isGlobal: bool = False):
        result_pair = CCoreWorkerProcess.GetCoreWorker().GetKV(key, isGlobal)
        if result_pair.second.IsNotFound():
            return None
        check_status(result_pair.second)
        return result_pair.first

    def exists_kv(self, key: Union[str, bytes], isGlobal: bool = False):
        result_pair = CCoreWorkerProcess.GetCoreWorker().ExistsKV(
            key, isGlobal)
        check_status(result_pair.second)
        return result_pair.first

    def delete_kv(self, key: Union[str, bytes], isGlobal: bool = False):
        result_pair = CCoreWorkerProcess.GetCoreWorker().DeleteKV(
            key, isGlobal)
        check_status(result_pair.second)
        return result_pair.first
...
def _dump_config_to_json_str() -> str:
    return RayConfig.instance().dump_to_json_str().decode("UTF-8")
