...
export const FlameVisualization: React.FC<FlameVisualizationProps> = ({
      ...
      while (i--) {
        ...
        if (children[0] && children[0].parent) {
          (children[0].parent as PartitionHierarchyNode).customValue =
            ((children[0].parent as PartitionHierarchyNode).customValue || 0) +
            childrenValue;
        }
      }
  ...
  useEffect(() => {
    ...
    const mainGroup = svg.append("g").attr("class", "flame-graph-group");
    ...
    const zoom = d3
      ...
      .on("zoom", (event) => {
        ...
        if (
          ...
          const tooltipData = d3.select(tooltipRef.current).datum() as any;
          if (tooltipData) {
            const newX = event.transform.applyX(tooltipData.x);
            const newY = event.transform.applyY(tooltipData.y);
    ...
    const tooltipHandler = {
      show: (d: PartitionHierarchyNode, element: SVGGElement) => {
        ...
        // Remove listener when hiding tooltip
        const originalHide = tooltipHandler.hide;
        tooltipHandler.hide = () => {
