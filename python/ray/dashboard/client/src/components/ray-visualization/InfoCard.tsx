...
const findNodeById = (id: string, graphData: GraphData): Node => {
  ...
  if (actor) return { ...actor, type: 'actor' };
...
const InfoCard = ({ data, visible, graphData }: InfoCardProps) => {
              ...
              <li key={`${node.id}-${index}`} className="connection-item">
                {node.type === 'method' && node.actorName && (
                  <div className="connection-actor-info">
                    <span className="connection-actor">Actor: {node.actorName}</span>
                  </div>
                )}
                ...
                <div className="connection-main-info">
                  ...
                  {'count' in node && node.count && <span className="connection-count">{node.count}次</span>}
                  {'speed' in node && node.speed && <span className="connection-speed">{node.speed}</span>}
                </div>
                
                {node.type === 'actor' && (
                  <div className="connection-actor-info">
                    <span className="connection-language">Language: {node.language}</span>
                  </div>
                )}
              </li>
            ))}
  ...
  // Render the flow information between nodes
  const renderFlowInfo = (sourceId: string, targetId: string, flowType: 'call' | 'data') => {
    const source = findNodeById(sourceId, graphData);
    const target = findNodeById(targetId, graphData);
    
    // Find the relevant flow details
    let flow: CallFlow | DataFlow | undefined;
    if (flowType === 'call') {
      flow = graphData.callFlows.find(f => f.source === sourceId && f.target === targetId);
    } else {
      flow = graphData.dataFlows.find(f => f.source === sourceId && f.target === targetId);
    }
    ...
    return (
      <div className="flow-detail">
        <span className="flow-source">{source.name}</span>
        <span className="flow-arrow">→</span>
        <span className="flow-target">{target.name}</span>
        {flowType === 'call' && 'count' in flow && <span className="flow-count">{flow.count}次</span>}
        {flowType === 'data' && 'speed' in flow && <span className="flow-speed">{flow.speed}</span>}
      </div>
    );
  };
