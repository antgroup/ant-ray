...
class InsightHead(dashboard_utils.DashboardHeadModule):
    ...
    async def get_debug_sessions(self, req: aiohttp.web.Request) -> aiohttp.web.Response:
        """Return a list of active debug sessions."""
        job_id = req.query.get("job_id", "default_job")
        class_name = req.query.get("class_name", "")
        func_name = req.query.get("func_name", "")
        ...
        if not address:
            return dashboard_optional_utils.rest_response(success=False, message="InsightMonitor address not found in KV store")
        host, port = address.decode().split(":")
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    f"http://{host}:{port}/get_debug_sessions",
                    json={"job_id": job_id, "class_name": class_name, "func_name": func_name}
                ) as response:
                    if response.status != 200:
                        return dashboard_optional_utils.rest_response(success=False, message=f"Error from insight monitor: {await response.text()}")
                    result = await response.json()
                    return dashboard_optional_utils.rest_response(success=True, message="Debug sessions retrieved successfully.", result=result)
        except Exception as e:
            logger.error(f"Error retrieving debug sessions: {str(e)}")
            return dashboard_optional_utils.rest_response(success=False, message=f"Error retrieving debug sessions: {str(e)}")

    @routes.get("/activate_debug_session")
    async def activate_debug_session(self, req: aiohttp.web.Request) -> aiohttp.web.Response:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    json={"job_id": job_id, "class_name": class_name, "func_name": func_name, "task_id": task_id}
                ) as response:
                    if response.status != 200:
                        return dashboard_optional_utils.rest_response(success=False, message=f"Error from insight monitor: {await response.text()}")
                    result = await response.json()
                    return dashboard_optional_utils.rest_response(success=True, message="Debug session activated successfully.", result=result)
        except Exception as e:
            logger.error(f"Error activating debug session: {str(e)}")
            return dashboard_optional_utils.rest_response(success=False, message=f"Error activating debug session: {str(e)}")
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    json={"task_id": task_id, "command": command, "args": args}
                ) as response:
                    if response.status != 200:
                        return dashboard_optional_utils.rest_response(success=False, message=f"Error from insight monitor: {await response.text()}")
                    result = await response.json()
                    return dashboard_optional_utils.rest_response(success=True, message="Debug command sent successfully.", result=result)
        except Exception as e:
            logger.error(f"Error sending debug command: {str(e)}")
            return dashboard_optional_utils.rest_response(success=False, message=f"Error sending debug command: {str(e)}")  
                    
    @routes.get("/deactivate_debug_session")
    async def deactivate_debug_session(self, req: aiohttp.web.Request) -> aiohttp.web.Response:
            ...
            async with aiohttp.ClientSession() as session:  
                async with session.post(
                    ...
                    if response.status != 200:
                        return dashboard_optional_utils.rest_response(success=False, message=f"Error from insight monitor: {await response.text()}")
                    result = await response.json()
                    return dashboard_optional_utils.rest_response(success=True, message="Debug session deactivated successfully.", result=result)
        except Exception as e:
            logger.error(f"Error deactivating debug session: {str(e)}")
            return dashboard_optional_utils.rest_response(success=False, message=f"Error deactivating debug session: {str(e)}")

    @routes.get("/get_active_debug_sessions")
    async def get_active_debug_sessions(self, req: aiohttp.web.Request) -> aiohttp.web.Response:
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"http://{host}:{port}/get_active_debug_sessions"
                ) as response:
                    if response.status != 200:
                        return dashboard_optional_utils.rest_response(success=False, message=f"Error from insight monitor: {await response.text()}")
                    result = await response.json()
                    return dashboard_optional_utils.rest_response(success=True, message="Active debug sessions retrieved successfully.", result=result)
        except Exception as e:
            logger.error(f"Error retrieving active debug sessions: {str(e)}")
            return dashboard_optional_utils.rest_response(success=False, message=f"Error retrieving active debug sessions: {str(e)}")
