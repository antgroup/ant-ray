...
class HeadNodeLeaderSelectorConfig:
    ...
    def __init__(self):
        ...
        # Default 3s
        self.check_interval_s = int(
            os.environ.get("RAY_HA_CHECK_INTERVAL_MS", "3000")) / 1000
        # Default 60s
        self.key_expire_time_ms = int(
            os.environ.get("RAY_HA_KEY_EXPIRE_TIME_MS", "60000"))
        # Default 2s
        self.wait_time_after_be_active_s = int(
            os.environ.get("RAY_HA_WAIT_TIME_AFTER_BE_ACTIVE_MS",
                           "2000")) / 1000
        # Default 3 day (1000 * 60 * 60 * 24 * 3).
        self.wait_pre_gcs_stop_max_time_s = int(
            os.environ.get("RAY_HA_WAIT_PRE_GCS_STOP_MAX_TIME_MS",
                           "259200000")) / 1000
        # Default redis/socket connect time out is 5s.
        self.connect_timeout_s = int(
            os.environ.get("RAY_HA_CONNECT_TIMEOUT_MS", "5000")) / 1000

    def __repr__(self) -> str:
        return f"HeadNodeLeaderSelectorConfig["\
...
class HeadNodeLeaderSelector:
    ...
    def set_role_type(self, role_type):
        if role_type != self._role_type:
            logger.info("This head node changed from %s to %s.",
