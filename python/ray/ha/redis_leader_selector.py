...
class RedisBasedLeaderSelector(HeadNodeLeaderSelector):
    ...
    def __init__(self, ray_params, redis_address, node_ip_address):
        ...
        gcs_server_port = 0 if ray_params.gcs_server_port is None\
            else ray_params.gcs_server_port
    ...
    def start(self):
        ...
        logger.info("Success to start redis leader selecotr. name:%s",
                    self.name)
        self.do_election()
    ...
    def do_election(self):
        if self._is_running:
            self.check_leader()
            if self._is_running:
                self._timer = threading.Timer(self._config.check_interval_s,
                                              self.do_election)
                self._timer.start()
        else:
            logger.warning(
                "Timer still running while leader selector stopped.")
        ...
        try:
            ...
            if isSetSuccess:
                ...
                logger.info(
                    ...
                    "become the active head node, name:%s.", self.name)
            else:
                self.expire_leader_key()
        except Exception:
            logger.error(
                "%s node failed to check leader.", self.name, exc_info=True)
            ...
            if self._real_failure_count > self._config.max_failure_count:
                logger.error(
                    ...
                    self._real_failure_count, self._config.max_failure_count,
                    self.get_role_type())
                self.stop()
        ...
        if leader_name == self.name:
            ...
            if not isSuccess:
                ...
                logger.error(
                    ...
                    "key failed, failure count:%d.", self._real_failure_count)
                if self._real_failure_count > self._config.max_failure_count:
                    logger.error(
                        "This active head node role "
                        "downgraded to standby for expire failed"
                        " %d times more then %d.", self._real_failure_count,
            ...
            if self.is_leader():
                ...
                if self._leader_name != str(leader_name):
                    ...
                    logger.info(
                        "This head node role is standby now. "
                        "active node is %s.", str(leader_name))
                else:
                    logger.debug(
                        "This head node role is standby now. "
                        "active node is %s.", str(leader_name))
    ...
    def do_action_after_be_active(self):
        expired_gcs_address = self._redis_client.get(
            ray_constants.GCS_ADDRESS_KEY)
        if expired_gcs_address:
            pre_gcs_address = str(expired_gcs_address, encoding="utf-8")
            self._redis_client.set(ray_constants.GCS_ADDRESS_KEY,
                                   self._init_gcs_address)
            logger.info("Reset gcs address from %s to invalid gcs address:%s",
                        pre_gcs_address, self._init_gcs_address)
            ...
            if is_disconnect:
                logger.info(
                    ...
                    use_time, pre_gcs_address)
                return True
            else:
                logger.error(
                    "After waiting for %d s, the previous gcs(%s) still"
                    " working. this maybe casue some wrong.", max_wait_time,
                    pre_gcs_address)
                return False
        return True
    ...
    def node_wait_to_be_active(self):
        ...
        while True:
            if not self.is_running():
                logger.error("The leader selector has stopped in waiting,"
                             " will restart it.")
                self.start()
                continue

            if self.is_leader():
                logger.info("This head node changed from standby to active, "
