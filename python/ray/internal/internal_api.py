...
def get_gcs_frozen_nodes_stub():
    import grpc
    from ray.core.generated import gcs_service_pb2_grpc

    worker = ray.worker.global_worker

    gcs_address = worker.redis_client.get("GcsServerAddress")
    channel = grpc.insecure_channel(
        gcs_address,
        options=[
            ("grpc.max_send_message_length", MAX_MESSAGE_LENGTH),
    ...
    return gcs_service_pb2_grpc.FrozenNodeGcsServiceStub(channel)


def freeze_nodes(node_name_set):
    from ray.core.generated import gcs_service_pb2
    stub = get_gcs_frozen_nodes_stub()
    request_pb = gcs_service_pb2.FreezeNodesRequest()
    request_pb.node_name_set.extend(node_name_set)
    return stub.FreezeNodes(request_pb)


def unfreeze_nodes(node_name_set):
    from ray.core.generated import gcs_service_pb2
    stub = get_gcs_frozen_nodes_stub()
    request_pb = gcs_service_pb2.UnfreezeNodesRequest()
    request_pb.node_name_set.extend(node_name_set)
    return stub.UnfreezeNodes(request_pb)
