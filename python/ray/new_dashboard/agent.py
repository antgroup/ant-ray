...
class DashboardAgent(object):
    def __init__(self,
        ...
        self._init_stats()
        # Initialize the RayConfig by config_list.
        ray._config.initialize(config_list or "")

    def _init_stats(self):
        """Initialize stats for recording metrics."""
        global_tags = {
        ...
        dashboard_utils.init_stats(global_tags, app_name="agent_stats")
    ...
    async def run(self):
        ...
        await site.start()
        http_host, self.http_port, *_ = site._server.sockets[0].getsockname()
        ...
        await self.aioredis_client.set(
            ...
            json.dumps({
                dashboard_consts.DASHBOARD_AGENT_HTTP_PORT: self.http_port,
                dashboard_consts.DASHBOARD_AGENT_GRPC_PORT: self.grpc_port,
                dashboard_consts.DASHBOARD_AGENT_TIMESTAMP: time.time(),
            }))
        # Set process group id before register to raylet
        # so that all subprocesses can be killed properly.
        os.setpgid(0, 0)
        # Register agent to agent manager.
        self.raylet_stub = agent_manager_pb2_grpc.AgentManagerServiceStub(
            self.aiogrpc_raylet_channel)

        await self.raylet_stub.RegisterAgent(
            agent_manager_pb2.RegisterAgentRequest(
                agent_pid=os.getpid(),
                agent_port=self.grpc_port,
                agent_ip_address=self.ip))

        # We get the value after the ray._config.initialize().
        heartbeat_interval_seconds = \
            ray._config.agent_heartbeat_period_milliseconds() / 1000

        @async_loop_forever(heartbeat_interval_seconds)
        async def _report_heartbeat():
            """Report heartbeat to raylet."""
            await self.raylet_stub.AgentHeartbeat(
                agent_manager_pb2.AgentHeartbeatRequest())

        tasks = [m.run(self.server) for m in modules]
        if sys.platform not in ["win32", "cygwin"]:
            tasks.append(check_parent_task)
        logger.info("Report heartbeat every %s seconds.",
                    heartbeat_interval_seconds)
        tasks.append(_report_heartbeat())
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="The config list to overwrite the default RayConfig.")
