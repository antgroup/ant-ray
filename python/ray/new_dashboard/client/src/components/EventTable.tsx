...
import { InfoRounded, SearchOutlined } from "@material-ui/icons";
...
const useEventTable = (props: EventTableProps) => {
  ...
  useEffect(() => {
    const getEvent = async () => {
      changeFilter("severity", "ERROR,FATAL,WARNING");
      try {
        if (job_id) {
  ...
  return {
    ...
    hostOptions: Array.from(new Set(events.map((e) => e.hostName))),
...
const EventTable = (props: EventTableProps) => {
  ...
  return (
    <div style={{ position: "relative" }}>
      {slsUrl && (
        <Card style={{ width: "100%", padding: 8 }} elevation={3}>
          <Grid container spacing={1} alignItems="center">
            <Grid item>
              <InfoRounded />
            </Grid>
            <Grid item>
              Event Table in Ray Dashboard may not contain all events, for more
              events visit:
            </Grid>
            <Grid item>
              <Button>
                <a href={slsUrl} rel="noopener noreferrer" target="_blank">
                  Event in sls
                </a>
              </Button>
            </Grid>
      ...
      <div>
        <Autocomplete
          ...
          multiple
          renderTags={(values: string[]) => values.join(',')}
          onChange={(_: any, value: string[]) => {
            changeFilter("label", value.join(','));
          }}
        ...
        <Autocomplete
          ...
          defaultValue={["ERROR","FATAL","WARNING"]}
          multiple
          renderTags={(values: string[]) => values.join(',')}
          onChange={(_: any, value: string[]) => {
            changeFilter("severity", value.join(','));
          }}
          renderInput={(params: TextFieldProps) => (
