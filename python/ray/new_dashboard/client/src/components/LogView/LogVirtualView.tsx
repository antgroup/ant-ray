...
export type LogVirtualViewProps = {
  ...
  startTime?: string;
  endTime?: string;
  handleClickLineNumber?: (num: number) => void;
};
    ...
    return (
      ...
      >
        <span
          style={{
            marginRight: 4,
            width: `${logs[logs.length - 1].i}`.length * 6 + 4,
            color: "#999",
            display: "inline-block",
            cursor: handleClickLineNumber ? "pointer" : "",
            userSelect: 'none',
          }}
          onClick={() => handleClickLineNumber && handleClickLineNumber(i + 1)}
        >
          {i + 1}
        </span>
        {low
