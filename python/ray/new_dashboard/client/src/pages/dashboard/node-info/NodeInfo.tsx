...
const liveNodesSelector = (state: StoreState) =>
  state.dashboard?.nodeInfo?.clients.filter(
    (node: Node) => node.raylet.state === "ALIVE",
  );
...
const NodeInfo: React.FC<{}> = () => {
  ...
  const nodes = useSelector<StoreState, Node[]>(liveNodesSelector);
  if (!nodes) {
    return <Typography color="textSecondary">Loading...</Typography>;
  }
  const clusterTotalWorkers = sum(nodes.map((n: Node) => n.workers.length));
