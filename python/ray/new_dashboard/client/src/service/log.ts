...
export const getLogDirectly = async (url: string) => {
  ...
  return typeof rsp.data === "string" ? rsp.data : JSON.stringify(rsp.data);
};

export const getLogProxy = async (url: string) => {
  ...
  return rsp.data.data || (rsp.data as string);
};

export const getLogDetail = async (url: string) => {
  let result;
  try {
    result = await getLogDirectly(url);
  } catch (e) {}
  if (!result) {
    result = await getLogProxy(url);
  }
