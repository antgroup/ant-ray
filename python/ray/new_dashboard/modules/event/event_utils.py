...
def get_external_logger(event_dir):
    handler = logging.handlers.WatchedFileHandler(
        os.path.join(event_dir,
                     f"event_{event_consts.EVENT_SOURCE_EXTERNAL}.log"))
    formatter = logging.Formatter("%(message)s")
    handler.setFormatter(formatter)

    logger = logging.getLogger(event_consts.EVENT_SOURCE_EXTERNAL)
    logger.propagate = False
    logger.setLevel(event_consts.EXTERNAL_LOG_LEVEL)
...
def _parse_line(event_str):
    return restore_newline(json.loads(event_str))
            ...
            for _ in range(n_lines):
                ...
                try:
                    if sep - start <= event_consts.READ_LINE_LENGTH_LIMIT:
                        ...
                        truncated_size = min(
                            100, event_consts.READ_LINE_LENGTH_LIMIT)
                        logger.warning(
                            "Ignored long string: %s...(%s chars)",
                            mm[start:start + truncated_size].decode("utf-8"),
                            sep - start)
                except Exception:
                    # We should handle all the read exceptions to
                    # keep reading forward.
                    e = new_event("AgentInternalError", {
                        "filename": f.name,
                        "offset": start,
...
def format_event(event_dict):
    try:
        ...
        logger.exception("Format event failed: %s", event_dict)
    return event_dict


def restore_newline(event_dict):
    try:
        event_dict["message"] = event_dict["message"]\
            .replace("\\n", "\n")\
            .replace("\\r", "\n")
...
def _new_event_id():
    # The event id size should be same as which code in `event.cc`.
    return binary_to_hex(np.random.bytes(18))


def new_event(label, custom_fields, message, severity="INFO"):
    event = {
    ...
    # Rewrite the keys because we don't support parsing key-values
    # from subfields of Json in SLS.
    if custom_fields:
        for key in ["job_id", "job_name", "node_id", "task_id"]:
            if key in custom_fields:
                event[key] = custom_fields[key]
    return event
