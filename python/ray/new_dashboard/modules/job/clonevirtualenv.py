...
def fixup_script_(root,
                  file_,
                  old_dir,
                  new_dir,
                  version,
                  rewrite_env_python=False):
    old_shebang = '#!%s/bin/python' % os.path.normcase(
        os.path.abspath(old_dir))
    new_shebang = '#!%s/bin/python' % os.path.normcase(
    ...
    elif (bang.startswith(old_shebang) and short_version
...
def fixup_pth_file(filename, old_dir, new_dir):
    ...
    for num, line in enumerate(lines):
        line = (line.decode('utf-8')
                if hasattr(line, 'decode') else line).strip()
...
def main():
    ...
    loglevel = (logging.WARNING, logging.INFO, logging.DEBUG)[min(
        2, options.verbose)]
