...
class LogHead(dashboard_utils.DashboardHeadModule):
    ...
    async def insert_log_url_to_node_info(self, node_info):
        ...
        if node_id is None:
            return
        agent_http_port = DataSource.agents.get(node_id, {}).get(
            dashboard_consts.DASHBOARD_AGENT_HTTP_PORT)
        if agent_http_port is None:
            return
        ip = node_info.get("raylet", {}).get("nodeManagerAddress")
        if ip is None:
            return
    ...
    async def get_log_index(self, req) -> aiohttp.web.Response:
        url_list = []
        agent_ips = []
        for node_id, data in DataSource.agents.items():
            ip = DataSource.node_id_to_ip[node_id]
            port = data[dashboard_consts.DASHBOARD_AGENT_HTTP_PORT]
            agent_ips.append(ip)
            url_list.append(self.LOG_URL_TEMPLATE.format(ip=ip, port=port))
    ...
    async def get_log_from_proxy(self, req) -> aiohttp.web.StreamResponse:
        ...
        async with self._proxy_session.request(
            ...
            try:
                ...
                # We omit the RuntimeError: Setting charset for
                # application/octet-stream doesn't make sense,
                # setup content_type first
                logger.warning("%s", ex)
