...
def test_profiling(fast_reporter_update, shutdown_only):
...
def test_node_physical_stats(enable_test_module, fast_reporter_update,
                             shutdown_only):
    addresses = ray.init(include_dashboard=True, num_cpus=6)
...
def test_run_cmd(ray_start_cluster_head, fast_reporter_update):
    ...
    raylet_proc_info = n1.all_processes[ray_constants.PROCESS_TYPE_RAYLET][0]
    ...
    def _run_ps():
        try:
            resp = requests.get(webui_url + f"/utils/ps?ip={ip}")
            resp.raise_for_status()
            result = resp.json()
            assert result["result"] is True, resp.text
            assert str(os.getpid()) in result["data"]["output"]
            return True
        except Exception as ex:
            logger.info(ex)
            return False

    wait_for_condition(_run_ps, timeout=10)
...
@pytest.mark.skip(reason="ANT-INTERNAL skipped the metrics exporter")
...
def test_enable_k8s_disk_usage(enable_k8s_disk_usage: bool):
    ...
    with patch.multiple(
        ...
        if enable_k8s_disk_usage:
            ...
            # Unless K8s disk usage display is enabled,
            # we should get dummy values.
            assert root_usage.total == 1
            assert root_usage.free == 1
