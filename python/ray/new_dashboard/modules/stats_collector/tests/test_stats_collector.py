...
def test_node_info(disable_aiohttp_cache, fast_reporter_update,
    ...
    while True:
        ...
        try:
            ...
            # Note: In ant internal codes, we have removed `GetViewData()`
            # in `node_manager.cc`, so do not check these two elements here.
            # Details see https://code.alipay.com/Arc/X/pull_requests/10204
...
def test_memory_table(disable_aiohttp_cache, fast_update_nodes,
                      ray_start_with_dashboard):
    assert (wait_until_server_available(ray_start_with_dashboard["webui_url"]))
...
def test_get_all_node_details(disable_aiohttp_cache, fast_update_nodes,
...
@pytest.mark.skip(reason="TO_BE_SOLVED: Fail to run in CI.")
@pytest.mark.parametrize(
    "ray_start_cluster_head", [{
        "include_dashboard": True
    }], indirect=True)
