...
def _str_to_bool(s, default_value=False):
    if s is None:
...
class Node:
    ...
    def __init__(self,
        ...
        if hasattr(ray_params, "node_name"):
            self._node_name = ray_params.node_name
        ...
        if head:
            ...
            # redis-server by the head node so that other nodes know whether
            # they should start to read them from redis.
            redis_client.set("GCS_READY", 1)
        if not connect_only:
    ...
    def _init_temp(self, redis_client):
        ...
        if self.head:
            ...
            # TODO(qwang): Refine this by adding `log-dir` option.
            if (self._ray_params.temp_dir is not None
                    and len(self._ray_params.temp_dir) != 0):
                self._temp_dir = self._ray_params.temp_dir
            else:
                self._temp_dir = ray._private.utils.decode(
                    redis_client.get("temp_dir"))
        ...
        if self.head:
            ...
            # TODO(qwang): This is a temporary change for arc/X,
            #  should be refined on Github.
            if self._temp_dir is not None:
                self._session_dir = os.path.join(self._temp_dir,
                                                 self.session_name)
            else:
                self._session_dir = ray._private.utils.decode(
        ...
        if self._ray_params.log_dir is not None:
            self._logs_dir = self._ray_params.log_dir
        else:
            self._logs_dir = os.path.join(self._session_dir, "logs")
        self._event_dir = self._logs_dir + "/events"
    ...
    def get_resource_spec(self):
        ...
        if not self._resource_spec:
            ...
            is_redis_deployed_in_head = self.head and \
                self._ray_params.external_redis_addresses is None
            ...
            self._resource_spec = ResourceSpec(
                ...
                if memory is None else memory, fin_object_store_memory,
                resources, self._ray_params.redis_max_memory).resolve(
                    is_redis_deployed_in_head=is_redis_deployed_in_head,
    ...
    def _get_cached_port(self,
        ...
        with FileLock(file_path + ".lock"):
            ...
            if (self.unique_id in ports_by_node
                ...
                # ANT-INTERNAL
                # TODO(buhe): just set port 0 if not specify it
                # TODO(buhe): disable metrics_export_port if not specify a
                # value
                # @buhe will fix this later
    ...
    def start_log_monitor(self):
        ...
        # ANT-INTERNAL BEGIN: We disabled log monitor process in Ant internal.
        if _str_to_bool(os.getenv("DISABLE_LOG_MONITOR"), default_value=True):
            logger.info("We disabled log monitor process in Ant internal Ray.")
            return
        # ANT-INTERNAL END: We disabled log monitor process in Ant internal.
    ...
    def start_dashboard(self, require_dashboard):
        ...
        Args:
            require_dashboard (bool): If true, this will raise an exception if
                we fail to start the webui. Otherwise it will print a warning
                if we fail to start the webui.
        """
    ...
    def start_monitor(self):
        ...
        """
        # ANT-INTERNAL BEGIN: We disabled monitor process by default in Ant.

        if _str_to_bool(os.environ.get("DISABLE_MONITOR"), default_value=True):
            logger.info("We disabled monitor process in Ant internal Ray.")
            return
        # ANT-INTERNAL END: We disabled monitor process in Ant internal.
    ...
    def kill_raylet(self, check_alive=True, clean_socket_file=False):
        """Kill the raylet.
        ...
        if clean_socket_file and self._raylet_socket_name:
            self.clean_temporary_file(self._raylet_socket_name)
        if clean_socket_file and self._plasma_store_socket_name:
            self.clean_temporary_file(self._plasma_store_socket_name)
    ...
    def check_processes_alive(self):
        """Check and protect the plasma store and raylet processes.
           ...
           1、Restart the unexpectedly terminated processes.
           2、[ha feature] Check if the role is downgraded from the active.
        """
        if self._ray_params.enable_head_ha\
           and hasattr(self, "leader_selector"):
            if self.leader_selector is not None and\
               not self.leader_selector.is_leader():
                msg = "This head node will be killed "\
                    "as it has changed from active to standby."
                logger.error(msg)
                raise RuntimeError(msg)

        fo_process_types = [ray_constants.PROCESS_TYPE_RAYLET]
        if self._curr_restart_retry_times > ray_constants. \
                RESTART_MAX_TIMES_WITHIN_MIN_SECONDS_BEFORE_DEAD:
            pass
            # logger.warning(
            #     "exceed failover attempt limit: %s > %s",
            #     self._curr_restart_retry_times,
            #     ray_constants.RESTART_MAX_TIMES_WITHIN_MIN_SECONDS_BEFORE_DEAD)

        for process_type in fo_process_types:
            if process_type not in self.all_processes:
                continue
            for process_info in self.all_processes[process_type]:
                ...
                logger.warning(
                    ...
                    "process(pid:{}) of type {}. return code {}".format(
                        pid, process_type, returncode))
                logger.warning("Killing Ray proccesses on this node.")
                # NOTE(kfstorm): Kill Raylet before killing Plasma Store. We
                # don't use `kill_all_processes` because it may kill head
                # processes such as GCS server as well. The processes to kill
                # here should align with the processes to be started in
                # `start_ray_processes`.
                self.kill_log_monitor(check_alive=False)
                self.kill_raylet(check_alive=False, clean_socket_file=True)
                # Exit code 120 means that the raylet shouldn't restart and
                # need be replaced with a new pod.
                # see kExitAndReplaceNodeCode.
                if returncode in [120]:
                    logger.error("Raylet {} exited with code {},"
                                 "don't restart it.".format(pid, returncode))
                    return False, False
                # Exit code 121 means that the raylet shouldn't restart and
                # release the pod to the k8s.
                # see kExitNodeCode.
                if returncode in [121]:
                    logger.error("Raylet {} exited with code {},"
                                 "delete it.".format(pid, returncode))
                    return False, True
                logger.warning("Restarting Ray processes on this node.")
                self.start_ray_processes()
        return True, False

    def clean_temporary_file(self, path):
        try:
            os.remove(path)
            logger.info("remove {} file success".format(path))
        except IOError as ex:
            logger.info("remove {} file failed for {}".format(path, ex))
