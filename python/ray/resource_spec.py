...
class ResourceSpec(
    ...
    def resolve(self, is_redis_deployed_in_head, node_ip_address=None):
        """Returns a copy with values filled out with system defaults.

        Args:
            is_redis_deployed_in_head (bool): Whether the redis is deployed in
                the head node.
        ...
        if memory is None:
            memory = (avail_memory - object_store_memory -
                      (redis_max_memory if is_redis_deployed_in_head else 0))
