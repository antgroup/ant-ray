...
@click.option(
    ...
    required=False,
    type=int,
    default=0,  # Disable auto port assignment for Ant-Ray
...
@click.option(
    "--log-dir",
    default=None,
    help="Manually specify the log directory of the Ray processes.")
@click.option(
    "--code-search-path",
    default=None,
    type=str,
...
@click.option(
    ...
    default=None,
    help=("Specify the external redis addresses separated by comma. The 1st "
          "one is the primary shard, the rests are the other shards."))
@click.option(
    "--metrics-service-address",
    default=None,
    help="The address that the metrics reporter will connect to.")
@click.option(
    "--metrics-username",
    default=None,
    help="Specify the username of metrics reporter.")
@click.option(
    "--metrics-job-name",
    default=None,
    help="Specify the job name of metrics reporter.")
@click.option(
    "--metrics-tag",
    default=None,
    help="Metrics tag should be specified like a dict split by colon, like:"
    "k1:v1:k2:v2:k3:v3")
@click.option(
    "--as-daemon",
    is_flag=True,
    default=False,
    help="If ture, `ray start` will run as a daemon and monitor raylet"
    " process.")
@click.option(
    "--node-name", required=False, type=str, help="the name of this node")
def start(node_ip_address, address, port, redis_password, redis_shard_ports,
          ...
          external_redis_addresses, metrics_service_address, metrics_username,
          metrics_job_name, metrics_tag, as_daemon, log_dir, code_search_path,
          node_name):
    ...
    if head:
        ...
        if external_redis_addresses is not None:
            ...
            if num_redis_shards is not None:
                ...
                if len(external_redis_addresses) != num_redis_shards + 1:
                    raise Exception("The length of --external-redis-addresses "
                ...
                if len(external_redis_addresses) == 1:
                    ...
                    # specified.
                    external_redis_addresses = external_redis_addresses * 2
                elif len(external_redis_addresses) != 2:
                    raise Exception(
                        "The length of --external-redis-addresses "
        ...
        ray_params.update_if_absent(
            ...
            external_redis_addresses=external_redis_addresses,
        )

        # Fail early when starting a new cluster when one is already running
        if address is None and external_redis_addresses is None:
    ...
    if as_daemon:
        while True:
            try:
                time.sleep(0.5)
                need_restart, need_delete = node.check_processes_alive()
                if not need_restart and need_delete:
                    # TODO optimization terminate action
                    cli_logger.error("This node will be terminated.")
                    os._exit(1)
            except Exception as err:
                cli_logger.error("This node will be terminated "
                                 "with an exception:" + repr(err))
                os._exit(1)
    if block:
        cli_logger.newline()
...
def status(address, redis_password):
    ...
    status = redis_client.hget(ray_constants.DEBUG_AUTOSCALING_STATUS, "value")
    error = redis_client.hget(ray_constants.DEBUG_AUTOSCALING_ERROR, "value")
