...
class SerializationContext:
    ...
    def _deserialize_object(self, data, metadata, object_ref):
        if metadata:
            ...
            if error_type == ErrorType.Value("TASK_EXECUTION_EXCEPTION"):
                ...
                return self._deserialize_worker_crashed_error(
                ...
                if error_info.HasField("runtime_env_setup_failed_error"):
                    error_msg = \
                        error_info.runtime_env_setup_failed_error.error_message
    ...
    def _deserialize_worker_crashed_error(self, data, metadata_fields):
        if not data:
            return WorkerCrashedError()
        ray_error_info = self._deserialize_error_info(data, metadata_fields)
        if ray_error_info.HasField("error_message"):
            return WorkerCrashedError(ray_error_info.error_message)
        else:
            return WorkerCrashedError()
