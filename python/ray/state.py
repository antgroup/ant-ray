...
from ray import gcs_utils, ray_constants
...
class GlobalState:
    ...
    def __init__(self):
        ...
        # The primary redis address of this Cluster.
        self.redis_address = None
    ...
    def get_all_resource_usage(self):
        ...
        batch_data_str = self.global_state_accessor.get_all_resource_usage()
        batch_data = \
            ray.gcs_utils.ResourceUsageBatchData.FromString(batch_data_str)
        result = {}
        for resources_data in batch_data.batch:
            node_id = ray._private.utils.binary_to_hex(resources_data.node_id)
            resources_available = resources_data.resources_available
            resources_total = resources_data.resources_total
            resources_normal_task = resources_data.resources_normal_task
            if node_id not in result:
                result[node_id] = {
                    "resources_total": resources_total,
                    "resources_available": resources_available,
                    "resources_normal_task": resources_normal_task
                }
        return result
    ...
    def get_cluster_resources(self):
        self._check_connected()
        all_node_resources = \
            self.global_state_accessor.get_cluster_resources()
        result = {}
        for data in all_node_resources:
            node_resources = ray.gcs_utils.NodeResources.FromString(data)
            node_id = ray._private.utils.binary_to_hex(node_resources.node_id)
            resources_available = node_resources.available_resources
            resources_total = node_resources.total_resources
            if node_id not in result:
                result[node_id] = {
                    "resources_total": resources_total,
                    "resources_available": resources_available
    ...
    def node_table(self):
        ...
        for node_info_item in node_table:
            ...
            node_info = {
                "NodeID": ray._private.utils.binary_to_hex(
                    item.basic_gcs_node_info.node_id),
                "Alive": item.basic_gcs_node_info.state ==
                gcs_utils.BasicGcsNodeInfo.GcsNodeState.Value("ALIVE"),
                "NodeManagerAddress": item.basic_gcs_node_info.
                node_manager_address,
                "NodeManagerHostname": item.basic_gcs_node_info.
                node_manager_hostname,
                "NodeManagerPort": item.basic_gcs_node_info.node_manager_port,
                "ObjectManagerPort": item.basic_gcs_node_info.
                object_manager_port,
    ...
    def job_table(self):
        ...
        Returns:
            ...
            - "NodeManagerHostname" (Hostname of the driver for this job),
        ...
        for i in range(len(job_table)):
            ...
            job_info["JobConfig"] = entry.config
    ...
    # TODO(qwang): Should be move to `ray.actors`
    def _execute_command_on_primary(self, *args):
        """Execute a Redis command on primary redis server.
        Args:
    ...
    def put_job_data(self, key, data):
        self._check_connected()
        self.global_state_accessor.put_job_data(key, data)

    def get_job_data(self, key):
        self._check_connected()
        return self.global_state_accessor.get_job_data(key)

    # TODO(qwang): name to actor_ids_for_job
    def actor_ids_for_driver(self, driver_id):
        """Get the actors for this driver.
        """
        self._check_connected()

        message = self.redis_client.execute_command(
            "SMEMBERS",
            b"DRIVER_RESOURCE:ACTOR:" + driver_id.binary(),
        )
        ...
        return [
            ray.ActorID(bytes_of_actor_id) for bytes_of_actor_id in message
        ]

    def update_job_resource_requirements(self, job_id,
                                         min_resource_requirements,
                                         max_resource_requirements):
        """Update the total resources of the specified job.
        """
        self._check_connected()
        result = False
        try:
            result = self.global_state_accessor.\
                update_job_resource_requirements(
                    job_id, min_resource_requirements,
                    max_resource_requirements)
...
def update_job_total_resources(total_memory_mb=None,
                               total_cpus=None,
                               total_gpus=None,
                               max_total_memory_mb=None,
                               max_total_cpus=None,
                               max_total_gpus=None):
    """Update the total resources of the current job.
    """
    return update_job_resource_requirements(
        job_id=ray.worker.global_worker.current_job_id,
        total_memory_mb=total_memory_mb,
        total_cpus=total_cpus,
        total_gpus=total_gpus,
        max_total_memory_mb=max_total_memory_mb,
        max_total_cpus=max_total_cpus,
        max_total_gpus=max_total_gpus)


def update_job_resource_requirements(job_id=None,
    ...
    if total_memory_mb is not None:
        if total_memory_mb % 50 != 0 or total_memory_mb <= 0:
            raise Exception(
                "total_memory_mb({}) must be positive integer multiple of 50.".
                format(total_memory_mb))
        else:
            min_resource_requirements["totalMemoryMb"] = total_memory_mb
            if max_total_memory_mb is None:
                max_resource_requirements["maxTotalMemoryMb"] = total_memory_mb

    if total_cpus is not None:
        if total_cpus <= 0:
            raise Exception(
                "total_cpus({}) must be greater than 0.".format(total_cpus))
        else:
            min_resource_requirements["totalCpus"] = total_cpus
            if max_total_cpus is None:
                max_resource_requirements["maxTotalCpus"] = total_cpus

    if total_gpus is not None:
        if total_gpus <= 0:
            raise Exception(
                "total_gpus({}) must be greater than 0.".format(total_gpus))
        else:
            min_resource_requirements["totalGpus"] = total_gpus
            if max_total_gpus is None:
                max_resource_requirements["maxTotalGpus"] = total_gpus

    if max_total_memory_mb is not None:
        if total_memory_mb is None:
            raise Exception("It is not allowed to set only the value of \
                max_total_memory_mb without setting the \
                value of total_memory_mb.")
        if max_total_memory_mb < total_memory_mb:
            raise Exception("max_total_memory_mb({}) must be positive \
                integer multiple of 50 and >= total_memory_mb({}).".format(
                max_total_memory_mb, total_memory_mb))
        else:
            max_resource_requirements["maxTotalMemoryMb"] = max_total_memory_mb

    if max_total_cpus is not None:
        if total_cpus is None:
            raise Exception("It is not allowed to set only the value of \
                max_total_cpus without setting the value of total_cpus.")
        if max_total_cpus < total_cpus:
            raise Exception(
                "max_total_cpus({}) must >= total_cpus({}).".format(
                    max_total_cpus, total_cpus))
        else:
            max_resource_requirements["maxTotalCpus"] = max_total_cpus

    if max_total_gpus is not None:
        if total_gpus is None:
            raise Exception("It is not allowed to set only the value of \
                max_total_gpus without setting the value of total_gpus.")
        if max_total_gpus < total_gpus:
            raise Exception("max_total_gpus({}) must >= total_gpus.".format(
                max_total_gpus))
        else:
            max_resource_requirements["maxTotalGpus"] = max_total_gpus

    if len(min_resource_requirements) == 0:
        raise Exception("Input parameters cannot be all none.")

    result = False
    try:
        result = state.update_job_resource_requirements(
...
def put_job_result(result):
    state.put_job_data(ray_constants.JOB_DATA_KEY_RESULT,
                       result.encode("utf-8"))


def get_job_result():
    return state.get_job_data(
        ray_constants.JOB_DATA_KEY_RESULT).decode("utf-8")
