...
def generate_config_map(**kwargs):
    result_config_map = {}
    if "job_config" in kwargs:
        result_config_map["_job_config"] = kwargs["job_config"]
        del kwargs["job_config"]
    result_config_map.update(generate_system_config_map(**kwargs))
    return result_config_map
...
def client_test_enabled() -> bool:
    return ray._private.client_mode_hook.client_mode_enabled
...
class TCPKiller:
    ...
    def block_tcp(self, port):
        """Use more than one tcpkill process to improve the success rate to
        disconnect the tcp connection, because the tcpkill cannot 100%
        guarantee the connection disconnected.
        """
        for _ in range(2):
            self.tcp_killers.append(
                subprocess.Popen(
                    f"sudo tcpkill -i lo -9 port {port}", shell=True))

    def unblock_tcp(self):
        for _ in range(len(self.tcp_killers)):
            subprocess.check_call(
                f"sudo pkill -9 -P {self.tcp_killers[_].pid}", shell=True)
