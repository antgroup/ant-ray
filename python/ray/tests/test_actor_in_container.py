...
@pytest.mark.skipif(sys.platform != "linux", reason="Only works on linux.")
def test_customize_worker_container_name_logdir():
    rand_test_name = random.randint(0, 65536)
    test_container_name = "customize_test_log_" + rand_test_name
    test_container_file = "container_test_" + rand_test_name
    test_share_file = "share_test_" + rand_test_name
    test_ray_log_file = "ray_log_test" + rand_test_name
    file_map = {
        "pod_container_file": "/home/admin/logs/" + test_container_name + "/" +
        test_container_file,
        "in_container_file": "/home/admin/logs/" + test_container_file,
        "pod_share_file": "/home/admin/logs/" + test_share_file,
        "container_share_file": "/home/admin/logs/share/" + test_share_file,
        "ray_log_file": "/home/admin/logs/ray-logs/" + test_ray_log_file
    }
    ...
    class TestActor(object):
        ...
        def touch_test_file(self):
            if pathlib.Path.exists(file_map["in_container_file"]):
                return False
            pathlib.Path.touch(file_map["in_container_file"])
            return True

        def read_share_file(self):
            return pathlib.Path.exists(file_map["container_share_file"])

        def read_ray_log_file(self):
            return pathlib.Path.exists(file_map["ray_log_file"])
    ...
    test_actor = TestActor(file_map).options(runtime_env=runtime_env).remote()

    f = test_actor.touch_test_file.remote()
    f.get()
    assert pathlib.Path.exists(file_map["pod_container_file"])
    pathlib.Path.touch(file_map["pod_share_file"])
    f = test_actor.read_share_file.remote()
    assert f.get()
    pathlib.Path.touch(file_map["ray_log_file"])
    f = test_actor.read_ray_log_file.remote()
    assert f.get()
