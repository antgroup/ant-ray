...
def test_client_thread_safe(call_ray_stop_only, shutdown_only):
    import ray
    ray.init(num_cpus=2)
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_interrupt_ray_get(call_ray_stop_only, shutdown_only):
    import ray
    ray.init(num_cpus=2)
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_real_ray_fallback(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_nested_function(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_put_get(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_put_failure_get(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_wait(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
        objectref = ray.put("hello world")
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_remote_functions(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_function_calling_function(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_basic_actor(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_pass_handles(ray_start_regular_shared, shutdown_only):
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_basic_log_stream(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
        log_msgs = []
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_stdout_log_stream(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
        log_msgs = []
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_serializing_exceptions(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_create_remote_before_start(ray_start_regular_shared, shutdown_only):
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_basic_named_actor(ray_start_regular_shared, shutdown_only):
...
def test_error_serialization(ray_start_regular_shared, shutdown_only):
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_internal_kv(ray_start_regular_shared, shutdown_only):
    with ray_start_client_server() as ray:
        assert ray._internal_kv_initialized()
        assert ray._internal_kv_put("apple", "b")
        assert not ray._internal_kv_put("apple", "asdf")
        ...
        # ANT-INTERNAL: conflict with api_server and api_server_rpc
        assert ray._internal_kv_list("app") == [b"apple"]
        ray._internal_kv_del("apple")
        assert ray._internal_kv_get("apple") is None


def test_startup_retry(ray_start_regular_shared, shutdown_only):
...
def test_dataclient_server_drop(ray_start_regular_shared, shutdown_only):
    from ray.util.client import ray as ray_client
    ray_client._inside_client_test = True
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_client_gpu_ids(call_ray_stop_only, shutdown_only):
    import ray
    ray.init(num_cpus=2)
...
def test_client_serialize_addon(call_ray_stop_only, shutdown_only):
    import ray
    import pydantic
...
@pytest.mark.skipif(sys.platform == "win32", reason="Failing on Windows.")
def test_client_load_code_from_local(shutdown_only):
    import os
    import ray
    ...
    with ray_start_client_server() as ray:
        ...
        class A:
            def foo(self):
                import ray
                assert ray.worker.global_worker.load_code_from_local is True
                return "foo"

        A.__module__ = "NotExistModule"
        A = ray.remote(A)
        a = A.remote()
        r = ray.get(a.foo.remote())
        assert r == "foo"
        ...
        bar.__module__ = "NotExistModule"
        bar = ray.remote(bar)
        r = ray.get(bar.remote())
        assert r == "bar"


def test_client_raise_stop_iteration(shutdown_only):
    from ray.util.client import ray
    ray.init()
