...
@pytest.mark.parametrize(
    "ray_start_regular", [{
        "_system_config": {
            "slow_task_print_log_interval_s": 2,
            "slow_actor_creation_task_send_event_interval_s": 1,
            "slow_non_actor_creation_task_send_event_interval_s": 2
        }
    }],
    indirect=True)
def test_slow_task(ray_start_regular):
    event_dir = ray_start_regular["event_dir"]

    def get_slow_task_events(keyword):
        return search_specific_event(event_dir, "CORE_WORKER",
                                     [("SLOW_TASK", keyword)])
    ...
    ray.get(slow_func.remote())

    # We expect the slow task event will be sent repeatedly.
    assert len(get_slow_task_events("slow_func")) > 1
    ...
    a = SlowActor.remote()
    ray.get(a.slow_method.remote())

    assert len(get_slow_task_events("__init__")) > 1
    assert len(get_slow_task_events("slow_method")) > 1


@pytest.mark.parametrize(
    "ray_start_regular", [{
        "_system_config": {
            "slow_actor_scheduling_duration_s": 2,
            "slow_actor_scheduling_send_event_interval_s": 1,
            "slow_actor_scheduling_check_interval_ms": 100,
        }
    }],
    indirect=True)
def test_slow_actor_scheduling_event(ray_start_regular):
    event_dir = ray_start_regular["event_dir"]

    def get_slow_actor_scheduling_events(keyword):
        return search_specific_event(event_dir, "GCS",
                                     [("SLOW_ACTOR_SCHEDULING", keyword)])
    ...
    assert len(get_slow_actor_scheduling_events("SLOW_ACTOR_SCHEDULING")) > 1
