...
@pytest.mark.skip(reason="TO_BE_SOLVED: need new gcs scheduler.")
...
@pytest.mark.parametrize(
    ...
    indirect=True)
def test_update_job_total_resources(ray_start_regular):
    job_table = ray.state.jobs()
    assert len(job_table) == 1
    # Test that the total_memory_mb to update must be a muliple of 50.
    got_exception = False
    try:
        ray.update_job_total_resources(total_memory_mb=55)
    except Exception:
        got_exception = True
    assert got_exception
    # Test that the input args must not be empty.
    got_exception = False
    try:
        ray.update_job_total_resources()
    except Exception:
        got_exception = True
    assert got_exception

    # Update the total memory to 50M.
    result = ray.update_job_total_resources(total_memory_mb=50)
    assert result is True
    job_table = ray.state.jobs()
    assert job_table[0]["JobConfig"].total_memory_units == 50 * 1024**2
    ...
    # Now the job's total memory is 100 MB.
    # Creating an actor that requires 500 MB and check it will be failed.
    actor = Actor.remote()
    ready_objects, remaining_objects = ray.wait(
        [actor.foo.remote()], timeout=3)
    assert len(ready_objects) == 0 and len(remaining_objects) == 1
    # Update the total memory to 100M.
    result = ray.update_job_total_resources(total_memory_mb=100)
    assert result is True
    job_table = ray.state.jobs()
    assert job_table[0]["JobConfig"].total_memory_units == 100 * 1024**2
    # Check that the above actor is now created.
    ready_objects, remaining_objects = ray.wait(
        [actor.foo.remote()], timeout=3)
    assert len(ready_objects) == 1 and len(remaining_objects) == 0
    # Scale down the total memory of this job to 50M.
    # Check that an exception will be thrown as the resources are still in use.
    got_exception = False
    try:
        ray.update_job_total_resources(total_memory_mb=50)
    except Exception:
        got_exception = True
    assert got_exception
    job_table = ray.state.jobs()
...
@pytest.mark.parametrize(
    ...
    indirect=True)
def test_update_job_total_resources_with_pg(ray_start_regular):
    job_table = ray.state.jobs()
    assert len(job_table) == 1
    # Update the total memory to 50M.
    result = ray.update_job_total_resources(total_memory_mb=250)
    assert result is True
    job_table = ray.state.jobs()
    assert job_table[0]["JobConfig"].total_memory_units == 250 * 1024**2

    @ray.remote(memory=50 * MB)
    class Actor:
        def __init__(self):
            pass
    ...
    assert ray.get(actor.foo.remote()) == "OK"

    # Check failed to scale down the total memory to 100M.
    got_exception = False
    try:
        ray.update_job_total_resources(total_memory_mb=100)
    except Exception:
        got_exception = True
    assert got_exception

    assert ray.update_job_total_resources(total_memory_mb=200)

    job_table = ray.state.jobs()
    assert job_table[0]["JobConfig"].total_memory_units == 200 * 1024**2

    assert ray.update_job_total_resources(total_memory_mb=150)
