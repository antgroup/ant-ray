...
def test_leader_selector_init():
    ...
    leader_selector = RedisBasedLeaderSelector(ray_params, "0.0.0.0:0",
                                               "0.0.0.0")
    ...
    assert leader_selector._config.wait_pre_gcs_stop_max_time_s \
        == 3600 * 24 * 3
...
def check_ha_env_config(request):
    ...
    os.environ["RAY_HA_CHECK_MAX_FAILURE_COUNT"] = request.param[
        "max_failure"][0]
    ...
    os.environ["RAY_HA_WAIT_TIME_AFTER_BE_ACTIVE_MS"] = request.param[
        "wait_time"][0]
...
def test_leader_selector_init_param(check_ha_env_config):
    ...
    leader_selector = RedisBasedLeaderSelector(ray_params, "0.0.0.0:0",
                                               "0.0.0.0")
    assert str(leader_selector.name, "utf-8").startswith("0.0.0.0:0")
    assert leader_selector._config.check_interval_s == result[
        "check_inverval"][1]
    assert leader_selector._config.key_expire_time_ms == result["expire_time"][
        1]
    assert leader_selector._config.max_failure_count == result["max_failure"][
        1]
    assert leader_selector._config.wait_pre_gcs_stop_max_time_s == result[
        "wait_gcs_max_time"][1]
    assert leader_selector._config.wait_time_after_be_active_s == result[
        "wait_time"][1]
