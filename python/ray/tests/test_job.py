...
ray.shutdown()
    ...
    # ANT-INTERNAL: make the case more stable.
    jobs = []
    for _ in range(10):
        time.sleep(1)
...
def test_kill_job(call_ray_start):
    address = call_ray_start
...
class Actor:
    ...
    def kill_job(self):
        ray.kill_current_job()
        return true

a = Actor.options().remote()
obj = a.kill_job.remote()
try:
    assert ray.get(obj)
...
""".format(address)
    ...
    run_string_as_driver_nonblocking(driver)
    # Wait for actor to be created
    wait_for_num_actors(1)
    actor_table = ray.state.actors()
    assert len(actor_table) == 1
    ...
    def has_dead_job():
        ...
        has_dead_job = False
        for job_info in job_table:
            if job_info["IsDead"]:
                has_dead_job = True
        return has_dead_job

    wait_for_condition(has_dead_job)
