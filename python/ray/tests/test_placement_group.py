...
def test_atomic_creation(ray_start_cluster, connect_to_client):
    ...
    for i in range(num_nodes):
        ...
        cluster.add_node(
            num_cpus=bundle_cpu_size * bundle_per_node,
            memory=500 * MB,
...
@pytest.mark.skip(
    reason="Skip this ut because we don't support detached actor temporarily")
...
a = A.options(detached=True, name="A").remote()
ray.get(a.create_pg.remote())
...
ray.shutdown()
    ...
    # Make sure the placement group is not removed when the detached actor
    # is killed for the first time.
    ray.kill(a, no_restart=False)
    wait_for_condition(lambda: assert_num_cpus(num_nodes * num_cpu_per_node))

    # The detached actor `a` should've been restarted.
    ready, _ = ray.wait([a.is_alive.remote()], timeout=5)
    assert len(ready) == 1
    wait_for_condition(lambda: assert_num_cpus(num_nodes))

    # Kill it again and make sure the placement group is removed.
    ray.kill(a, no_restart=False)
    wait_for_condition(lambda: assert_num_cpus(num_nodes * num_cpu_per_node))
...
def test_placement_group_wait_api(ray_start_cluster_head):
    ...
    # Create placement group 1 successfully.
    placement_group1 = ray.util.placement_group(
    ...
    # Create placement group 2 successfully.
    placement_group2 = ray.util.placement_group(
        strategy="STRICT_SPREAD", bundles=[{
            "CPU": 1
        }, {
            "CPU": 1
...
@pytest.mark.parametrize(
    "ray_start_cluster_head", [
        generate_system_config_map(
            gcs_create_placement_group_retry_interval_ms=200)
    ],
    indirect=True)
def test_schedule_placement_groups_at_the_same_time(ray_start_cluster_head):
    cluster = ray_start_cluster_head
    cluster.add_node(num_cpus=4)
    cluster.wait_for_nodes()
...
@pytest.mark.skip(
    reason="""Skip this ut because we have a different detached semantcis
    in ant internal.""")
