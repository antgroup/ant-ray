...
def _fill_object_store_and_get(obj, succeed=True, object_MiB=20,
    ...
    def object_exists(o):
        return ray.worker.global_worker.core_worker.object_exists(o)

    if succeed:
        wait_for_condition(lambda: object_exists(obj), timeout=10)
    else:
        wait_for_condition(lambda: not object_exists(obj), timeout=10)
...
def test_actor_holding_serialized_reference(one_worker_100MiB, use_ray_put,
    ...
    if failure:
        ...
        with pytest.raises(ray.exceptions.RayActorError):
            ray.get(actor.delete_ref1.remote())
        # Note: This sleep is used to let this driver has received the actor
        # dead message.
        time.sleep(1)
    else:
...
# Test that a reference borrowed by an actor constructor is
# freed if the actor is cancelled before being scheduled.
def test_actor_constructor_borrow_cancellation(ray_start_regular):
    # Schedule the actor with a non-existent resource so it's
    # guaranteed to never be scheduled.
    @ray.remote(resources={"nonexistent_resource": 1})
    class Actor:
        def __init__(self, obj_containing_ref):
            raise ValueError("The actor constructor should not be reached; "
                             "the actor creation task should be cancelled "
                             "before the actor is scheduled.")

        def should_not_be_run(self):
            raise ValueError("This method should never be reached.")

    # Test with implicit cancellation by letting the actor handle
    # go out-of-scope.
