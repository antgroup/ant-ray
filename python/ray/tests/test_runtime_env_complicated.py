...
# bazel test and pytest have different path of __file__, make sure get the
# right path for them.
def get_yaml_path():
    yaml_path = "experimental/packaging/example_pkg/ray_pkg.yaml"
    current = os.path.dirname(__file__)
    pos = current.find("bazel-out")
    # For bazel test
    if pos != -1:
        return current[:pos] + "python/ray/" + yaml_path
    else:
        # For pytest
        pos = current.find("test")
        return current[:pos] + yaml_path


@unittest.skipIf(sys.platform == "win32", "Fail to create temp dir.")
def test_experimental_package(shutdown_only):
    ray.init(num_cpus=2)
    pkg = ray.experimental.load_package(get_yaml_path())
