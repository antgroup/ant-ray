...
def test_hybrid_policy(ray_start_cluster):
    ...
    num_nodes = 2
    num_cpus = 10
    memory_units = 50 * 1024**2
    for _ in range(num_nodes):
        cluster.add_node(num_cpus=num_cpus, memory=num_cpus * memory_units)
    ...
    @ray.remote(num_cpus=1, memory=1 * memory_units)
    def get_node():
...
def test_legacy_spillback_distribution(ray_start_cluster):
    ...
    cluster.add_node(
        num_cpus=0, _system_config={"scheduler_spread_threshold": 0})
    ray.init(address=cluster.address)
    cluster.wait_for_nodes()
...
@pytest.mark.skipif(
    gcs_task_scheduling_enabled(),
    reason="GCS actor scheduler does not support GPU avoidance for now.")
@pytest.mark.skipif(sys.platform == "win32", reason="Fails on windows")
