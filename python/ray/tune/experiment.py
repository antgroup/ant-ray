...
class Experiment:
    ...
    def __init__(self,
        ...
        if self.upload_dir:
            ...
            def oss_prefix_replace(oss_dir):
                from ray.tune.sync_client import OSS_PREFIX, S3_PREFIX
                if oss_dir is not None and oss_dir.startswith(OSS_PREFIX):
                    oss_dir = oss_dir.replace(OSS_PREFIX, S3_PREFIX)
                return oss_dir

            self.upload_dir = oss_prefix_replace(self.upload_dir)
            self.remote_checkpoint_dir = os.path.join(self.upload_dir,
                                                      self.dir_name)
        else:
