...
def checkpoint_deleter(trial_id, runner):
    ...
    def delete(checkpoint):
        ...
        if checkpoint.storage == Checkpoint.PERSISTENT and checkpoint.value:
            ...
            # Note(qwang): We should make sure that the checkpoint was deleted
            # from durable trainer before we remove it from local. This is
            # used to avoid the case that the trial and the durable trainer
            # run on the same node and then causes the OSS checkpoints leak.
            deleted = ray.get(
                runner.delete_checkpoint.remote(checkpoint.value))
            if deleted:
                logger.debug("Succeeded to delete trainer's checkpoint.")
            else:
                logger.warning("Failed to delete trainer's checkpoint.")
