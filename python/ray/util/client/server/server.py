    ...
    def lookup_or_register_func(
        ...
        with disable_client_hook():
            if id not in self.function_refs:
                ...
                # Indicate that this function is from a ray client
                # In the function manager, we will force loading this
                # function from GCS, instead of from local.
                func.__from_ray_client__ = True
    ...
    def lookup_or_register_actor(self, id: bytes, client_id: str,
        ...
        with disable_client_hook():
            if id not in self.registered_actor_classes:
                ...
                # Indicate that this class is from a ray client
                # In the function manager, we will force loading this
                # class from GCS, instead of from local.
                actor_class.__from_ray_client__ = True
...
def serve(connection_str, ray_connect_handler=None):
    ...
    with disable_client_hook():
        ray_init_by_server = (not ray.is_initialized())
    ...
    data_servicer = DataServicer(task_servicer, ray_init_by_server)
