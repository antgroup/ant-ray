...
class RayInternalKvStore:
    def __init__(self):
        # ANT-INTERNAL: In Ray1.4, internal_kv API doesn't need gcs_client.
    ...
    def set(self, key: str, data: bytes) -> bool:
        key = "{}-{}".format(ray.get_runtime_context().job_id.hex(), key)
    ...
    def wait(self, keys: list):
        while True:
            all_exist = True
            for key in keys:
                key = "{}-{}".format(ray.get_runtime_context().job_id.hex(),
                                     key)
