...
class _ray_internal_insight_monitor:
    ...
    def get_call_graph_data(self, job_id):
        ...
        # Add data flows
        for flow_data in self.data_flows.get(job_id, []):
            source, target = flow_data["flow_key"].split("->")
            ...
            if source_id and target_id:
                # Calculate speed in MB/s (assuming size is in bytes)
                size_mb = flow_data["size"] / (1024 * 1024) / flow_data["duration"] # Convert bytes to MB
                speed = f"{size_mb:.2f}MB/s"
    ...
    def emit_object_record_get(self, recv_record):
        ...
        callee_class = recv_record["recv_class"]
        callee_func = recv_record["recv_func"]
...
def record_object_arg_get(object_id):
    ...
    if caller_class is not None and caller_class.startswith("_ray_internal_insight_monitor"):
        return
    ...
    current_task_name = ray.get_runtime_context().get_task_name()
    if current_task_name is not None:
        recv_func = current_task_name.split(".")[-1]
    else:
        recv_func = "main"
    ...
    ray.get(monitor.emit_object_record_get.remote(object_recv_record))
...
def record_object_put(object_id, size):
    ...
    ray.get(monitor.emit_object_record_put.remote(object_record))


def record_object_arg_put(object_id, size, callee):
    if object_id == "ffffffffffffffffffffffffffffffffffffffffffffffffffffffff":
        return
    callee_class = None
    ...
    if callee_class is not None and callee_class.startswith("_ray_internal_insight_monitor"):
        return
...
def record_object_get(object_id, task_id):
    ...
    if task_id.actor_id() == monitor._ray_actor_id:
        return
