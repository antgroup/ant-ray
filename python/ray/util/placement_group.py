...
class PlacementGroup:
    ...
    def add_bundles(self, bundles: List[Dict[str, float]]):
        ...
        Returns:
            throw a runtime exception if the state of current
            placement is not `Created` or `Updated`.
        """
        worker = ray.worker.global_worker
        worker.check_connected()

        validate_bundles(bundles)
        convert_memory_from_bytes_to_unit_if_exist(bundles)
    ...
    def remove_bundles(self, bundle_indexes: List[int]):
        """

        Args:
            bundle_indexes(List[int]):
                A list of bundle indexes to be removed.

        Returns:
            throw a runtime exception if the bundle index isn't exist.
        """
        worker = ray.worker.global_worker
        worker.check_connected()

        assert (self.id is not None
                ), "Can't remove bundles before create the placement group."
        worker.core_worker.remove_placement_group_bundles(
            self.id, bundle_indexes)
        self._fill_bundle_cache_if_needed(True)
...
def convert_memory_from_bytes_to_unit_if_exist(
        bundles: List[Dict[str, float]]):
    """
    convert memory resource to unit if memory specified.
    ...
    Returns:
        Throw `ValueError` if memory specified and can't convert to unit.
    """
    for bundle in bundles:
        # Convert unit if memory resource specified.
        if "memory" in bundle and bundle["memory"] != 0:
            bundle["memory"] = to_memory_units(bundle["memory"], round_up=True)
...
def get_placement_group(placement_group_name: str) -> PlacementGroup:
    ...
    if placement_group_info is None:
        raise ValueError("Failed to look up placement group"
                         f" with name: {placement_group_name}")
    else:
