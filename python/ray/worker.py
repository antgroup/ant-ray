...
class Worker:
    ...
    def run_function_on_all_workers(self, function, run_on_other_jobs=False):
        ...
        Args:
            ...
            run_on_other_jobs: The boolean that indicates whether we want to
                run this function on other jobs. One case is we may need to
                share objects across jobs.
        """
        ...
        if self.mode is None:
            ...
            self.redis_client.hset(
                ...
                mapping={
                    ...
                    "run_on_other_jobs": str(run_on_other_jobs),
                })
...
def get_gpu_ids():
    ...
    for resource, assignment in all_resource_ids.items():
        ...
        # ANT-INTERNAL: "nvidia.com/" is used in internal
        import re
        if resource == "GPU" or resource.startswith("nvidia.com/") or re.match(
                r"^GPU_group_[0-9A-Za-z]+$", resource):
...
def init(
    ...
    if job_config and job_config.code_search_path:
        ...
        #     ray.shutdown()
        #     ray.init()
        #     # Here the flag `load_code_from_local` is still True.
        #     ray.shutdown()
...
def kill_current_job():
    """Kill current job. This API doesn't gurantee that this
    job will be killed eventually. It just sends kill job RPC to GCS.

    This API is not allowed in driver.
    It raises RaySystemError when error happening during killing.
    """

    worker = global_worker
    worker.check_connected()
    job_id = ray.get_runtime_context().job_id
    ok = worker.core_worker.kill_job(job_id)
    if not ok:
        raise ray.exceptions.RaySystemError(
            f"Error happens during killing job ${job_id}")
...
def shutdown(_exiting_interpreter=False):
    ...
    if hasattr(global_worker, "core_worker"):
        if not _exiting_interpreter or global_worker.mode != WORKER_MODE:
            global_worker.core_worker.shutdown(_exiting_interpreter)
...
def connect(node,
    ...
    try:
        ...
        # In Ant internal, we support to run job with different python version
        logger.warning("Python version check exception: {}".format(e))
        # if mode == SCRIPT_MODE:
        #     raise e
    ...
    if mode == SCRIPT_MODE:
        # ANT-INTERNAL: This is required for GCS tasks scheduling.
        if not ray_constants.is_multiple_of_memory_unit(
                job_config.java_worker_process_default_memory_mb * 1024 *
                1024):
            raise ValueError("Invalid java_worker_process_default_memory_mb(" +
                             job_config.java_worker_process_default_memory_mb +
                             "). It should be multiple of 50.")
    ...
    def decorator(function_or_class):
        ...
        if inspect.isclass(function_or_class):
            ...
            return ray.actor.make_actor(function_or_class, num_cpus, num_gpus,
                                        ...
                                        max_task_retries, scheduling_strategy)
...
def remote(*args, **kwargs):
    ...
    It can also be used with specific keyword arguments as follow:
