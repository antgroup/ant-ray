...
def setup(input_args):
    ...
    if args.worker_entrypoint:
        worker_executable = args.worker_entrypoint

    # In ANT-INTERNAL, it is a temporary workaroud for install
    # some pip dependencies
    if runtime_env.get("pip") and not runtime_env.get("conda"):
        pip_list = runtime_env["pip"]
        # The flag '-U' can upgrade or downgrade package version
        pip_install_command = ["pip", "install", "-U"]
        pip_install_command.extend(pip_list)
        logger.info(
            "Staring install pip package: {}".format(pip_install_command))
        pip_install_result = subprocess.run(pip_install_command)
        if pip_install_result.returncode != 0:
            logger.warning("Failed to install pip package.")
            sys.exit(1)
        else:
            logger.info("Succeeded to install pip package.")
    elif runtime_env.get("conda") or runtime_env.get("pip"):
