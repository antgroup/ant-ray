...
TEST(ActorWorkerAssignmentIDTest, TestJobId) {
  {
    auto job_id = JobID::FromInt(23456);
    auto actor_worker_assignment_id = ray::GenerateActorWorkerAssignmentId(job_id);
    ASSERT_EQ(ray::GetJobIdFromActorWokerAssignmentId(actor_worker_assignment_id),
              job_id);
  }
}
