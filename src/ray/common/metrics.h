...
inline ray::stats::Count GetRedisOperationCountCounterMetric() {
  return ray::stats::Count{
      /*name=*/"redis_operation_count",
      /*description=*/"Number of operations invoked on Redis",
      /*unit=*/"",
      /*tag_keys=*/{"Operation", "TableName"},
  };
}

inline ray::stats::Sum GetRedisOperationDataSizeBytesSumMetric() {
  return ray::stats::Sum{
      /*name=*/"redis_operation_data_size_bytes",
      /*description=*/
      "Data size of an operation on Redis includes input data size for write "
      "operations and output data size for read operations",
      /*unit=*/"bytes",
      /*tag_keys=*/{"Operation", "TableName", "Type"},
  };
}
