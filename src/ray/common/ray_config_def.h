...
// ANT-INTERNAL
#include "ray/common/ray_config_def.internal.h"
...
RAY_CONFIG(uint64_t, raylet_whole_report_resources_period_milliseconds, 10000)
...
/// Max number of times create worker with runtime env.
RAY_CONFIG(int64_t, worker_register_timeout_max_retries, 3)
...
RAY_CONFIG(uint32_t, gcs_lease_worker_retry_interval_ms, 5000)
/// Duration to wait between retries for creating actor in gcs server.
RAY_CONFIG(uint32_t, gcs_create_actor_retry_interval_ms, 5000)
/// Duration to wait between retries for creating placement group in gcs server.
RAY_CONFIG(uint32_t, gcs_create_placement_group_retry_interval_ms, 5000)
...
RAY_CONFIG(int32_t, ping_gcs_rpc_server_max_retries, 86400)
...
RAY_CONFIG(int64_t, metrics_report_batch_size, 500)
...
/// The duration between heartbeats sent by the dashboard agent.
RAY_CONFIG(uint64_t, agent_heartbeat_period_milliseconds, 2000)

/// If the agnet has not sent a heartbeat in the last num_heartbeats_timeout
/// heartbeat intervals, the raylet monitor process will restart it.
RAY_CONFIG(int64_t, agent_num_heartbeats_timeout, 60)
...
RAY_CONFIG(int64_t, gcs_server_request_timeout_seconds, 15)

/// Whether to enable worker prestarting: https://github.com/ray-project/ray/issues/12052
// ANT-INTERNAL: We disable worker prestarting internally. Prestarting workers doesn't
// work for Java as Java workers normally has dynamic worker options. Workers prestarted
// has no dynamic worker options and can't be used by Java actors.
...
RAY_CONFIG(uint64_t, metrics_report_interval_ms, 60000)
...
/// Worker porfiling duration millseconds.
RAY_CONFIG(uint32_t, worker_profiling_duration_ms, 60 * 1000)
...
/// The threshold time for placement group scheduling slow.
RAY_CONFIG(uint64_t, placement_group_scheduling_slow_threshold_time_s,
           /*  10 min */ 60 * 10)

/// The interval time for detect placement group scheduling slow.
RAY_CONFIG(uint64_t, placement_group_detect_scheduling_slow_interval_time_s,
           /*  5 min */ 60 * 5)
