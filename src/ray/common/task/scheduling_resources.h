...
const std::string kMEM_ResourceLabel = "MEM";
const std::string kMemory_ResourceLabel = "memory";
const std::string kBundle_ResourceLabel = "bundle";

/// ======== ANT-INTERNAL below =========
const std::string kBigMemory_ResourceLabel = "BIG_MEMORY";
const std::string kACPU_ResourceLabel = "ACPU";
/// ======== ANT-INTERNAL above =========
...
class ResourceSet {
 public:
  ...
  /// \param need need_restore: A flag to represent whether add the returned resoruces
  /// back to `resource_capacity_`
  /// \return A flag indicate that whether return successful or not.
  bool ReturnBundleResources(const PlacementGroupID &group_id, const int bundle_index,
                             bool need_restore = true);

  bool ReturnBundleResources(const ResourceSet &bundle_resources,
                             bool need_restore = true);
  ...
  /// Return ture if resource_capacity_ constains rare resource label.
  ///
  /// \return Return ture if resource_capacity_ constains rare resource label.
  bool ContainsRareResources() const;

  /// Return ture if resource_capacity_ constains placement group.
  ///
  /// Return ture if resource_capacity_ constains placement group.
  bool ContainsPlacementGroup() const;

  /// Return ture if resource_capacity_ constains the specified placement group.
  ///
  /// \param placement_group_id.
  /// Return ture if resource_capacity_ constains the specified placement group.
  bool ContainsPlacementGroup(const PlacementGroupID &placement_group_id,
  ...
  /// Return the unit of memory.
  ///
  /// \return unit of memory.
  const ResourceSet GetMemory() const;
...
class SchedulingResources {
 public:
  ...
  ///
  /// \param[out] realtime_available_resources The realtime available resources (with
  /// normal task resources subtracted).
  /// \return Immutable set of resources with currently available capacity.
  const ResourceSet &GetAvailableResources(
      ResourceSet *realtime_available_resources = nullptr) const;
  ...
  void SubtractResource(const ResourceSet &other);
  ...
  /// \return True if resources sufficient, False otherwise.
  bool PrepareBundleResources(const PlacementGroupID &group, const int bundle_index,
  ...
  /// \brief Get node-wide runtime resources.
  ///
  /// \return Node-wide runtime resources.
  const ResourceSet &GetNodeRuntimeResources() const;

  /// \brief Set the node-wide runtime resources.
  ///
  /// \param newset: The new resource set to update.
  /// \return Void.
  void SetNodeRuntimeResources(const ResourceSet &newset);

  /// \brief Get the total required resources.
  ///
  /// \return Total required resources.
  const ResourceSet &GetTotalRequiredResources() const;

  /// \brief Get the mutable total required resources.
  ///
  /// \return Total required resources.
  ResourceSet &GetMutableTotalRequiredResources();
 ...
 private:
  ...
  /// TODO(jinwei): Node-wide runtime resources (to be deleted).
  ResourceSet resources_node_runtime_;
  /// The total required (logical) resources of PGs and
  /// non-PG-dependent actors.
  ResourceSet resources_total_required_;
};
