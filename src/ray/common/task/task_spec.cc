...
ActorID TaskSpecification::SourceActorId() const {
  return ActorID::FromBinary(message_->source_actor_id());
}
...
bool TaskSpecification::IsNoReply() const { return message_->no_reply(); }

bool TaskSpecification::EnableTaskFastFail() const {
  return message_->actor_task_spec().enable_task_fast_fail();
}

std::vector<ConcurrentGroup> TaskSpecification::ConcurrentGroups() const {
  RAY_CHECK(IsActorCreationTask());
  std::vector<ConcurrentGroup> concurrent_groups;
  auto &actor_creation_task_spec = message_->actor_creation_task_spec();
  const auto size = actor_creation_task_spec.concurrent_groups().size();
  for (auto i = 0; i < size; ++i) {
    auto &curr_group_message = actor_creation_task_spec.concurrent_groups(i);
