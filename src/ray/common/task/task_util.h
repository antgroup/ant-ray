 ...
 public:
  ...
  TaskSpecBuilder &SetCommonTaskSpec(
      ...
      const std::unordered_map<std::string, std::string> &override_environment_variables =
          {},
      ActorID source_actor_id = ActorID::Nil(), bool no_reply = false) {
    ...
    *message_->mutable_function_descriptor() = function_descriptor->GetMessage();
    ...
    message_->set_source_actor_id(source_actor_id.Binary());
    ...
    message_->set_no_reply(no_reply);
    return *this;
  }
  ...
  TaskSpecBuilder &SetActorCreationTaskSpec(
      ...
      const std::vector<ConcurrentGroup> &concurrent_groups = {},
      const std::string &extension_data = "",
      const std::unordered_map<std::string, std::string> &extended_properties = {},
    ...
    actor_creation_spec->mutable_extended_properties()->insert(
        extended_properties.begin(), extended_properties.end());
    for (const auto &concurrent_group : concurrent_groups) {
      rpc::ConcurrentGroup *group = actor_creation_spec->add_concurrent_groups();
      group->set_name(concurrent_group.name);
      group->set_max_concurrency(concurrent_group.max_concurrency);

      // Fill into function descriptor.
      for (auto &item : concurrent_group.function_descriptors) {
        rpc::FunctionDescriptor *fd = group->add_function_descriptors();
        *fd = item->GetMessage();
      }
    }
    message_->mutable_scheduling_strategy()->CopyFrom(scheduling_strategy);
    actor_creation_spec->mutable_labels()->insert(labels.begin(), labels.end());
    return *this;
  }
