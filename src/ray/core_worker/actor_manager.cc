...
ActorID ActorManager::RegisterActorHandle(std::unique_ptr<ActorHandle> actor_handle,
  ...
  RAY_UNUSED(AddActorHandle(std::move(actor_handle), call_site, caller_address, actor_id,
                            actor_creation_return_id, is_self));
...
std::pair<std::shared_ptr<const ActorHandle>, Status> ActorManager::GetNamedActorHandle(
  ...
  if (actor_id.IsNil()) {
    ...
    RAY_CHECK_OK(gcs_client_->Actors().AsyncGetByName(
        name, ray_namespace,
        [this, &actor_id, name, ray_namespace, call_site, &caller_address, ready_promise](
            Status status, const boost::optional<rpc::ActorTableData> &result) {
          if (status.ok() && result) {
            auto actor_handle =
                std::make_unique<ActorHandle>(*result, name, ray_namespace);
            actor_id = actor_handle->GetActorID();
            AddNewActorHandle(std::move(actor_handle), call_site, caller_address,
                              /*is_detached*/ true);
          } else {
    ...
    if (ready_promise->get_future().wait_for(std::chrono::seconds(
      ...
      stream << "There was timeout in getting the actor handle with name '" << name
             << "'. It is probably "
                "because GCS server is dead or there's a high load there.";
      auto error_msg = stream.str();
      RAY_LOG(ERROR) << error_msg;
      return std::make_pair(nullptr, Status::TimedOut(error_msg));
    }
  } else {
    ...
    AddActorHandle(std::make_unique<ActorHandle>(serialized_actor_handle), call_site,
                   caller_address, actor_id, ObjectID::ForActorHandle(actor_id));
  }
...
bool ActorManager::AddNewActorHandle(std::unique_ptr<ActorHandle> actor_handle,
  ...
  return AddActorHandle(std::move(actor_handle), call_site, caller_address, actor_id,
...
void ActorManager::WaitForActorOutOfScope(
  ...
  if (it == actor_handles_.end()) {
    ...
    auto callback = [this, actor_id, actor_handle,
...
void ActorManager::SubscribeActorState(const ActorID &actor_id) {
  {
    ...
    if (iter != subscribed_actors_.end()) {
      if (!iter->second.second.emplace(worker_id_).second) {
        return;
      }
    } else {
      subscribed_actors_[actor_id] = {/*valid*/ true, {worker_id_}};
    }
  }
  ...
  auto actor_notification_callback =
      std::bind(&ActorManager::HandleActorStateNotification, this, std::placeholders::_1,
                std::placeholders::_2);
  shared_actor_info_accessor_->AsyncSubscribe(
      ...
      [this, actor_id, cached_actor_name](Status status) {
        if (status.ok() && !cached_actor_name.empty()) {
          ...
          absl::MutexLock lock(&cache_mutex_);
          auto iter = subscribed_actors_.find(actor_id);
          if (iter != subscribed_actors_.end() && iter->second.first) {
...
bool ActorManager::IsActorKilledOrOutOfScope(const ActorID &actor_id) const {
  absl::MutexLock lock(&cache_mutex_);
  auto iter = subscribed_actors_.find(actor_id);
  return iter == subscribed_actors_.end() || !iter->second.first;
}
