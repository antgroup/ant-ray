...
using ActorEventCallbackType =
    std::function<void(const ActorID &, const rpc::ActorTableData &)>;

using ActorMigrationEventCallbackType =
    std::function<void(const rpc::ActorMigrationNotificationBatch &)>;
...
struct CoreWorkerOptions {
  ...
  /// Function to call worker profile handler.
  std::function<void(bool)> profile_handler;
...
class CoreWorkerProcess : public rpc::CoreWorkerServiceHandler {
 public:
  ///
  ...
  static void Shutdown(bool shutdown_with_error);
  ...
  /// Subscribe actor state change events.
  ///
  /// \return True if we succeeded to subscribe.
  static bool SubscribeActorStateChangeEvents(
      ActorEventCallbackType actor_event_callback);

  /// Subscribe actor migration events.
  ///
  /// \return True if we succeeded to subscribe.
  static bool SubscribeActorMigrationEvents(
...
#define DECLARE_VOID_RPC_SERVICE_HANDLER_METHOD(METHOD)                               \
  void Handle##METHOD(const rpc::METHOD##Request &request, rpc::METHOD##Reply *reply, \
...
#undef DECLARE_VOID_RPC_SERVICE_HANDLER_METHOD
 ...
 private:
  ...
  /// Set of worker ID. A worker ID is added to this set when it starts creation, so we
  /// can delay some events if the worker is not ready yet.
  std::unordered_set<WorkerID> worker_ids_ GUARDED_BY(worker_map_mutex_);
  ...
  /// Common rpc service for all worker modules. (brpc stream)
  std::unique_ptr<rpc::CoreWorkerBrpcStreamService> brpc_stream_service_;

  /// RPC server used to receive tasks to execute.
  std::unique_ptr<rpc::BrpcServer> core_worker_server_;

  /// Statistics and metrics. (Ant-version ray only.)
  CoreWorkerStats worker_stats_;
...
class CoreWorker : public rpc::CoreWorkerServiceHandler,
                   public std::enable_shared_from_this<CoreWorker> {
// ANT-INTERNAL
#include "ray/core_worker/core_worker.internal.core_worker.h"
 ...
 public:
  ...
  /// Mark that an error occurred and the CoreWorker instance is about to shut down.
  void MarkShutdownWithError();
  ...
  const std::string &GetCurrentNodeName() const { return node_name_; }
  ...
  Status SubmitTask(const RayFunction &function,
                    ...
                    int max_retries, bool retry_exceptions,
  ...
  /// Remove bundles from the specific placement group.
  ///
  /// \param[in] placement_group_id ID of the placement group will remove bundles.
  /// \param[in] bundle_indexes Bundle indexes will be removed.
  ...
  /// \param[in] task_options Options for this task.
  /// \param[out] return_ids Ids of the return objects.
  /// \return status of submission, ok or try again.
  Status SubmitActorTask(const ActorID &actor_id, const RayFunction &function,
                         const std::vector<std::unique_ptr<TaskArg>> &args,
                         const TaskOptions &task_options,
                         std::vector<ObjectID> *return_ids);

  /// Tell GCS to kill a job. It will retry 10 times if failed.
  bool KillJob(const JobID &job_id);
  ...
  /// TODO (kfstorm): This API works correctly only if num_workers == 1. It's a temporary
 ...
 public:
  ...
  std::pair<bool, Status> PutKV(const std::string &key, const std::string &value,
                                bool overwrite, bool isGlobal);

  std::pair<std::string, Status> GetKV(const std::string &key, bool isGlobal);

  std::pair<bool, Status> ExistsKV(const std::string &key, bool isGlobal);

  std::pair<bool, Status> DeleteKV(const std::string &key, bool isGlobal);
 ...
 private:
  Status InternalKillJob(const JobID &job_id);

  /// Create global owner, only be called on driver.
  void CreateHAObjectGlobalOwner();
  ...
  ///
  /// \param[in] scheduling_strategy The scheduling strategy.
  /// task.
  Status ValidatePlacementGroupBundleIndex(
      const rpc::SchedulingStrategy &scheduling_strategy);
  ...
  /// Run the request_service_ event loop.
  void RunRequestService();

  /// Run the reply_service_ event loop.
  void RunReplyService();

  std::string AddPrefixForKey(const std::string &key, bool isGlobal);

  Status ValidateLabels(const std::unordered_map<std::string, std::string> &labels);

  void InsertDefaultLabelKeys(std::unordered_map<std::string, std::string> &labels,
                              const ActorID &actor_id);

  Status ValidateSchedulingStrategy(const rpc::SchedulingStrategy &scheduling_strategy);
 ...
 private:
  ...
  bool has_shutdown_error_ = false;
  ...
  instrumented_io_context::work io_work_;

  /// The event loop where the RPC replies are handled.
  instrumented_io_context reply_service_;

  /// Keeps the reply_service_ alive.
  boost::asio::io_service::work reply_io_work_;

  // The threads that run reply_service_.
  std::vector<std::thread> reply_io_threads_;
  ...
  instrumented_io_context::work task_execution_service_work_;
  ...
  int64_t max_direct_call_returned_object_size_;
 ...
 private:
  ...
  std::shared_ptr<CoreWorkerStats::TaskStatistics> task_stats_;

  std::shared_ptr<CoreWorkerStats::RunningTaskStatistics> running_task_stats_;

  std::shared_ptr<CoreWorkerStats::MemoryStoreStatistics> memory_store_stats_;

  std::unique_ptr<rpc::JobConfig> job_config_;

  /// The event loop where the RPC requests are handled.
  instrumented_io_context request_service_;

  /// Keeps the request_service_ alive.
  boost::asio::io_service::work request_io_work_;

  // The thread that runs request_service_.
  std::thread request_io_thread_;

  // Because we cannot access `core_worker_server_` in class `CoreWorker`,
  // so, use `core_worker_port_` instead of `core_worker_server_->GetPort()` to
  // get port;
  int core_worker_port_;

  ray::stats::Percentile plasma_get_time_cost_;
  ray::stats::Percentile memory_get_time_cost_;
};
