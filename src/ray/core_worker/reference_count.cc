...
void ReferenceCounter::ReleaseAllLocalReferences() {
  absl::MutexLock lock(&mutex_);
  /// NOTE: A copy here to avoid iterator invalidation issue because we may erase
  /// the elements in `RemoveLocalReferenceInternal()` below.
...
std::vector<ObjectID> ReferenceCounter::ResetObjectsOnRemovedNode(
  ...
  for (auto it = object_id_refs_.begin(); it != object_id_refs_.end(); it++) {
    ...
    it->second.locations.erase(raylet_id);
  }
  return lost_objects;
}
