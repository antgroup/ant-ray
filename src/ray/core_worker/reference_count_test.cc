...
static const std::unordered_map<ObjectID, std::vector<ObjectID>> empty_nested_return_ids;
...
TEST(DistributedReferenceCountTest, TestNoBorrow) {
  ...
  // count into its own.
  owner->HandleSubmittedTaskFinished(outer_id, empty_nested_return_ids,
...
TEST(DistributedReferenceCountTest, TestSimpleBorrowerFailure) {
  ...
  // The owner receives the borrower's reply and merges the borrower's ref
  // count into its own.
  std::unordered_map<ObjectID, std::vector<ObjectID>> empty_nested_return_ids;
  owner->HandleSubmittedTaskFinished(outer_id, empty_nested_return_ids,
                                     borrower->address_, borrower_refs);
  borrower->FlushBorrowerCallbacks();
  // Check that owner now has borrower in inner's borrowers list.
...
TEST(DistributedReferenceCountTest, TestBorrowerTree) {
  ...
  // count into its own.
  owner->HandleSubmittedTaskFinished(outer_id, empty_nested_return_ids,
                                     borrower1->address_, borrower_refs);
  ...
  borrower1->HandleSubmittedTaskFinished(outer_id2, empty_nested_return_ids,
                                         borrower2->address_, borrower_refs);
...
TEST(DistributedReferenceCountTest, TestNestedObjectDifferentOwners) {
  ...
  // ID.
  borrower1->HandleSubmittedTaskFinished(borrower_id, empty_nested_return_ids,
                                         borrower2->address_, borrower_refs);
  ...
  // The owner receives the borrower's reply and merges the borrower's ref
  // count into its own.
  owner->HandleSubmittedTaskFinished(owner_id3, empty_nested_return_ids,
                                     borrower1->address_, borrower_refs);
...
TEST(DistributedReferenceCountTest, TestBorrowerPingPong) {
  ...
  borrower->HandleSubmittedTaskFinished(outer_id2, empty_nested_return_ids,
                                        owner->address_, borrower_refs);
  borrower->FlushBorrowerCallbacks();
...
TEST(DistributedReferenceCountTest, TestDuplicateBorrower) {
  ...
  // count into its own.
  owner->HandleSubmittedTaskFinished(outer_id, empty_nested_return_ids,
                                     borrower->address_, borrower_refs1);
  owner->HandleSubmittedTaskFinished(outer_id, empty_nested_return_ids,
                                     borrower->address_, borrower_refs2);
  borrower->FlushBorrowerCallbacks();
  // Check that owner now has borrower in inner's borrowers list.
  ASSERT_TRUE(owner->rc_.HasReference(inner_id));
  // Check that owner's ref count for outer == 0 since the borrower task
...
TEST(DistributedReferenceCountTest, TestDuplicateNestedObject) {
  ...
  owner->HandleSubmittedTaskFinished(owner_id2, empty_nested_return_ids,
                                     borrower1->address_, borrower_refs);
...
TEST(DistributedReferenceCountTest, TestReturnObjectIdBorrowChain) {
  ...
  caller->HandleSubmittedTaskFinished(return_id, empty_nested_return_ids,
                                      borrower->address_, borrower_refs);
