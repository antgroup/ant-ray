...
void SharedActorInfoAccessor::AsyncSubscribe(
  ...
  if (callbacks.empty()) {
    ...
    RAY_CHECK_OK(gcs_client_->Actors().AsyncSubscribe(actor_id, gcs_callback, done));
  } else {
    if (!callbacks.emplace(worker_id, actor_notification_callback).second) {
      return;
    }
