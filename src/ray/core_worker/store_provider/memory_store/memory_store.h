...
class CoreWorkerMemoryStore {
 public:
  ...
  CoreWorkerMemoryStore(
      ...
      std::shared_ptr<CoreWorkerStats::MemoryStoreStatistics> stats = nullptr,
 ...
 private:
  ...
  std::function<bool(const std::vector<ObjectID> &)> should_wait_objects_;

  std::function<void(const RayObject &)> unhandled_exception_handler_;

  std::shared_ptr<CoreWorkerStats::MemoryStoreStatistics> stats_;
