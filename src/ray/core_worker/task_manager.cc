...
Status TaskManager::ResubmitTask(const TaskID &task_id,
  ...
  bool resubmit = false;
  {
    ...
    if (!it->second.pending) {
      ...
      it->second.submission_time = current_time_ms();
...
bool TaskManager::PendingTaskFailed(const TaskID &task_id, rpc::ErrorType error_type,
  ...
  TaskSpecification spec;
  {
    ...
    // NOTE(kfstorm): ANT-INTERNAL. A task with no reply won't be added to task manager.
    // See `no_reply`.
    if (it == submissible_tasks_.end()) {
      return false;
    }
  ...
  if (!will_retry) {
    ...
    if (immediately_mark_object_fail) {
      MarkPendingTaskFailed(task_id, spec, error_type, ray_error_info);
    }
  }
