...
class TaskManager : public TaskFinisherInterface, public TaskResubmissionInterface {
 public:
  ...
  /// Warn for tasks pending for too long. It logs tasks which have run for
  /// a long time and haven't finished. It's helpful for debugging tasks with maybe lost
  /// replies.
  void WarnAboutHangingTasks() const;
 ...
 private:
  struct TaskEntry {
    TaskEntry(const TaskSpecification &spec_arg, int num_retries_left_arg,
      ...
      submission_time = current_time_ms();
    }
    ...
    // The first submission time of the task.
