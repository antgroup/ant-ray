 ...
 public:
  explicit ConcurrencyGroupManager(
    ...
    for (auto &group : concurrency_groups) {
      ...
      auto fiber = std::make_shared<ConcurrencyGroupExecutorType>(max_concurrency);
      auto &fds = group.function_descriptors;
      for (auto fd : fds) {
        functions_to_executor_index_[fd->ToString()] = fiber;
      }
      name_to_executor_index_[name] = fiber;
    }

    // If max concurrency of default group is 1, the tasks of default group
    // will be performed in the default concurrency group.
