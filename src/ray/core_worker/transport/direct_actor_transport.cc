// Copyright 2017 The Ray Authors.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//  http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

#include "ray/core_worker/transport/direct_actor_transport.h"

#include <thread>

#include "ray/common/task/task.h"
#include "ray/gcs/pb_util.h"

using ray::rpc::ActorTableData;
using namespace ray::gcs;

namespace ray {

void CoreWorkerDirectActorTaskSubmitter::AddActorQueueIfNotExists(
    const ActorID &actor_id) {
  absl::MutexLock lock(&mu_);
  // No need to check whether the insert was successful, since it is possible
  // for this worker to have multiple references to the same actor.
  client_queues_.emplace(actor_id, std::make_shared<ClientQueue>());
  client_queues_[actor_id]->max_pending_calls = max_pending_calls_;
}

void CoreWorkerDirectActorTaskSubmitter::KillActor(const ActorID &actor_id,
                                                   bool force_kill, bool no_restart) {
  absl::ReleasableMutexLock lock(&mu_);
  rpc::KillActorRequest request;
  request.set_intended_actor_id(actor_id.Binary());
  request.set_force_kill(force_kill);
  request.set_no_restart(no_restart);

  auto it = client_queues_.find(actor_id);
  // The language frontend can only kill actors that it has a reference to.
  RAY_CHECK(it != client_queues_.end());

  {
    absl::MutexLock lock(&(it->second->mu_));
    if (!it->second->pending_force_kill) {
      it->second->pending_force_kill = request;
    } else if (force_kill) {
      // Overwrite the previous request to kill the actor if the new request is a
      // force kill.
      it->second->pending_force_kill->set_force_kill(true);
      if (no_restart) {
        // Overwrite the previous request to disable restart if the new request's
        // no_restart flag is set to true.
        it->second->pending_force_kill->set_no_restart(true);
      }
    }
  }

  lock.Release();
  SendPendingTasks(actor_id, *(it->second));
}

Status CoreWorkerDirectActorTaskSubmitter::SubmitTask(ClientQueue &queue,
                                                      TaskSpecification task_spec) {
  auto task_id = task_spec.TaskId();
  auto actor_id = task_spec.ActorId();
  if (queue.state != rpc::ActorTableData::DEAD) {
    // We must release the lock before resolving the task dependencies since
    // the callback may get called in the same call stack.
    uint64_t send_pos = task_spec.ActorCounter();
    resolver_.ResolveDependencies(task_spec, [this, send_pos, actor_id]() {
      {
        std::shared_ptr<ClientQueue> queue;
        {
          absl::MutexLock lock(&mu_);
          auto it = client_queues_.find(actor_id);
          RAY_CHECK(it != client_queues_.end());
          queue = it->second;
        }
        absl::ReleasableMutexLock lock(&(queue->mu_));
        auto it = queue->requests.find(send_pos);
        // Only dispatch tasks if the submitted task is still queued. The task
        // may have been dequeued if the actor has since failed.
        if (it != queue->requests.end()) {
          it->second.second = true;
          lock.Release();
          // TODO: resolve dependencies callback may be called in different thread,
          // if the task has arguments by reference. At present no backpressure here.
          SendPendingTasks(actor_id, *queue);
        }
      }
    });
  } else {
    // Do not hold the lock while calling into task_finisher_.
    task_finisher_->MarkTaskCanceled(task_id);
    rpc::ErrorType error_type;
    rpc::RayErrorInfo error_info;
    {
      absl::MutexLock lock(&mu_);
      auto queue = client_queues_.find(task_spec.ActorId());
      const auto &death_cause = queue->second->death_cause;
      error_type = GenErrorTypeFromDeathCause(death_cause);
      error_info = GetErrorInfoFromActorDeathCause(death_cause);
    }
    auto status = Status::IOError("cancelling task of dead actor");
    // No need to increment the number of completed tasks since the actor is
    // dead.
    RAY_UNUSED(
        !task_finisher_->PendingTaskFailed(task_id, error_type, &status, &error_info));
  }

  // If the task submission subsequently fails, then the client will receive
  // the error in a callback.
  return Status::OK();
}

void CoreWorkerDirectActorTaskSubmitter::AppendTask(
    const TaskSpecification &task_spec,
    const std::function<void(const ActorID &)> &callback) {
  auto actor_id = task_spec.ActorId();
  RAY_LOG(DEBUG) << "AppendTask task: " << task_spec.DebugString();
  RAY_CHECK(task_spec.IsActorTask());

  uint64_t send_pos = 0;
  std::shared_ptr<ClientQueue> queue;
  {
    absl::MutexLock lock(&mu_);
    auto it = client_queues_.find(actor_id);
    RAY_CHECK(it != client_queues_.end());
    queue = it->second;
  }

  absl::ReleasableMutexLock lock(&(queue->mu_));
  // We must fix the send order prior to resolving dependencies, which may
  // complete out of order. This ensures that we will not deadlock due to
  // backpressure. The receiving actor will execute the tasks according to
  // this sequence number.
  send_pos = task_spec.ActorCounter();
  auto inserted = queue->requests.emplace(send_pos, std::make_pair(task_spec, false));
  RAY_CHECK(inserted.second);
  actor_task_io_service_.post(
      [this, queue, task_spec, callback, actor_id]() {
        RAY_UNUSED(SubmitTask(*queue, task_spec));
        if (callback) {
          callback(actor_id);
        }
      },
      "CoreWorker.SubmitActorTask");
}

void CoreWorkerDirectActorTaskSubmitter::DisconnectRpcClient(ClientQueue &queue) {
  if (queue.rpc_client) {
    // NOTE (kfstorm): This stops auto-reconnection inside the RPC client.
    queue.rpc_client->Close();
  }
  queue.rpc_client = nullptr;
  core_worker_client_pool_->Disconnect(ray::WorkerID::FromBinary(queue.worker_id));
  queue.worker_id.clear();
  queue.pending_force_kill.reset();
}

void CoreWorkerDirectActorTaskSubmitter::FailInflightTasks(
    const std::unordered_map<TaskID, rpc::ClientCallback<rpc::PushTaskReply>>
        &inflight_task_callbacks) {
  // NOTE(kfstorm): We invoke the callbacks with a bad status to act like there's a
  // network issue. We don't call `task_finisher_.PendingTaskFailed` directly because
  // there's much more work to do in the callback.
  auto status = Status::IOError("Fail all inflight tasks due to actor state change.");
  rpc::PushTaskReply reply;
  for (const auto &entry : inflight_task_callbacks) {
    entry.second(status, reply);
  }
}

void CoreWorkerDirectActorTaskSubmitter::ConnectActor(const ActorID &actor_id,
                                                      const rpc::Address &address,
                                                      int64_t num_restarts) {
  RAY_LOG(DEBUG) << "Connecting to actor " << actor_id << " at worker "
                 << WorkerID::FromBinary(address.worker_id());
  std::shared_ptr<ClientQueue> queue;
  {
    absl::MutexLock lock(&mu_);
    auto it = client_queues_.find(actor_id);
    RAY_CHECK(it != client_queues_.end());
    queue = it->second;
  }

  std::unordered_map<TaskID, rpc::ClientCallback<rpc::PushTaskReply>>
      inflight_task_callbacks;

  {
    absl::MutexLock lock(&(queue->mu_));
    RAY_LOG(INFO) << "Connecting to actor " << actor_id
                  << ", next_send_position = " << queue->next_send_position
                  << ", next_task_reply_position = " << queue->next_task_reply_position;
    if (num_restarts < queue->num_restarts) {
      // This message is about an old version of the actor and the actor has
      // already restarted since then. Skip the connection.
      RAY_LOG(INFO) << "Skip actor that has already been restarted, actor_id="
                    << actor_id;
      return;
    }

    if (queue->rpc_client &&
        queue->rpc_client->Addr().ip_address() == address.ip_address() &&
        queue->rpc_client->Addr().port() == address.port()) {
      RAY_LOG(DEBUG) << "Skip actor that has already been connected, actor_id="
                     << actor_id;
      return;
    }

    if (queue->state == rpc::ActorTableData::DEAD) {
      // This message is about an old version of the actor and the actor has
      // already died since then. Skip the connection.
      return;
    }

    queue->num_restarts = num_restarts;
    if (queue->rpc_client) {
      // Clear the client to the old version of the actor.
      DisconnectRpcClient(*queue);
      inflight_task_callbacks = std::move(queue->inflight_task_callbacks);
      queue->inflight_task_callbacks.clear();
    }

    queue->state = rpc::ActorTableData::ALIVE;
    // Update the mapping so new RPCs go out with the right intended worker id.
    queue->worker_id = address.worker_id();
    // Create a new connection to the actor.
    queue->rpc_client = core_worker_client_pool_->GetOrConnect(address);
    queue->rpc_client->SetOnConnectedCallback([this, actor_id]() {
      if (actor_reconnected_callback_) {
        RAY_LOG(INFO) << "Call actor reconnect callback " << actor_id;
        std::shared_ptr<ClientQueue> queue;
        {
          absl::MutexLock lock(&mu_);
          auto it = client_queues_.find(actor_id);
          if (it == client_queues_.end()) {
            RAY_LOG(WARNING) << "Can not find client queue for " << actor_id;
            return;
          }
          queue = it->second;
        }
        absl::ReleasableMutexLock lock(&(queue->mu_));
        if (queue->has_conn_error) {
          // Release the lock here in case the user wants to lock it again (e.g. by
          // submitting new tasks) in the callback.
          lock.Release();
          // The previous connection is broken and the actor is still alive without no
          // reconstruction. Invoke the reconnected callback.
          actor_reconnected_callback_(actor_id);
        } else {
          RAY_LOG(WARNING) << "There is no has_conn_error, ignore the callback";
        }
      } else {
        RAY_LOG(WARNING) << "No actor reconnect callback registered.";
      }
    });
    queue->rpc_client->SetOnDisconnectedCallback([this, actor_id]() {
      std::shared_ptr<ClientQueue> queue;
      {
        absl::MutexLock lock(&mu_);
        auto it = client_queues_.find(actor_id);
        if (it == client_queues_.end()) {
          return;
        }
        queue = it->second;
      }
      absl::ReleasableMutexLock lock(&(queue->mu_));
      if (queue->state == rpc::ActorTableData::ALIVE) {
        // Mark as connection error if the actor is alive.
        queue->has_conn_error = true;
        lock.Release();
      }
    });
    // This assumes that all replies from the previous incarnation
    // of the actor have been received. This assumption should be OK
    // because we fail all inflight tasks in `DisconnectRpcClient`.
    RAY_LOG(INFO) << "Resetting caller starts at for actor " << actor_id << " from "
                  << queue->caller_starts_at << " to " << queue->next_task_reply_position;
    queue->caller_starts_at = queue->next_task_reply_position;

    // ANT-INTERNAL
    queue->last_not_completed_has_reply_task_id = TaskID::Nil();
    queue->no_reply_task_count_after_a_has_reply_task.clear();
    ResendOutOfOrderTasks(actor_id, *queue);
  }

  SendPendingTasks(actor_id, *queue);

  // NOTE(kfstorm): We need to make sure the lock is released before invoking callbacks.
  FailInflightTasks(inflight_task_callbacks);
}

void CoreWorkerDirectActorTaskSubmitter::DisconnectActor(
    const ActorID &actor_id, int64_t num_restarts, bool dead,
    const rpc::ActorDeathCause &death_cause) {
  RAY_LOG(DEBUG) << "Disconnecting from actor " << actor_id;
  std::shared_ptr<ClientQueue> queue;
  {
    absl::MutexLock lock(&mu_);
    auto it = client_queues_.find(actor_id);
    RAY_CHECK(it != client_queues_.end());
    queue = it->second;
  }

  std::unordered_map<TaskID, rpc::ClientCallback<rpc::PushTaskReply>>
      inflight_task_callbacks;

  {
    absl::MutexLock lock(&(queue->mu_));
    RAY_LOG(INFO) << "Disconnecting from actor " << actor_id
                  << ", next_send_position = " << queue->next_send_position
                  << ", next_task_reply_position = " << queue->next_task_reply_position;
    if (!dead) {
      RAY_CHECK(num_restarts > 0);
    }
    if (num_restarts <= queue->num_restarts && !dead) {
      // This message is about an old version of the actor that has already been
      // restarted successfully. Skip the message handling.
      RAY_LOG(INFO)
          << "Skip actor disconnection that has already been restarted, actor_id="
          << actor_id;
      return;
    }

    // The actor failed, so erase the client for now. Either the actor is
    // permanently dead or the new client will be inserted once the actor is
    // restarted.
    DisconnectRpcClient(*queue);
    inflight_task_callbacks = std::move(queue->inflight_task_callbacks);
    queue->inflight_task_callbacks.clear();

    if (dead) {
      queue->state = rpc::ActorTableData::DEAD;
      queue->death_cause = death_cause;
      // If there are pending requests, treat the pending tasks as failed.
      RAY_LOG(INFO) << "Failing pending tasks for actor " << actor_id
                    << " because the actor is already dead.";

      auto &requests = queue->requests;
      auto head = requests.begin();

      auto status = Status::IOError("cancelling all pending tasks of dead actor");
      rpc::ErrorType error_type = GenErrorTypeFromDeathCause(death_cause);
      const auto error_info = GetErrorInfoFromActorDeathCause(death_cause);

      while (head != requests.end()) {
        const auto &task_spec = head->second.first;
        task_finisher_->MarkTaskCanceled(task_spec.TaskId());
        // No need to increment the number of completed tasks since the actor is
        // dead.
        RAY_UNUSED(!task_finisher_->PendingTaskFailed(task_spec.TaskId(), error_type,
                                                      &status, &error_info));
        head = requests.erase(head);
      }

      auto &wait_for_death_info_tasks = queue->wait_for_death_info_tasks;

      RAY_LOG(INFO) << "Failing tasks waiting for death info, size="
                    << wait_for_death_info_tasks.size() << ", actor_id=" << actor_id;
      for (auto &net_err_task : wait_for_death_info_tasks) {
        RAY_UNUSED(task_finisher_->MarkPendingTaskFailed(net_err_task.second->TaskId(),
                                                         *net_err_task.second, error_type,
                                                         &error_info));
      }

      // No need to clean up tasks that have been sent and are waiting for
      // replies. They will be treated as failed once the connection dies.
      // We retain the sequencing information so that we can properly fail
      // any tasks submitted after the actor death.
    } else if (queue->state != rpc::ActorTableData::DEAD) {
      // Only update the actor's state if it is not permanently dead. The actor
      // will eventually get restarted or marked as permanently dead.
      queue->state = rpc::ActorTableData::RESTARTING;
      queue->num_restarts = num_restarts;
    }

    // Reset `has_conn_error` because the previous disconnection is not a connection error
    // and the actor state has changed. By doing so, once the new RPC client is created,
    // the reconnected callback won't be invoked for this actor.
    queue->has_conn_error = false;
  }

  // NOTE(kfstorm): We need to make sure the lock is released before invoking callbacks.
  FailInflightTasks(inflight_task_callbacks);
}

void CoreWorkerDirectActorTaskSubmitter::CheckTimeoutTasks() {
  std::vector<std::shared_ptr<TaskSpecification>> task_specs;
  {
    absl::MutexLock lock(&mu_);
    for (auto &queue_pair : client_queues_) {
      auto &queue = *queue_pair.second;
      auto deque_itr = queue.wait_for_death_info_tasks.begin();
      while (deque_itr != queue.wait_for_death_info_tasks.end() &&
             /*timeout timestamp*/ deque_itr->first < current_time_ms()) {
        auto task_spec = deque_itr->second;
        task_specs.push_back(task_spec);
        deque_itr = queue.wait_for_death_info_tasks.erase(deque_itr);
      }
    }
  }

  // Do not hold mu_, because MarkPendingTaskFailed may call python from cpp,
  // and may cause deadlock with SubmitActorTask thread when aquire GIL.
  for (auto &task_spec : task_specs) {
    task_finisher_->MarkPendingTaskFailed(task_spec->TaskId(), *task_spec,
                                          rpc::ErrorType::ACTOR_DIED);
  }
}

void CoreWorkerDirectActorTaskSubmitter::SubscribeActorReconnected(
    const std::function<void(const ActorID &)> &callback) {
  actor_reconnected_callback_ = callback;
}

void CoreWorkerDirectActorTaskSubmitter::WaitForShutdown() {
  if (actor_task_io_thread_.joinable()) {
    actor_task_io_thread_.join();
  }
}

void CoreWorkerDirectActorTaskSubmitter::Shutdown() {
  actor_task_io_service_.stop();
  closed_ = true;
}

void CoreWorkerDirectActorTaskSubmitter::SendPendingTasks(const ActorID &actor_id,
                                                          ClientQueue &queue) {
  absl::ReleasableMutexLock lock(&(queue.mu_));
  if (!queue.rpc_client) {
    if (queue.state == rpc::ActorTableData::RESTARTING) {
      auto &requests = queue.requests;
      auto head = requests.begin();
      // All tasks sent to the same actor have the same EnableTaskFastFail
      // flag value. So we only need to check the flag of the first queued task.
      if (head != requests.end() && head->second.first.EnableTaskFastFail()) {
        // Fail all pending requests.
        while (head != requests.end() && head->first <= queue.next_send_position &&
               head->second.second) {
          auto task_spec =
              std::make_shared<TaskSpecification>(std::move(head->second.first));
          head = requests.erase(head);
          // Do not hold the lock while calling into task_finisher_.
          task_finisher_->MarkTaskCanceled(task_spec->TaskId());
          auto status = Status::IOError("The actor is temporarily unavailable.");
          // No need to increment the number of completed tasks since the actor is
          // dead.
          queue.next_send_position++;
          rpc::Address addr;
          rpc::PushTaskReply reply;

          bool increment_completed_tasks =
              TaskFinishErrorCallback(queue, task_spec, addr, status, reply);
          lock.Release();
          ReplyCallback(task_spec, increment_completed_tasks, status, reply);
        }
      }
    }
    return;
  }

  // Check if there is a pending force kill. If there is, send it and disconnect the
  // client.
  if (queue.pending_force_kill) {
    RAY_LOG(INFO) << "Sending KillActor request to actor " << actor_id;
    // It's okay if this fails because this means the worker is already dead.
    queue.rpc_client->KillActor(*queue.pending_force_kill, nullptr);
    queue.pending_force_kill.reset();
  }

  // Submit all pending requests.
  auto &requests = queue.requests;
  auto head = requests.begin();
  while (head != requests.end() && (/*seqno*/ head->first <= queue.next_send_position) &&
         (/*dependencies_resolved*/ head->second.second)) {
    // If the task has been sent before, skip the other tasks in the send
    // queue.
    bool skip_queue = head->first < queue.next_send_position;
    auto shared_task_spec = std::make_shared<TaskSpecification>(head->second.first);

    RAY_CHECK(!queue.worker_id.empty());
    PushActorTask(queue, shared_task_spec, skip_queue);

    head = requests.erase(head);
    queue.next_send_position++;
  }
}

void CoreWorkerDirectActorTaskSubmitter::ResendOutOfOrderTasks(const ActorID &actor_id,
                                                               ClientQueue &queue) {
  if (!queue.rpc_client) {
    return;
  }
  RAY_CHECK(!queue.worker_id.empty());

  RAY_LOG(INFO) << "Out of order completed tasks num: "
                << queue.out_of_order_completed_tasks.size() << " actor id: " << actor_id;
  for (const auto &completed_task : queue.out_of_order_completed_tasks) {
    // Making a copy here because we are flipping a flag and the original value is
    // const.
    auto task_spec = completed_task.second;
    RAY_LOG(INFO) << "Resend out of order task: actor_counter "
                  << task_spec->ActorCounter() << " next_send_position "
                  << queue.next_send_position << " caller_starts_at "
                  << queue.caller_starts_at << " next_task_reply_position "
                  << queue.next_task_reply_position << " actor id "
                  << task_spec->ActorId() << " task id " << task_spec->TaskId();
    task_spec->GetMutableMessage().set_skip_execution(true);
    /// skip_queue is true
    PushActorTask(queue, task_spec, /*skip_queue=*/true);
  }
  queue.out_of_order_completed_tasks.clear();
}

void CoreWorkerDirectActorTaskSubmitter::PushActorTask(
    ClientQueue &queue, std::shared_ptr<TaskSpecification> &task_spec, bool skip_queue) {
  auto request = std::make_unique<rpc::PushTaskRequest>();

  // Run actor creation task immediately on the main thread, without going
  // through a scheduling queue. But we still need to go through task interceptor.
  request->set_intended_worker_id(queue.worker_id);
  RAY_CHECK(task_spec->ActorCounter() >= queue.caller_starts_at)
      << "actor counter " << task_spec->ActorCounter() << " " << queue.caller_starts_at;
  request->set_sequence_number(task_spec->ActorCounter() - queue.caller_starts_at);

  const auto &task_id = task_spec->TaskId();
  const auto &actor_id = task_spec->ActorId();
  const auto actor_counter = task_spec->ActorCounter();
  RAY_LOG(DEBUG) << "Pushing task " << task_id << " to actor " << actor_id
                 << " actor counter " << actor_counter << " seq no "
                 << request->sequence_number() << " no_reply: " << task_spec->IsNoReply();
  const auto &addr = queue.rpc_client->Addr();
  rpc::ClientCallback<rpc::PushTaskReply> reply_callback = nullptr;

  // Note this check is internal only and is a performance optimization.
  if (!task_spec->IsNoReply()) {
    // ANT-INTERNAL
    queue.last_not_completed_has_reply_task_id = task_id;

    // Note that we only set the `reply_callback` when reply is required.
    // By passing nullptr, the RPC server won't send a reply.
    reply_callback = [this, task_spec, addr = std::move(addr)](
                         const Status &status, const rpc::PushTaskReply &reply) {
      bool increment_completed_tasks = TaskFinishCallback(task_spec, addr, status, reply);
      ReplyCallback(task_spec, increment_completed_tasks, status, reply);
    };
  } else {
    // ANT-INTERNAL: If the task has no reply, count it as completed when all previous
    // has-reply tasks has completed.
    if (!queue.last_not_completed_has_reply_task_id.IsNil()) {
      ++queue.no_reply_task_count_after_a_has_reply_task
            [queue.last_not_completed_has_reply_task_id];
    } else {
      queue.next_task_reply_position++;
    }
  }

  // NOTE(kfstorm): After the swap, the variable `task_spec` is invalid and cannot be used
  // anymore. To avoid accessing `task_spec` accidently in future code merges, in
  // ant-version Ray, we moved the copy/swap of the task spec to right before invoking the
  // RPC client.
  if (!task_spec->IsNoReply()) {
    // NOTE(swang): CopyFrom is needed because if we use Swap here and the task
    // fails, then the task data will be gone when the TaskManager attempts to
    // access the task.
    request->mutable_task_spec()->CopyFrom(task_spec->GetMessage());
  } else {
    // We don't add request to TaskManager if reply is not required, so it's safe to swap
    // here.
    request->mutable_task_spec()->Swap(&task_spec->GetMutableMessage());
  }

  rpc::ClientCallback<rpc::PushTaskReply> wrapped_callback;
  if (reply_callback) {
    queue.inflight_task_callbacks.emplace(task_id, std::move(reply_callback));
    wrapped_callback = [this, task_id, actor_id](const Status &status,
                                                 const rpc::PushTaskReply &reply) {
      std::shared_ptr<ClientQueue> queue;
      {
        absl::MutexLock lock(&mu_);
        auto it = client_queues_.find(actor_id);
        RAY_CHECK(it != client_queues_.end());
        queue = it->second;
      }
      rpc::ClientCallback<rpc::PushTaskReply> reply_callback;
      {
        absl::MutexLock lock(&queue->mu_);
        auto callback_it = queue->inflight_task_callbacks.find(task_id);
        if (callback_it == queue->inflight_task_callbacks.end()) {
          RAY_LOG(DEBUG) << "The task " << task_id
                         << " has already been marked as failed. Ingore the reply.";
          return;
        }
        reply_callback = std::move(callback_it->second);
        queue->inflight_task_callbacks.erase(callback_it);
      }
      reply_callback(status, reply);
    };
  }
  queue.rpc_client->PushActorTask(std::move(request), skip_queue,
                                  std::move(wrapped_callback));
}

bool CoreWorkerDirectActorTaskSubmitter::IsActorAlive(const ActorID &actor_id) const {
  std::shared_ptr<ClientQueue> queue;
  {
    absl::MutexLock lock(&mu_);
    auto it = client_queues_.find(actor_id);
    if (it == client_queues_.end()) return false;
    queue = it->second;
  }
  absl::MutexLock lock(&(queue->mu_));
  if (queue->rpc_client) {
    return true;
  }
  return false;
}

bool CoreWorkerDirectActorTaskSubmitter::TaskFinishCallback(
    std::shared_ptr<TaskSpecification> task_spec, const rpc::Address &addr, Status status,
    const rpc::PushTaskReply &reply) {
  const auto &task_id = task_spec->TaskId();
  const auto task_skipped = task_spec->GetMessage().skip_execution();
  if (task_skipped) {
    // NOTE(simon):Increment the task counter regardless of the status because the
    // reply for a previously completed task. We are not calling CompletePendingTask
    // because the tasks are pushed directly to the actor, not placed on any queues
    // in task_finisher_.
  } else if (status.ok()) {
    task_finisher_->CompletePendingTask(task_id, reply, addr);
  } else {
    const auto &actor_id = task_spec->ActorId();
    absl::ReleasableMutexLock lock(&mu_);
    auto queue_pair = client_queues_.find(actor_id);
    RAY_CHECK(queue_pair != client_queues_.end());
    auto queue = queue_pair->second;
    lock.Release();
    absl::MutexLock queue_lock(&queue->mu_);
    return TaskFinishErrorCallback(*queue, task_spec, addr, status, reply);
  }
  return true;
}

bool CoreWorkerDirectActorTaskSubmitter::TaskFinishErrorCallback(
    ClientQueue &queue, std::shared_ptr<TaskSpecification> task_spec,
    const rpc::Address &addr, Status status, const rpc::PushTaskReply &reply) {
  const auto &task_id = task_spec->TaskId();

  bool increment_completed_tasks = true;

  // push task failed due to network error. For example, actor is dead
  // and no process response for the push task.

  /// If fastfail feature enabled on this task, we should mark this task failed
  /// immediately.
  bool immediately_mark_object_fail =
      (queue.state == rpc::ActorTableData::DEAD || task_spec->EnableTaskFastFail());

  rpc::ErrorType error_type = GenErrorTypeFromDeathCause(queue.death_cause);
  const auto error_info = GetErrorInfoFromActorDeathCause(queue.death_cause);
  bool will_retry = task_finisher_->PendingTaskFailed(
      task_id, error_type, &status, &error_info, immediately_mark_object_fail);
  if (will_retry) {
    increment_completed_tasks = false;
  } else if (!immediately_mark_object_fail) {
    // put it to wait_for_death_info_tasks and wait for Death info
    int64_t death_info_timeout_ts =
        current_time_ms() + RayConfig::instance().timeout_ms_task_wait_for_death_info();
    queue.wait_for_death_info_tasks.emplace_back(death_info_timeout_ts, task_spec);
    RAY_LOG(INFO) << "PushActorTask failed because of network error, this task "
                     "will be stashed away and waiting for Death info from GCS, task_id="
                  << task_id
                  << ", wait queue size=" << queue.wait_for_death_info_tasks.size()
                  << ", status: " << status;
  }
  return increment_completed_tasks;
}

void CoreWorkerDirectActorTaskSubmitter::RunActorTaskIOService() {
  SetThreadName("actor_task");
  BlockSignal();
  actor_task_io_service_.run();
}

void CoreWorkerDirectActorTaskSubmitter::ReplyCallback(
    std::shared_ptr<TaskSpecification> task_spec, bool increment_completed_tasks,
    Status status, const rpc::PushTaskReply &reply) {
  const auto &task_id = task_spec->TaskId();
  const auto &actor_id = task_spec->ActorId();
  const auto actor_counter = task_spec->ActorCounter();

  std::shared_ptr<ClientQueue> queue_ptr = nullptr;
  {
    absl::MutexLock lock(&mu_);
    auto it = client_queues_.find(actor_id);
    RAY_CHECK(it != client_queues_.end());
    queue_ptr = it->second;
  }

  absl::MutexLock lock(&(queue_ptr->mu_));
  if (increment_completed_tasks) {
    auto &queue = *queue_ptr;

    // ANT-INTERNAL
    if (queue.last_not_completed_has_reply_task_id == task_id) {
      queue.last_not_completed_has_reply_task_id = TaskID::Nil();
    }

    // Try to increment queue.next_task_reply_position consecutively until we
    // cannot. In the case of tasks not received in order, the following block
    // ensure queue.next_task_reply_position are incremented to the max possible
    // value.
    queue.out_of_order_completed_tasks.insert({actor_counter, task_spec});
    auto min_completed_task = queue.out_of_order_completed_tasks.begin();
    while (min_completed_task != queue.out_of_order_completed_tasks.end()) {
      if (min_completed_task->first == queue.next_task_reply_position) {
        auto min_completed_task_id = min_completed_task->second->TaskId();
        queue.next_task_reply_position++;
        // increment the iterator and erase the old value
        queue.out_of_order_completed_tasks.erase(min_completed_task++);

        // ANT-INTERNAL: Once a has-reply task has completed, increase
        // `next_task_reply_position` with subsequent no-reply task count.
        auto it =
            queue.no_reply_task_count_after_a_has_reply_task.find(min_completed_task_id);
        if (it != queue.no_reply_task_count_after_a_has_reply_task.end()) {
          queue.next_task_reply_position += it->second;
          queue.no_reply_task_count_after_a_has_reply_task.erase(it);
          RAY_LOG(INFO) << "Increase next_task_reply_position with subsequent "
                           "no-reply task count "
                        << queue.next_task_reply_position << " actor id " << actor_id;
        }
      } else {
        break;
      }
    }

    RAY_LOG(DEBUG) << "Got PushTaskReply for actor " << actor_id << " with actor_counter "
                   << actor_counter << " new queue.next_task_reply_position is "
                   << queue.next_task_reply_position
                   << " and size of out_of_order_tasks set is "
                   << queue.out_of_order_completed_tasks.size() << " status: " << status;
  }

  // Dec current_pending_calls whether increment_completed_tasks is true
  // or not. If increment_completed_tasks is false, means the task is retried.
  if (queue_ptr->current_pending_calls > 0) {
    queue_ptr->current_pending_calls--;
  }
}

bool CoreWorkerDirectActorTaskSubmitter::IncPendingTaskNumsIfNotFull(
    const ActorID &actor_id) {
  std::shared_ptr<ClientQueue> queue;
  {
    absl::MutexLock lock(&mu_);
    auto it = client_queues_.find(actor_id);
    RAY_CHECK(it != client_queues_.end());
    queue = it->second;
  }
  absl::MutexLock lock(&(queue->mu_));
  if (queue->current_pending_calls >= queue->max_pending_calls) {
    return true;
  } else {
    queue->current_pending_calls++;
    return false;
  }
}

void CoreWorkerDirectTaskReceiver::Init(
    std::shared_ptr<rpc::CoreWorkerClientPool> client_pool, rpc::Address rpc_address,
    std::shared_ptr<DependencyWaiter> dependency_waiter,
    boost::asio::io_service &task_execution_service) {
  waiter_ = std::move(dependency_waiter);
  rpc_address_ = rpc_address;
  client_pool_ = client_pool;
}

void CoreWorkerDirectTaskReceiver::HandleTask(
    const rpc::PushTaskRequest &request, rpc::PushTaskReply *reply,
    rpc::SendReplyCallback send_reply_callback) {
  RAY_CHECK(waiter_ != nullptr) << "Must call init() prior to use";
  // Use `mutable_task_spec()` here as `task_spec()` returns a const reference
  // which doesn't work with std::move.
  TaskSpecification task_spec(
      std::move(*(const_cast<rpc::PushTaskRequest &>(request).mutable_task_spec())));

  // If GCS server is restarted after sending an actor creation task to this core worker,
  // the restarted GCS server will send the same actor creation task to the core worker
  // again. We just need to ignore it and reply ok.
  if (task_spec.IsActorCreationTask() &&
      worker_context_.GetCurrentActorID() == task_spec.ActorCreationId()) {
    send_reply_callback(Status::OK(), nullptr, nullptr);
    RAY_LOG(INFO) << "Ignoring duplicate actor creation task for actor "
                  << task_spec.ActorCreationId()
                  << ". This is likely due to a GCS server restart.";
    return;
  }

  if (task_spec.IsActorCreationTask()) {
    RAY_LOG(INFO) << "Actor " << task_spec.ActorCreationId() << " concurrency enabled.";
    SetMaxActorConcurrency(task_spec.IsAsyncioActor(), task_spec.MaxActorConcurrency());
  }

  // Only assign resources for non-actor tasks. Actor tasks inherit the resources
  // assigned at initial actor creation time.
  std::shared_ptr<ResourceMappingType> resource_ids;
  if (!task_spec.IsActorTask()) {
    resource_ids.reset(new ResourceMappingType());
    for (const auto &mapping : request.resource_mapping()) {
      std::vector<std::pair<int64_t, double>> rids;
      for (const auto &ids : mapping.resource_ids()) {
        rids.push_back(std::make_pair(ids.index(), ids.quantity()));
      }
      (*resource_ids)[mapping.name()] = rids;
    }
  }

  auto accept_callback = [this, reply, task_spec,
                          resource_ids](rpc::SendReplyCallback send_reply_callback) {
    if (task_spec.GetMessage().skip_execution()) {
      send_reply_callback(Status::OK(), nullptr, nullptr);
      return;
    }

    auto num_returns = task_spec.NumReturns();
    if (task_spec.IsActorCreationTask() || task_spec.IsActorTask()) {
      // Decrease to account for the dummy object id.
      num_returns--;
    }
    RAY_CHECK(num_returns >= 0);

    std::vector<std::shared_ptr<RayObject>> return_objects;
    bool is_application_level_error = false;
    auto status =
        task_handler_(task_spec, resource_ids, &return_objects,
                      reply->mutable_borrowed_refs(), &is_application_level_error);
    reply->set_is_application_level_error(is_application_level_error);

    bool objects_valid = return_objects.size() == num_returns;
    if (objects_valid) {
      for (size_t i = 0; i < return_objects.size(); i++) {
        auto return_object = reply->add_return_objects();
        ObjectID id = ObjectID::FromIndex(task_spec.TaskId(), /*index=*/i + 1);
        return_object->set_object_id(id.Binary());

        // The object is nullptr if it already existed in the object store.
        const auto &result = return_objects[i];
        return_object->set_size(result->GetSize());
        if (result->GetData() != nullptr && result->GetData()->IsPlasmaBuffer()) {
          return_object->set_in_plasma(true);
        } else {
          if (result->GetData() != nullptr) {
            return_object->set_data(result->GetData()->Data(), result->GetData()->Size());
          }
          if (result->GetMetadata() != nullptr) {
            return_object->set_metadata(result->GetMetadata()->Data(),
                                        result->GetMetadata()->Size());
          }
          for (const auto &nested_id : result->GetNestedIds()) {
            return_object->add_nested_inlined_ids(nested_id.Binary());
          }
        }
      }

      if (task_spec.IsActorCreationTask()) {
        auto concurrent_groups = task_spec.ConcurrentGroups();
        /// The default max concurrency for creating PoolManager should
        /// be 0 if this is an asyncio actor.
        const int default_max_concurrency =
            task_spec.IsAsyncioActor() ? 0 : task_spec.MaxActorConcurrency();
        pool_manager_ =
            std::make_shared<PoolManager>(concurrent_groups, default_max_concurrency);
        RAY_LOG(INFO) << "Actor creation task finished, task_id: " << task_spec.TaskId()
                      << ", actor_id: " << task_spec.ActorCreationId();
        // Tell raylet that an actor creation task has finished execution, so that
        // raylet can publish actor creation event to GCS, and mark this worker as
        // actor, thus if this worker dies later raylet will restart the actor.
        RAY_CHECK_OK(task_done_());
      }
    }
    if (status.ShouldExitWorker()) {
      // Don't allow the worker to be reused, even though the reply status is OK.
      // The worker will be shutting down shortly.
      reply->set_worker_exiting(true);
      if (objects_valid) {
        // This happens when max_calls is hit. We still need to return the objects.
        send_reply_callback(Status::OK(), nullptr, nullptr);
      } else {
        send_reply_callback(status, nullptr, nullptr);
      }
    } else {
      RAY_CHECK(objects_valid) << return_objects.size() << "  " << num_returns;
      send_reply_callback(status, nullptr, nullptr);
    }
  };

  auto reject_callback = [](rpc::SendReplyCallback send_reply_callback) {
    send_reply_callback(Status::Invalid("client cancelled stale rpc"), nullptr, nullptr);
  };

  auto dependencies = task_spec.GetDependencies(false);

  if (task_spec.IsActorTask()) {
    auto it = actor_scheduling_queues_.find(task_spec.CallerWorkerId());
    if (it == actor_scheduling_queues_.end()) {
      auto result = actor_scheduling_queues_.emplace(
          task_spec.CallerWorkerId(),
          std::unique_ptr<SchedulingQueue>(new ActorSchedulingQueue(
              task_main_io_service_, *waiter_, pool_manager_, is_asyncio_,
              max_concurrency_, RayConfig::instance().task_reorder_wait_seconds())));
      it = result.first;
    }
    it->second->Add(request.sequence_number(), request.client_processed_up_to(),
                    std::move(accept_callback), std::move(reject_callback),
                    task_spec.FunctionDescriptor(), std::move(send_reply_callback),
                    task_spec.TaskId(), dependencies);
  } else {
    // Add the normal task's callbacks to the non-actor scheduling queue.
    normal_scheduling_queue_->Add(
        request.sequence_number(), request.client_processed_up_to(),
        std::move(accept_callback), std::move(reject_callback),
        task_spec.FunctionDescriptor(), std::move(send_reply_callback),
        task_spec.TaskId(), dependencies);
  }
}

void CoreWorkerDirectTaskReceiver::RunNormalTasksFromQueue() {
  // If the scheduling queue is empty, return.
  if (normal_scheduling_queue_->TaskQueueEmpty()) {
    return;
  }

  // Execute as many tasks as there are in the queue, in sequential order.
  normal_scheduling_queue_->ScheduleRequests();
}

bool CoreWorkerDirectTaskReceiver::CancelQueuedNormalTask(TaskID task_id) {
  // Look up the task to be canceled in the queue of normal tasks. If it is found and
  // removed successfully, return true.
  return normal_scheduling_queue_->CancelTaskIfFound(task_id);
}

void CoreWorkerDirectTaskReceiver::SetMaxActorConcurrency(bool is_asyncio,
                                                          int max_concurrency) {
  RAY_CHECK(max_concurrency_ == 0)
      << "SetMaxActorConcurrency should only be called at most once.";
  max_concurrency_ = max_concurrency;
  is_asyncio_ = is_asyncio;
}

}  // namespace ray
