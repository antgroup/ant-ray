  ...
  resolver_.ResolveDependencies(task_spec, [this, task_spec]() {
    ...
    if (task_spec.IsActorCreationTask()) {
      ...
      RAY_CHECK_OK(actor_creator_->AsyncCreateActor(
          task_spec, [this, actor_id, task_id,
                      weak_task_finisher =
                          std::weak_ptr<ray::TaskFinisherInterface>(task_finisher_)](
                         Status status, const rpc::CreateActorReply &reply) {
            // Core worker might have been destructed when this callback is triggered.
            // So skip the following if this happens.
            auto shared_task_finisher = weak_task_finisher.lock();
            if (!shared_task_finisher) {
              return;
            }
            if (status.ok()) {
              ...
              while (!task_queue.empty()) {
                ...
                if (reply.failure_type() ==
                  ...
                  *(error_info.mutable_error_message()) =
                      absl::StrCat("Could not submit the task ", task_spec.TaskId().Hex(),
                                   ", reason: ", reply.scheduling_failure_message());
                  RAY_UNUSED(task_finisher_->PendingTaskFailed(
                      task_spec.TaskId(), rpc::ErrorType::TASK_UNSCHEDULABLE_ERROR,
...
void CoreWorkerDirectTaskSubmitter::PushNormalTask(
  ...
  bool retry_exceptions = task_spec.GetMessage().retry_exceptions();
  ...
  client.PushNormalTask(
      std::move(request),
      [this, task_id, is_actor, is_actor_creation, retry_exceptions, scheduling_key, addr,
       assigned_resources](Status status, const rpc::PushTaskReply &reply) {
        {
          ...
          // Decrement the total number of tasks in flight to any worker with the
          // current scheduling_key.
          auto &scheduling_key_entry = scheduling_key_entries_[scheduling_key];
          RAY_CHECK(scheduling_key_entry.active_workers.size() >= 1);
        ...
        if (!status.ok()) {
          ...
          error_msg << "The task " << task_id
                    << " died unexpectedly while executing in node " << addr.ip_address
                    << ", worker id:" << addr.worker_id
                    << ", status: " << status.ToString();
          *(error_info.mutable_error_message()) = error_msg.str();

          RAY_UNUSED(task_finisher_->PendingTaskFailed(
              task_id,
              is_actor ? rpc::ErrorType::ACTOR_DIED : rpc::ErrorType::WORKER_DIED,
              &status, is_actor ? nullptr : &error_info));
        } else {
          if (!retry_exceptions || !reply.is_application_level_error() ||
