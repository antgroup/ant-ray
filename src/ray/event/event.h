...
using EventPostProcessor = std::function<void(rpc::Event &event)>;
...
class RayEventContext final {
 public:
  ...
  void SetEventPostProcessor(const EventPostProcessor callback);
  ...
  inline const std::vector<EventPostProcessor> &GetEventPostProcessors() const {
    return event_post_processors_;
  }

  inline bool GetInitialzed() const { return initialized_; }

  // ANT-INTERNAL
  void SetLabelBlacklist(std::string blacklist);

  // ANT-INTERNAL
  const std::unordered_set<std::string> &GetLabelBlacklist() const {
    return label_blacklist_;
  };
 ...
 private:
  ...
  std::vector<EventPostProcessor> event_post_processors_;
  ...
  // ANT-INTERNAL
  std::unordered_set<std::string> label_blacklist_;
};
