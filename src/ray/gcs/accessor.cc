...
Status InternalKVAccessor::Del(const std::string &key, bool &deleted) {
  std::promise<Status> ret_promise;
  RAY_CHECK_OK(AsyncInternalKVDel(
      key, [&ret_promise, &deleted](Status status, boost::optional<int> deleted_num) {
        deleted = static_cast<bool>(deleted_num.value_or(0));
        ret_promise.set_value(status);
      }));
  return ret_promise.get_future().get();
}
