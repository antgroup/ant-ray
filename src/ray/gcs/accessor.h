...
class JobInfoAccessor {
 public:
  ...
  /// Put job result to GCS asynchronously.
  ///
  /// \param job_id The job ID.
  /// \param key The job data key.
  /// \param data The job data.
  /// \param callback Callback that will be called after result has been put
  /// to GCS.
  /// \return Status
  virtual Status AsyncPutData(const JobID &job_id, const std::string &key,
  ...
  /// \param job_id The job ID.
  /// \param key The job data key.
  /// \param callback Callback that will be called after result has been got
  /// from GCS.
  /// \return Status
  virtual Status AsyncGetData(const JobID &job_id, const std::string &key,
  ...
  /// Notify GCS that the driver has exited.
  ///
  /// \param job_id The job ID of the driver.
  /// \param exit_with_error Whether the driver exited with an error.
  /// \param callback Callback that will be called after notification finished.
  /// \return Status
  virtual Status AsyncNotifyDriverExit(const JobID &job_id, bool exit_with_error,
                                       const StatusCallback &callback) = 0;
  ...
  /// Get job info from GCS asynchronously.
  ///
  /// \param job_id ID of the job that will be got from GCS.
  /// \param callback Callback that will be called when received reply.
  /// \return Status
  virtual Status AsyncGet(const JobID &job_id,
                          const OptionalItemCallback<rpc::JobTableData> &callback) = 0;
  ...
  /// Subscribe to job update notification.
  ///
  /// \param ID of the job.
  /// \param min_resource_requirements Minimum resources of the job declared.
  /// \param max_resource_requirements Maximum resources of the job declared.
  /// \param done Callback that will be called when update successfully.
  /// \return Status
  virtual Status AsyncUpdateJobResourceRequirements(
      const JobID &job_id,
  ...
  virtual Status AsyncKill(const JobID &job_id, const StatusCallback &callback,
                           bool mark_as_failed = false) = 0;
...
class TaskInfoAccessor {
 public:
  ...
  /// Add a task to CS asynchronously.
  ///
...
class NodeResourceInfoAccessor {
 public:
  ...
  /// Get node's resource usage from GCS asynchronously.
  ///
  /// \param node_id The ID of node to lookup resource usage.
  /// \param callback Callback that will be called after lookup finishes.
  /// \return Status
  virtual Status AsyncGetResourceUsage(
      const NodeID &node_id,
      const OptionalItemCallback<rpc::ResourcesData> &callback) = 0;
  ...
  /// Get cluster scheduling resources from GCS asynchronously.
  ///
  /// \param callback Callback that will be called after lookup finishes.
  /// \return Status
  virtual Status AsyncGetClusterResources(
...
class PlacementGroupInfoAccessor {
 public:
  ...
  /// Remove bundles from the placement group asynchronously.
  ///
  /// \param placement_group_id Id of the placement group to remove bundles.
  /// \param bundle_indexes The bundles will be removed.
  virtual Status AsyncRemovePlacementGroupBundles(
      const ray::PlacementGroupID &placement_group_id,
      const std::vector<int> &bundle_indexes, const StatusCallback &callback) = 0;
  ...
  /// Subscribe to all placement group bundles changed notification.
  ///
  /// \param subscribe Callback that will be called each time when receives a bundles
  /// update notification.
  /// \param done Callback that will be called when subscription is
  /// complete.
  /// \return Status.
  virtual Status AsyncSubscribeAllBundlesChangedEvent(
      const SubscribeCallback<PlacementGroupID,
                              rpc::PlacementGroupBundlesChangedNotification> &subscribe,
      const StatusCallback &done) = 0;
...
class NodegroupInfoAccessor {
 public:
  virtual ~NodegroupInfoAccessor() = default;

  /// Get all nodegroup data from GCS asynchronously.
  ///
  /// \param callback Callback that will be called when received reply.
  /// \return Status
  virtual Status AsyncGetAll(const MultiItemCallback<rpc::NodegroupData> &callback) = 0;

  /// Subscribe to any add operations of nodegroups.
  ///
  /// \param subscribe Callback that will be called each time when nodegroup info is
  /// changed.
  /// \param done Callback that will be called when subscription is complete and
  /// we are ready to receive notification.
  /// \return Status
  virtual Status AsyncSubscribeAll(
...
class FrozenNodeAccessor {
 public:
  virtual ~FrozenNodeAccessor() = default;

  /// Subscribe to any add operations of frozen nodes.
  ///
  /// \param subscribe Callback that will be called each time when frozen nodes is
  /// updated, the frozen node set will be passed to this callback.
  /// \param done Callback that will be called when subscription is complete and
  /// we are ready to receive notification.
  /// \return Status
  virtual Status AsyncSubscribeFrozenNode(
      const ItemCallback<std::unordered_set<std::string>> &subscribe,
      const StatusCallback &done) = 0;

  /// Get all frozen node data from GCS asynchronously.
  ///
  /// \param callback Callback that will be called when received reply.
  /// \return Status
  virtual Status AsyncGetAll(
      const ItemCallback<std::unordered_set<std::string>> &callback) = 0;

  virtual Status ReportFreezeDone(const std::string &node_name, const NodeID &node_id,
                                  const StatusCallback &done) = 0;
...
class ActorMigrationAccessor {
 public:
  virtual ~ActorMigrationAccessor() = default;

  /// Subscribe to actor migration events.
  ///
  /// \param subscribe Callback of the events.
  /// \param done Callback when subscription is established.
  /// \return Status
  virtual Status AsyncSubscribeActorMigrationEvents(
...
class InternalKVAccessor {
 public:
  ...
  ///
  /// \param key The key to delete
  /// \param deleted It's an output parameter. It'll be set to be true if
  ///     any row is deleted.
  /// \return Status
  Status Del(const std::string &key, bool &deleted);
