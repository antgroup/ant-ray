...
class GcsClient : public std::enable_shared_from_this<GcsClient> {
 public:
  ...
  virtual std::pair<std::string, int> GetApiServerAddress() {
    return std::make_pair("", 0);
  }
  ...
  /// Get the sub-interface for accessing node information in GCS.
  /// This function is thread safe.
  FullNodeInfoAccessor &FullNodeInfo() {
    RAY_CHECK(full_node_info_accessor_ != nullptr);
    return *full_node_info_accessor_;
  }
  ...
  // Get the sub-interface for accessing nodegroup infomation in GCS.
  // This function is thread safe.
  NodegroupInfoAccessor &Nodegroups() {
    RAY_CHECK(nodegroup_accessor_ != nullptr);
    return *nodegroup_accessor_;
  }

  // Get the sub-interface for accessing frozen node infomation in GCS.
  // This function is thread safe.
  FrozenNodeAccessor &FrozenNodes() {
    RAY_CHECK(frozen_node_accessor_ != nullptr);
    return *frozen_node_accessor_;
  }

  // Get the sub-interface for accessing frozen node infomation in GCS.
  // This function is thread safe.
  ActorMigrationAccessor &ActorMigration() {
    RAY_CHECK(actor_migration_accessor_ != nullptr);
    return *actor_migration_accessor_;
  }
 ...
 protected:
  ...
  std::unique_ptr<BasicNodeInfoAccessor> node_accessor_;
  std::unique_ptr<FullNodeInfoAccessor> full_node_info_accessor_;
  ...
  std::unique_ptr<NodegroupInfoAccessor> nodegroup_accessor_;
  std::unique_ptr<FrozenNodeAccessor> frozen_node_accessor_;
  std::unique_ptr<ActorMigrationAccessor> actor_migration_accessor_;
};
