...
class GlobalStateAccessor {
 public:
  ...
  ///
  /// \return The ip and port of the api server.
  ...
  /// \return A list of `GcsNodeInfo` objects serialized in protobuf format.
  std::vector<std::string> GetAllNodeInfoByNodegroup(const std::string &nodegroup_id);
  ...
  /// Get resource usage of a node from GCS Service.
  ///
  /// \param node_id The ID of node to look up in the GCS Service.
  ...
  /// deserialized with protobuf function.
  std::unique_ptr<std::string> GetResourceUsage(const NodeID &node_id);
  ...
  /// Get latest cluster scheduling resources from GCS Service. Only used when
  /// gcs scheduler enabled.
  ///
  /// \return node resources info. To support multi-language, we serialize each
  /// data and return the serialized string. Where used, it needs to be
  /// deserialized with protobuf function.
  std::vector<std::string> GetClusterResources();
  ...
  /// \param min_resource_requirements The minimum resources that the current job can use.
  /// \param max_resource_requirements The maximum resources that the current job can use.
  bool UpdateJobResourceRequirements(
      const JobID &job_id,
  ...
  std::pair<bool, std::string> PutJobData(const JobID &job_id, const std::string &key,
                                          const std::string &data);
  std::string GetJobData(const JobID &job_id, const std::string &key);
 ...
 private:
  ...
  std::shared_ptr<ServiceBasedGcsClient> gcs_client_;
