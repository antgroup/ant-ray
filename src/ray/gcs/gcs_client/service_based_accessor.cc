...
Status ServiceBasedJobInfoAccessor::AsyncPutData(const JobID &job_id,
                                                 const std::string &key,
                                                 const std::string &data,
                                                 const StatusCallback &callback) {
  RAY_LOG(DEBUG) << "Putting job data, job id = " << job_id << ", key = " << key
                 << ", data = " << data;
  rpc::PutJobDataRequest request;
  request.set_job_id(job_id.Binary());
  request.set_key(key);
  request.set_data(data);
  client_impl_->GetGcsRpcClient().PutJobData(
      request,
      [job_id, data, callback](const Status &status, const rpc::PutJobDataReply &reply) {
        if (callback) {
          callback(status);
        }
        RAY_LOG(DEBUG) << "Finished putting job data, status = " << status
                       << ", job id = " << job_id << ", data = " << data;
      });
  return Status::OK();
}

Status ServiceBasedJobInfoAccessor::AsyncGetData(
    const JobID &job_id, const std::string &key,
    const OptionalItemCallback<std::string> &callback) {
  RAY_LOG(DEBUG) << "Getting job data, job id = " << job_id << ", key = " << key;
  rpc::GetJobDataRequest request;
  request.set_job_id(job_id.Binary());
  request.set_key(key);
  client_impl_->GetGcsRpcClient().GetJobData(
      request,
      [job_id, callback](const Status &status, const rpc::GetJobDataReply &reply) {
        if (callback) {
          callback(status, reply.data());
        }
        RAY_LOG(DEBUG) << "Finished getting job data, status = " << status
                       << ", job id = " << job_id << ", data = " << reply.data();
      });
  return Status::OK();
}
...
Status ServiceBasedJobInfoAccessor::AsyncNotifyDriverExit(
    const JobID &job_id, bool exit_with_error, const StatusCallback &callback) {
  RAY_LOG(DEBUG) << "Notify driver exit. job id = " << job_id
                 << ", with error = " << exit_with_error;
  rpc::NotifyDriverExitRequest request;
  request.set_job_id(job_id.Binary());
  request.set_exit_with_error(exit_with_error);
  client_impl_->GetGcsRpcClient().NotifyDriverExit(
      request, [job_id, exit_with_error, callback](
                   const Status &status, const rpc::NotifyDriverExitReply &reply) {
        if (callback) {
          callback(status);
        }
        RAY_LOG(DEBUG) << "Finished Notifying driver exit. status = " << status
                       << ", job id = " << job_id << ", with error = " << exit_with_error;
      });
  return Status::OK();
}
...
Status ServiceBasedJobInfoAccessor::AsyncGet(
    const JobID &job_id, const OptionalItemCallback<JobTableData> &callback) {
  RAY_LOG(DEBUG) << "Getting job info, job id = " << job_id;
  RAY_CHECK(callback != nullptr);
  rpc::GetJobRequest request;
  request.set_job_id(job_id.Binary());
  client_impl_->GetGcsRpcClient().GetJob(
      request, [job_id, callback](const Status &status, const rpc::GetJobReply &reply) {
        if (reply.has_data()) {
          callback(status, reply.data());
        } else {
          callback(status, boost::none);
        }
        RAY_LOG(DEBUG) << "Finished getting job info, status = " << status
                       << ", job id = " << job_id;
      });
  return Status::OK();
}
...
Status ServiceBasedJobInfoAccessor::AsyncSubscribeAll(
  ...
  RAY_LOG(DEBUG) << "Subscribing all job.";
...
Status ServiceBasedJobInfoAccessor::AsyncUpdateJobResourceRequirements(
    const JobID &job_id,
    const std::unordered_map<std::string, double> &min_resource_requirements,
    const std::unordered_map<std::string, double> &max_resource_requirements,
    const StatusCallback &done) {
  RAY_LOG(INFO) << "Updating job resource requirements, job id = " << job_id;
  rpc::UpdateJobResourceRequirementsRequest request;
  request.set_job_id(job_id.Binary());
  request.mutable_min_resource_requirements()->insert(min_resource_requirements.begin(),
                                                      min_resource_requirements.end());
  request.mutable_max_resource_requirements()->insert(max_resource_requirements.begin(),
                                                      max_resource_requirements.end());
  client_impl_->GetGcsRpcClient().UpdateJobResourceRequirements(
      request, [job_id, done](const Status &status,
                              const rpc::UpdateJobResourceRequirementsReply &reply) {
        if (done) {
          done(status);
        }
        RAY_LOG(INFO) << "Finished updating job resource requirements, status = "
                      << status << ", job id = " << job_id;
      });
  return Status::OK();
}

Status ServiceBasedJobInfoAccessor::AsyncKill(const JobID &job_id,
                                              const StatusCallback &callback,
                                              bool mark_as_failed) {
  RAY_LOG(INFO) << "Killing job " << job_id;
  rpc::DropJobRequest request;
  request.set_job_id(job_id.Binary());
  request.set_mark_as_failed(mark_as_failed);
  client_impl_->GetGcsRpcClient().DropJob(
      request,
      [job_id, callback](const Status &status, const ray::rpc::DropJobReply &reply) {
        RAY_UNUSED(reply);
        callback(status);
        RAY_LOG(INFO) << "Finished sending KillJob RPC to GCS. job_id is " << job_id;
      });
  return Status::OK();
}
...
Status FullNodeInfoAccessor::RegisterSelf(const GcsNodeInfo &local_node_info,
                                          const StatusCallback &callback) {
  auto node_id = NodeID::FromBinary(local_node_info.basic_gcs_node_info().node_id());
  RAY_LOG(DEBUG) << "Registering node info, node id = " << node_id << ", address is = "
                 << local_node_info.basic_gcs_node_info().node_manager_address();
  RAY_CHECK(local_node_id_.IsNil()) << "This node is already connected.";
  RAY_CHECK(local_node_info.basic_gcs_node_info().state() ==
            rpc::BasicGcsNodeInfo::ALIVE);
  ...
  client_impl_->GetGcsRpcClient().RegisterNode(
      request, [this, node_id, local_node_info, callback](
        ...
        if (status.ok()) {
          ...
          local_node_id_ =
              NodeID::FromBinary(local_node_info.basic_gcs_node_info().node_id());
        }
        if (callback) {
          callback(status);
        }
...
Status FullNodeInfoAccessor::UnregisterSelf() {
  RAY_CHECK(!local_node_id_.IsNil()) << "This node is disconnected.";
  NodeID node_id = NodeID::FromBinary(local_node_info_.basic_gcs_node_info().node_id());
  RAY_LOG(INFO) << "Unregistering node info, node id = " << node_id;
  rpc::UnregisterNodeRequest request;
  request.set_node_id(local_node_info_.basic_gcs_node_info().node_id());
  client_impl_->GetGcsRpcClient().UnregisterNode(
      request,
      [this, node_id](const Status &status, const rpc::UnregisterNodeReply &reply) {
        if (status.ok()) {
          local_node_info_.mutable_basic_gcs_node_info()->set_state(
              BasicGcsNodeInfo::DEAD);
...
const NodeID &FullNodeInfoAccessor::GetSelfId() const { return local_node_id_; }

const GcsNodeInfo &FullNodeInfoAccessor::GetSelfInfo() const { return local_node_info_; }

Status FullNodeInfoAccessor::AsyncRegister(const GcsNodeInfo &node_info,
                                           const StatusCallback &callback) {
...
Status FullNodeInfoAccessor::AsyncUnregister(const NodeID &node_id,
                                             const StatusCallback &callback) {
...
template <class Msg, class Req, class Reply, RpcMethod<Req, Reply> method>
Status BaseServiceBasedNodeInfoAccessor<Msg, Req, Reply, method>::AsyncGetAll(
    const MultiItemCallback<Msg> &callback, const std::string &nodegroup_id /*= ""*/) {
  RAY_LOG(DEBUG) << "Getting full information of all nodes.";
  Req request;
  request.set_nodegroup_id(nodegroup_id);
  (client_impl_->GetGcsRpcClient().*
   method)(request, [callback](const Status &status, const Reply &reply) {
    ...
    RAY_LOG(DEBUG) << "Finished getting information of all nodes, status = " << status;
  });
  return Status::OK();
}
...
template <class Msg, class Req, class Reply, RpcMethod<Req, Reply> method>
Status
BaseServiceBasedNodeInfoAccessor<Msg, Req, Reply, method>::AsyncSubscribeToNodeChange(
    const SubscribeCallback<NodeID, Msg> &subscribe, const StatusCallback &done) {
  RAY_CHECK(subscribe != nullptr);
  ...
  fetch_node_data_operation_ = [this](const StatusCallback &done) {
    auto callback = [this, done](const Status &status,
                                 const std::vector<Msg> &node_info_list) {
  ...
  subscribe_node_operation_ = [this](const StatusCallback &done) {
    ...
    return client_impl_->GetGcsPubSub().SubscribeAll(pubsub_channel_, on_subscribe, done);
  };
...
template <class Msg, class Req, class Reply, RpcMethod<Req, Reply> method>
const Msg *BaseServiceBasedNodeInfoAccessor<Msg, Req, Reply, method>::Get(
  ...
  if (entry != node_cache_.end()) {
    if (filter_dead_nodes &&
        GetStateFromNodeInfo(entry->second) == rpc::BasicGcsNodeInfo::DEAD) {
      return nullptr;
    }
    return &entry->second;
  }
...
template <class Msg, class Req, class Reply, RpcMethod<Req, Reply> method>
const std::unordered_map<NodeID, Msg>
    &BaseServiceBasedNodeInfoAccessor<Msg, Req, Reply, method>::GetAll() const {
  return node_cache_;
}

template <class Msg, class Req, class Reply, RpcMethod<Req, Reply> method>
bool BaseServiceBasedNodeInfoAccessor<Msg, Req, Reply, method>::IsRemoved(
    const NodeID &node_id) const {
  return removed_nodes_.count(node_id) == 1;
}

template <class Msg, class Req, class Reply, RpcMethod<Req, Reply> method>
Status BaseServiceBasedNodeInfoAccessor<Msg, Req, Reply, method>::AsyncReportHeartbeat(
...
template <class Msg, class Req, class Reply, RpcMethod<Req, Reply> method>
void BaseServiceBasedNodeInfoAccessor<Msg, Req, Reply, method>::HandleNotification(
    const Msg &node_info) {
  NodeID node_id = NodeID::FromBinary(GetNodeIdFromNodeInfo(node_info));
  bool is_alive = (GetStateFromNodeInfo(node_info) == rpc::BasicGcsNodeInfo::ALIVE);
  auto entry = node_cache_.find(node_id);
  bool is_notif_new;
  if (entry == node_cache_.end()) {
    ...
    bool was_alive =
        (GetStateFromNodeInfo(entry->second) == rpc::BasicGcsNodeInfo::ALIVE);
  ...
  if (is_notif_new) {
    ...
    Msg &cache_data = node_cache_[node_id];
    node_change_callback_(node_id, cache_data);
  }
}

BasicGcsNodeInfo_GcsNodeState BasicNodeInfoAccessor::GetStateFromNodeInfo(
    const BasicGcsNodeInfo &node_info) const {
  return node_info.state();
}

BasicGcsNodeInfo_GcsNodeState FullNodeInfoAccessor::GetStateFromNodeInfo(
    const GcsNodeInfo &node_info) const {
  return node_info.basic_gcs_node_info().state();
}

void BasicNodeInfoAccessor::UpdateNodeCache(const BasicGcsNodeInfo &node_info) {
  NodeID node_id = NodeID::FromBinary(GetNodeIdFromNodeInfo(node_info));
  auto &node = node_cache_[node_id];
  if (GetStateFromNodeInfo(node_info) == rpc::BasicGcsNodeInfo::ALIVE) {
    node = node_info;
  } else {
    node.set_node_id(node_info.node_id());
    node.set_state(rpc::BasicGcsNodeInfo::DEAD);
    node.set_timestamp(node_info.timestamp());
  }
}

void FullNodeInfoAccessor::UpdateNodeCache(const GcsNodeInfo &node_info) {
  NodeID node_id = NodeID::FromBinary(GetNodeIdFromNodeInfo(node_info));
  auto &node = node_cache_[node_id];
  if (GetStateFromNodeInfo(node_info) == rpc::BasicGcsNodeInfo::ALIVE) {
    node = node_info;
  } else {
    node.mutable_basic_gcs_node_info()->set_node_id(
        node_info.basic_gcs_node_info().node_id());
    node.mutable_basic_gcs_node_info()->set_state(rpc::BasicGcsNodeInfo::DEAD);
    node.mutable_basic_gcs_node_info()->set_timestamp(
        node_info.basic_gcs_node_info().timestamp());
  }
}

const std::string &BasicNodeInfoAccessor::GetNodeIdFromNodeInfo(
    const BasicGcsNodeInfo &node_info) const {
  return node_info.node_id();
}

const std::string &FullNodeInfoAccessor::GetNodeIdFromNodeInfo(
    const GcsNodeInfo &node_info) const {
  return node_info.basic_gcs_node_info().node_id();
}

template <class Msg, class Req, class Reply, RpcMethod<Req, Reply> method>
void BaseServiceBasedNodeInfoAccessor<Msg, Req, Reply, method>::AsyncResubscribe(
  ...
  if (is_pubsub_server_restarted) {
    ...
    if (subscribe_internal_config_operation_ != nullptr) {
      RAY_CHECK_OK(subscribe_internal_config_operation_(nullptr));
    }
  } else {
...
template <class Msg, class Req, class Reply, RpcMethod<Req, Reply> method>
Status BaseServiceBasedNodeInfoAccessor<Msg, Req, Reply, method>::AsyncGetInternalConfig(
    const OptionalItemCallback<std::string> &callback) {
...
template <class Msg, class Req, class Reply, RpcMethod<Req, Reply> method>
Status
BaseServiceBasedNodeInfoAccessor<Msg, Req, Reply, method>::AsyncSubscribeInternalConfig(
    const ItemCallback<std::string> &subscribe, const StatusCallback &done) {
  subscribe_internal_config_operation_ = [this, subscribe](const StatusCallback &done) {
    auto on_subscribe = [subscribe](const std::string &id,
                                    const std::string &config_json_str) {
      subscribe(config_json_str);
    };
    return client_impl_->GetGcsPubSub().Subscribe(INTERNAL_CONFIG_CHANNEL, "",
                                                  on_subscribe, done);
  };
  return subscribe_internal_config_operation_(done);
}
...
Status ServiceBasedNodeResourceInfoAccessor::AsyncGetResourceUsage(
    const NodeID &node_id, const OptionalItemCallback<rpc::ResourcesData> &callback) {
  rpc::GetResourceUsageRequest request;
  request.set_node_id(node_id.Binary());
  client_impl_->GetGcsRpcClient().GetResourceUsage(
      request, [callback](const Status &status, const rpc::GetResourceUsageReply &reply) {
        if (reply.has_resources()) {
          callback(status, reply.resources());
        } else {
          callback(status, boost::none);
        }
      });
...
Status ServiceBasedNodeResourceInfoAccessor::AsyncGetClusterResources(
    const MultiItemCallback<rpc::NodeResources> &callback) {
  rpc::GetClusterResourcesRequest request;
  client_impl_->GetGcsRpcClient().GetClusterResources(
      request,
      [callback](const Status &status, const rpc::GetClusterResourcesReply &reply) {
        callback(status, VectorFromProtobuf(reply.node_resources()));
        RAY_LOG(DEBUG)
            << "Finished getting cluster scheduling resources from GCS, status = "
            << status;
      });
  return Status::OK();
}
...
Status ServiceBasedStatsInfoAccessor::AsyncAddProfileData(
    ...
    const StatusCallback &callback) {
  if (RayConfig::instance().profiler_disabled()) {
    if (callback) {
      callback(Status::OK());
    }
    return Status::OK();
  ...
  client_impl_->GetGcsRpcClient().CreatePlacementGroup(
      request, [placement_group_spec, callback](
        ...
        if (status.ok()) {
          ...
          success_message << "Finished registering placement group. placement group id = "
                          << placement_group_spec.PlacementGroupId();
          RAY_LOG(INFO) << success_message.str();
          RAY_EVENT(INFO, EVENT_LABEL_PLACEMENT_GROUP_REGISTER_SUCCESS)
                  .WithField("job_id", placement_group_spec.GetCreatorJobId().Hex())
              << success_message.str();
        } else {
          std::ostringstream error_message;
          error_message << "Placement group id = "
                        << placement_group_spec.PlacementGroupId()
                        << " failed to be registered. " << status;
          RAY_LOG(ERROR) << error_message.str();
          RAY_EVENT(ERROR, EVENT_LABEL_PLACEMENT_GROUP_REGISTER_FAILED)
                  .WithField("job_id", placement_group_spec.GetCreatorJobId().Hex())
              << error_message.str();
        }
        if (callback) {
...
Status ServiceBasedPlacementGroupInfoAccessor::AsyncRemovePlacementGroupBundles(
    const ray::PlacementGroupID &placement_group_id,
    const std::vector<int> &bundle_indexes, const StatusCallback &callback) {
  RAY_LOG(DEBUG) << "Removing bundles from the placement group: " << placement_group_id;
  RAY_CHECK(callback);
  rpc::RemovePlacementGroupBundlesRequest request;
  request.set_placement_group_id(placement_group_id.Binary());
  for (const auto &index : bundle_indexes) {
    request.add_bundle_indexes(index);
  }

  client_impl_->GetGcsRpcClient().RemovePlacementGroupBundles(
      request, [placement_group_id, callback, bundle_indexes](
                   const Status &, const rpc::RemovePlacementGroupBundlesReply &reply) {
        auto status =
            reply.status().code() == (int)StatusCode::OK
                ? Status()
                : Status(StatusCode(reply.status().code()), reply.status().message());
        std::ostringstream removed_bundle_indexes_str;
        std::copy(bundle_indexes.begin(), bundle_indexes.end(),
                  std::ostream_iterator<int>(removed_bundle_indexes_str, ","));

        if (status.ok()) {
          std::ostringstream success_message;
          success_message << "Finished remove placement group bundles: ["
                          << removed_bundle_indexes_str.str()
                          << "] from placement group: " << placement_group_id;
          RAY_LOG(INFO) << success_message.str();
          RAY_EVENT(INFO, EVENT_LABEL_PLACEMENT_GROUP_REMOVE_BUNDLES_SUCCESS)
                  .WithField("job_id", placement_group_id.JobId().Hex())
              << success_message.str();
        } else {
          std::ostringstream error_message;
          error_message << "Failed to remove placement group bundles: ["
                        << removed_bundle_indexes_str.str()
                        << "] from placement group: " << placement_group_id << ", cause "
                        << status.message();
          RAY_LOG(ERROR) << error_message.str();
          RAY_EVENT(ERROR, EVENT_LABEL_PLACEMENT_GROUP_REMOVE_BUNDLES_FAILED)
                  .WithField("job_id", placement_group_id.JobId().Hex())
              << error_message.str();
        }
        callback(status);
...
Status ServiceBasedPlacementGroupInfoAccessor::AsyncSubscribeAllBundlesChangedEvent(
    const SubscribeCallback<PlacementGroupID,
                            rpc::PlacementGroupBundlesChangedNotification> &subscribe,
    const StatusCallback &done) {
  RAY_LOG(DEBUG) << "Subscribing all placement group bundles changed notification.";
  RAY_CHECK(subscribe != nullptr);
  auto on_subscribe = [subscribe](const std::string &id, const std::string &data) {
    rpc::PlacementGroupBundlesChangedNotification bundles_change_notification;
    bundles_change_notification.ParseFromString(data);
  ...
  Status status = client_impl_->GetGcsPubSub().SubscribeAll(
      PLACEMENT_GROUP_BUNDELS_CHANGED_CHANNEL, on_subscribe, done);
  RAY_LOG(DEBUG)
      << "Finished subscribing all placement group bundles changed notification.";
  return status;
}

ServiceBasedNodegroupInfoAccessor::ServiceBasedNodegroupInfoAccessor(
    ServiceBasedGcsClient *client_impl)
    : client_impl_(client_impl) {}

Status ServiceBasedNodegroupInfoAccessor::AsyncGetAll(
    const MultiItemCallback<rpc::NodegroupData> &callback) {
  RAY_LOG(DEBUG) << "Getting all nodegroup info.";
  RAY_CHECK(callback);
  rpc::GetAllNodegroupsRequest request;
  request.set_include_sub_nodegroups(true);
  client_impl_->GetGcsRpcClient().GetAllNodegroups(
      request, [callback](const Status &status, const rpc::GetAllNodegroupsReply &reply) {
        std::vector<rpc::NodegroupData> nodegroup_data_list;
        nodegroup_data_list.insert(nodegroup_data_list.end(),
                                   reply.nodegroup_data_list().begin(),
                                   reply.nodegroup_data_list().end());
        callback(status, std::move(nodegroup_data_list));
        RAY_LOG(DEBUG) << "Finished getting all nodegroup info.";
      });
  return Status::OK();
}

Status ServiceBasedNodegroupInfoAccessor::AsyncSubscribeAll(
    const SubscribeCallback<NodegroupID, rpc::NodegroupData> &subscribe,
    const StatusCallback &done) {
  RAY_LOG(DEBUG) << "Subscribing all nodegroups.";
  RAY_CHECK(subscribe != nullptr);
  fetch_all_data_operation_ = [this, subscribe](const StatusCallback &done) {
    auto callback = [subscribe, done](
                        const Status &status,
                        const std::vector<rpc::NodegroupData> &nodegroup_info_list) {
      for (auto &nodegroup_data : nodegroup_info_list) {
        subscribe(NodegroupID::FromBinary(nodegroup_data.nodegroup_id()), nodegroup_data);
      }
      if (done) {
        done(status);
      }
  ...
  subscribe_operation_ = [this, subscribe](const StatusCallback &done) {
    auto on_subscribe = [subscribe](const std::string &id, const std::string &data) {
      rpc::NodegroupData nodegroup_data;
      nodegroup_data.ParseFromString(data);
      subscribe(NodegroupID::FromBinary(id), nodegroup_data);
    };
    return client_impl_->GetGcsPubSub().SubscribeAll(NODEGROUP_CHANNEL, on_subscribe,
                                                     done);
  };
  return subscribe_operation_(
      [this, done](const Status &status) { fetch_all_data_operation_(done); });
...
void ServiceBasedNodegroupInfoAccessor::AsyncResubscribe(
    bool is_pubsub_server_restarted) {
  RAY_LOG(INFO) << "Reestablishing subscription for nodegroup info.";
  auto fetch_all_done = [](const Status &status) {
    RAY_LOG(INFO)
        << "Finished fetching all nodegroup information from gcs server after gcs "
           "server or pub-sub server is restarted.";
  };
...
Status ServiceBasedInternalKVAccessor::AsyncInternalKVDel(
    const std::string &key, const OptionalItemCallback<int> &callback) {
...
ServiceBasedFrozenNodeAccessor::ServiceBasedFrozenNodeAccessor(
    ServiceBasedGcsClient *client_impl)
    : client_impl_(client_impl) {}

Status ServiceBasedFrozenNodeAccessor::AsyncSubscribeFrozenNode(
    const ItemCallback<std::unordered_set<std::string>> &subscribe,
    const StatusCallback &done) {
  auto on_pub_sub_subscribe = [subscribe](const std::string &id,
                                          const std::string &data) {
    if (subscribe) {
      rpc::FrozenNodesNotification notification;
      notification.ParseFromString(data);
      std::unordered_set<std::string> std_node_name_set(
          notification.frozen_node_set().begin(), notification.frozen_node_set().end());
      subscribe(std_node_name_set);
    }
  };

  subscribe_operation_ = [this, on_pub_sub_subscribe](const StatusCallback sub_done) {
    return client_impl_->GetGcsPubSub().SubscribeAll(FROZEN_HOST_CHANNEL,
                                                     on_pub_sub_subscribe, sub_done);
  };

  fetch_all_data_operation_ = [this, subscribe](const StatusCallback fetch_done) {
    return AsyncGetAll(
        [subscribe, fetch_done](const std::unordered_set<std::string> frozen_node_set) {
          if (subscribe) {
            subscribe(frozen_node_set);
          }
          fetch_done(Status::OK());
        });
  };

  return subscribe_operation_(
      [this, done](Status status) { return fetch_all_data_operation_(done); });
}

void ServiceBasedFrozenNodeAccessor::AsyncResubscribe(bool is_pubsub_server_restarted) {
  RAY_LOG(INFO) << "Reestablishing subscription for frozen node info.";
  auto fetch_all_done = [](const Status &status) {
    RAY_LOG(INFO)
        << "Finished fetching all frozen node information from gcs server after gcs "
           "server or pub-sub server is restarted.";
  };
...
Status ServiceBasedFrozenNodeAccessor::AsyncGetAll(
    const ItemCallback<std::unordered_set<std::string>> &callback) {
  RAY_CHECK(callback);
  rpc::GetAllFrozenNodesRequest request;
  client_impl_->GetGcsRpcClient().GetAllFrozenNodes(
      request,
      [callback](const Status &status, const rpc::GetAllFrozenNodesReply &reply) {
        std::unordered_set<std::string> std_node_name_set(reply.frozen_node_set().begin(),
                                                          reply.frozen_node_set().end());
        callback(std_node_name_set);
      });
  return Status::OK();
}

Status ServiceBasedFrozenNodeAccessor::ReportFreezeDone(const std::string &node_name,
                                                        const NodeID &node_id,
                                                        const StatusCallback &done) {
  RAY_LOG(INFO) << "Reporting freeze done, node name=" << node_name;
  rpc::ReportFreezeDoneRequest request;
  request.set_node_name(node_name);
  request.set_node_id(node_id.Binary());
  client_impl_->GetGcsRpcClient().ReportFreezeDone(
      request, [done](const Status &status, const rpc::ReportFreezeDoneReply &reply) {
        done(status);
      });
  return Status::OK();
}

ServiceBasedActorMigrationAccessor::ServiceBasedActorMigrationAccessor(
    ServiceBasedGcsClient *client_impl)
    : client_impl_(client_impl) {}

Status ServiceBasedActorMigrationAccessor::AsyncSubscribeActorMigrationEvents(
    const ItemCallback<rpc::ActorMigrationNotificationBatch> &subscribe,
    const StatusCallback &done) {
  auto on_pub_sub_subscribe = [subscribe](const std::string &id,
                                          const std::string &data) {
    if (subscribe) {
      rpc::ActorMigrationNotificationBatch notification_batch;
      notification_batch.ParseFromString(data);
      subscribe(notification_batch);
    }
  };

  subscribe_operation_ = [this, on_pub_sub_subscribe, done]() {
    RAY_UNUSED(client_impl_->GetGcsPubSub().SubscribeAll(ACTOR_MIGRATION_CHANNEL,
                                                         on_pub_sub_subscribe, done));
  };
...
void ServiceBasedActorMigrationAccessor::AsyncResubscribe(
    bool is_pubsub_server_restarted) {
  RAY_LOG(DEBUG) << "Reestablishing subscription for node resource info.";
  // If the pub-sub server has also restarted, we need to resubscribe to the pub-sub
  // server.
...
template class BaseServiceBasedNodeInfoAccessor<GcsNodeInfo, GetAllFullNodeInfoRequest,
                                                GetAllFullNodeInfoReply,
                                                &GcsRpcClient::GetAllFullNodeInfo>;

template class BaseServiceBasedNodeInfoAccessor<
    BasicGcsNodeInfo, GetAllBasicNodeInfoRequest, GetAllBasicNodeInfoReply,
    &GcsRpcClient::GetAllBasicNodeInfo>;
}  // namespace gcs
}  // namespace ray
