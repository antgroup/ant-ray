...
class ServiceBasedJobInfoAccessor : public JobInfoAccessor {
 public:
  ...
  Status AsyncPutData(const JobID &job_id, const std::string &key,
                      const std::string &data, const StatusCallback &callback) override;

  Status AsyncGetData(const JobID &job_id, const std::string &key,
                      const OptionalItemCallback<std::string> &callback) override;

  Status AsyncAdd(const std::shared_ptr<rpc::JobTableData> &data_ptr,
                  const StatusCallback &callback) override;

  Status AsyncNotifyDriverExit(const JobID &job_id, bool exit_with_error,
                               const StatusCallback &callback) override;
  ...
  Status AsyncKill(const JobID &job_id, const StatusCallback &callback,
                   bool mark_as_failed = false) override;

  Status AsyncGet(const JobID &job_id,
                  const OptionalItemCallback<rpc::JobTableData> &callback) override;
  ...
  Status AsyncUpdateJobResourceRequirements(
      const JobID &job_id,
...
using RpcMethod = void (GcsRpcClient::*)(const Req &, const ClientCallback<Reply> &);
...
class BaseServiceBasedNodeInfoAccessor {
 public:
  explicit BaseServiceBasedNodeInfoAccessor(ServiceBasedGcsClient *client_impl,
                                            const std::string &pubsub_channel)
      : client_impl_(client_impl), pubsub_channel_(pubsub_channel) {}

  virtual ~BaseServiceBasedNodeInfoAccessor() = default;

  virtual Status AsyncGetAll(const MultiItemCallback<Msg> &callback,
  ...
  virtual const Msg *Get(const NodeID &node_id, bool filter_dead_nodes = true) const;

  virtual const std::unordered_map<NodeID, Msg> &GetAll() const;
  ...
  virtual Status AsyncSubscribeInternalConfig(const ItemCallback<std::string> &subscribe,
                                              const StatusCallback &done);

 protected:
  virtual BasicGcsNodeInfo_GcsNodeState GetStateFromNodeInfo(
      const Msg &node_info) const = 0;
  virtual void UpdateNodeCache(const Msg &node_info) = 0;
  virtual const std::string &GetNodeIdFromNodeInfo(const Msg &node_info) const = 0;
  /// Save the subscribe operation in this function, so we can call it again when PubSub
  /// server restarts from a failure.
  SubscribeOperation subscribe_node_operation_;

  SubscribeOperation subscribe_internal_config_operation_;

  /// Save the fetch data operation in this function, so we can call it again when GCS
  /// server restarts from a failure.
  FetchDataOperation fetch_node_data_operation_;

  void HandleNotification(const Msg &node_info);

  ServiceBasedGcsClient *client_impl_;

  const std::string pubsub_channel_;

  using NodeChangeCallback = std::function<void(const NodeID &id, const Msg &node_info)>;
  ...
  std::unordered_map<NodeID, Msg> node_cache_;
  /// The set of removed nodes.
  std::unordered_set<NodeID> removed_nodes_;
};

class FullNodeInfoAccessor
    : public BaseServiceBasedNodeInfoAccessor<GcsNodeInfo, GetAllFullNodeInfoRequest,
                                              GetAllFullNodeInfoReply,
                                              &GcsRpcClient::GetAllFullNodeInfo> {
 public:
  explicit FullNodeInfoAccessor(ServiceBasedGcsClient *client_impl)
      : BaseServiceBasedNodeInfoAccessor(client_impl, FULL_NODE_INFO_CHANNEL) {}

  virtual Status RegisterSelf(const GcsNodeInfo &local_node_info,
                              const StatusCallback &callback);

  virtual Status UnregisterSelf();

  virtual const NodeID &GetSelfId() const;

  virtual const GcsNodeInfo &GetSelfInfo() const;

  virtual Status AsyncRegister(const GcsNodeInfo &node_info,
                               const StatusCallback &callback);

  virtual Status AsyncUnregister(const NodeID &node_id, const StatusCallback &callback);

 protected:
  BasicGcsNodeInfo_GcsNodeState GetStateFromNodeInfo(
      const GcsNodeInfo &node_info) const override;
  void UpdateNodeCache(const GcsNodeInfo &node_info) override;
  const std::string &GetNodeIdFromNodeInfo(const GcsNodeInfo &node_info) const override;
  ;
...
class BasicNodeInfoAccessor
    : public BaseServiceBasedNodeInfoAccessor<
          BasicGcsNodeInfo, GetAllBasicNodeInfoRequest, GetAllBasicNodeInfoReply,
          &GcsRpcClient::GetAllBasicNodeInfo> {
 public:
  explicit BasicNodeInfoAccessor(ServiceBasedGcsClient *client_impl)
      : BaseServiceBasedNodeInfoAccessor(client_impl, BASIC_NODE_INFO_CHANNEL) {}

 protected:
  BasicGcsNodeInfo_GcsNodeState GetStateFromNodeInfo(
      const BasicGcsNodeInfo &node_info) const override;
  void UpdateNodeCache(const BasicGcsNodeInfo &node_info) override;
  const std::string &GetNodeIdFromNodeInfo(
      const BasicGcsNodeInfo &node_info) const override;
};
...
class ServiceBasedNodeResourceInfoAccessor : public NodeResourceInfoAccessor {
 public:
  ...
  Status AsyncGetResourceUsage(
      const NodeID &node_id,
      const OptionalItemCallback<rpc::ResourcesData> &callback) override;

  Status AsyncGetAllResourceUsage(
      const ItemCallback<rpc::ResourceUsageBatchData> &callback) override;

  Status AsyncGetClusterResources(
...
class ServiceBasedPlacementGroupInfoAccessor : public PlacementGroupInfoAccessor {
 ...
 public:
  ...
  Status AsyncRemovePlacementGroupBundles(const ray::PlacementGroupID &placement_group_id,
  ...
  Status AsyncSubscribeAllBundlesChangedEvent(
      const SubscribeCallback<PlacementGroupID,
                              rpc::PlacementGroupBundlesChangedNotification> &subscribe,
      const StatusCallback &done) override;
...
class ServiceBasedNodegroupInfoAccessor : public NodegroupInfoAccessor {
 public:
  explicit ServiceBasedNodegroupInfoAccessor(ServiceBasedGcsClient *client_impl);

  virtual ~ServiceBasedNodegroupInfoAccessor() = default;

  Status AsyncGetAll(const MultiItemCallback<rpc::NodegroupData> &callback) override;
...
class ServiceBasedFrozenNodeAccessor : public FrozenNodeAccessor {
 public:
  explicit ServiceBasedFrozenNodeAccessor(ServiceBasedGcsClient *client_impl);

  Status AsyncSubscribeFrozenNode(
      const ItemCallback<std::unordered_set<std::string>> &subscribe,
      const StatusCallback &done) override;

  Status AsyncGetAll(
      const ItemCallback<std::unordered_set<std::string>> &callback) override;

  Status ReportFreezeDone(const std::string &node_name, const NodeID &node_id,
                          const StatusCallback &done) override;
...
class ServiceBasedActorMigrationAccessor : public ActorMigrationAccessor {
 public:
  explicit ServiceBasedActorMigrationAccessor(ServiceBasedGcsClient *client_impl);

  Status AsyncSubscribeActorMigrationEvents(
 ...
 private:
  ServiceBasedGcsClient *client_impl_;
  std::function<void()> subscribe_operation_;
};
