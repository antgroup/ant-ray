...
class RAY_EXPORT ServiceBasedGcsClient : public GcsClient {
 public:
  explicit ServiceBasedGcsClient(const GcsClientOptions &options,
                                 bool gcs_service_address_check_enable = true,
  ...
  std::pair<std::string, int> GetApiServerAddress() override;
 ...
 private:
  ...
  bool GetGcsServerAddressFromRedis(RedisContextWrapper *context,
  ...
  bool gcs_service_address_check_enable_;
