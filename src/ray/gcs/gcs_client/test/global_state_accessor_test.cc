...
TEST_F(GlobalStateAccessorTest, TestNodeTable) {
  ...
  for (int index = 0; index < node_count; ++index) {
    ...
    ASSERT_EQ(node_data.basic_gcs_node_info().node_manager_address(),
              std::string("127.0.0.") +
                  std::to_string(node_data.basic_gcs_node_info().node_manager_port()));
  }
}
...
TEST_F(GlobalStateAccessorTest, TestNodeResourceTable) {
  ...
  for (int index = 0; index < node_count; ++index) {
    auto node_table_data =
        Mocker::GenNodeInfo(index, std::string("127.0.0.") + std::to_string(index));
    auto node_id = NodeID::FromBinary(node_table_data->basic_gcs_node_info().node_id());
    std::promise<bool> promise;
    RAY_CHECK_OK(gcs_client_->FullNodeInfo().AsyncRegister(
        *node_table_data, [&promise](Status status) { promise.set_value(status.ok()); }));
    WaitReady(promise.get_future(), timeout_ms_);
  ...
  for (int index = 0; index < node_count; ++index) {
    rpc::GcsNodeInfo node_data;
    node_data.ParseFromString(node_table[index]);
    auto resource_map_str = global_state_->GetNodeResourceInfo(
        NodeID::FromBinary(node_data.basic_gcs_node_info().node_id()));
    rpc::ResourceMap resource_map;
    resource_map.ParseFromString(resource_map_str);
    ASSERT_EQ(
        static_cast<uint32_t>(
            (*resource_map.mutable_items())[std::to_string(node_data.basic_gcs_node_info()
                                                               .node_manager_port())]
                .resource_capacity()),
        node_data.basic_gcs_node_info().node_manager_port() + 1);
  }
}
...
TEST_F(GlobalStateAccessorTest, TestGetAllResourceUsage) {
  ...
  resources1->set_node_id(node_table_data->basic_gcs_node_info().node_id());
  ...
  heartbeat2->set_node_id(node_table_data->basic_gcs_node_info().node_id());
  ...
  heartbeat3->set_node_id(node_table_data->basic_gcs_node_info().node_id());
