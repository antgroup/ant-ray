        ...
        R"(
{
 ...
 protected:
  ...
  bool NotifyDriverExit(const JobID &job_id) {
    std::promise<bool> promise;
    RAY_CHECK_OK(gcs_client_->Jobs().AsyncNotifyDriverExit(
        job_id, /*exit_with_error=*/false,
  ...
  bool RegisterActor(const std::shared_ptr<rpc::ActorTableData> &actor_table_data,
    ...
    // worker client is called during the actor registration process. In order to
    // simulate the scenario of registration failure, we set the address to an illegal
    // value.
    if (!is_detached) {
      rpc::Address address;
  ...
  bool SubscribeToNodeChange(
    ...
    std::promise<bool> promise;
    RAY_CHECK_OK(gcs_client_->FullNodeInfo().AsyncSubscribeToNodeChange(
        subscribe, [&promise](Status status) { promise.set_value(status.ok()); }));
    return WaitReady(promise.get_future(), timeout_ms_);
  }

  bool RegisterSelf(const rpc::GcsNodeInfo &local_node_info) {
    Status status = gcs_client_->FullNodeInfo().RegisterSelf(local_node_info, nullptr);
    return status.ok();
  }

  bool UnregisterSelf() {
    Status status = gcs_client_->FullNodeInfo().UnregisterSelf();
    return status.ok();
  }

  bool RegisterNode(const rpc::GcsNodeInfo &node_info) {
    std::promise<bool> promise;
    RAY_CHECK_OK(gcs_client_->FullNodeInfo().AsyncRegister(
  ...
  std::vector<rpc::GcsNodeInfo> GetNodeInfoList() {
    ...
    RAY_CHECK_OK(gcs_client_->FullNodeInfo().AsyncGetAll(
  ...
  bool UnregisterNode(const NodeID &node_id) {
    std::promise<bool> promise;
    RAY_CHECK_OK(gcs_client_->FullNodeInfo().AsyncUnregister(
  ...
  bool ReportHeartbeat(const std::shared_ptr<rpc::HeartbeatTableData> heartbeat) {
    std::promise<bool> promise;
    RAY_CHECK_OK(gcs_client_->FullNodeInfo().AsyncReportHeartbeat(
  ...
  absl::flat_hash_set<NodeID> RegisterNodeAndMarkDead(int node_count) {
    ...
    for (int index = 0; index < node_count; ++index) {
      ...
      auto node_id = NodeID::FromBinary(node_info->basic_gcs_node_info().node_id());
...
TEST_F(ServiceBasedGcsClientTest, TestJobInfo) {
  ...
  ASSERT_TRUE(NotifyDriverExit(add_job_id));
...
TEST_F(ServiceBasedGcsClientTest, DISABLED_TestActorSubscribeAll) {
...
TEST_F(ServiceBasedGcsClientTest, TestNodeInfo) {
  ...
  NodeID node1_id = NodeID::FromBinary(gcs_node1_info->basic_gcs_node_info().node_id());
  ...
  auto on_subscribe = [&register_count, &unregister_count](const NodeID &node_id,
    ...
    if (data.basic_gcs_node_info().state() == rpc::BasicGcsNodeInfo::ALIVE) {
      ++register_count;
    } else if (data.basic_gcs_node_info().state() == rpc::BasicGcsNodeInfo::DEAD) {
      ++unregister_count;
    }
  };
  ...
  EXPECT_EQ(gcs_client_->FullNodeInfo().GetSelfId(), node1_id);
  EXPECT_EQ(gcs_client_->FullNodeInfo().GetSelfInfo().basic_gcs_node_info().node_id(),
            gcs_node1_info->basic_gcs_node_info().node_id());
  EXPECT_EQ(gcs_client_->FullNodeInfo().GetSelfInfo().basic_gcs_node_info().state(),
            gcs_node1_info->basic_gcs_node_info().state());

  // Register a node to GCS.
  auto gcs_node2_info = Mocker::GenNodeInfo();
  NodeID node2_id = NodeID::FromBinary(gcs_node2_info->basic_gcs_node_info().node_id());
  ...
  ASSERT_TRUE(gcs_client_->FullNodeInfo().Get(node1_id));
  EXPECT_EQ(gcs_client_->FullNodeInfo().GetAll().size(), 2);
  ...
  EXPECT_EQ(node_list.size(), 2);
  EXPECT_EQ(node_list[0].basic_gcs_node_info().state(), rpc::BasicGcsNodeInfo::DEAD);
  EXPECT_EQ(node_list[1].basic_gcs_node_info().state(), rpc::BasicGcsNodeInfo::DEAD);
  ASSERT_TRUE(gcs_client_->FullNodeInfo().IsRemoved(node2_id));
}
...
TEST_F(ServiceBasedGcsClientTest, TestJobTableResubscribe) {
  ...
  // restarted, and the GCS server keeps a job record, so `job_update_count` plus one.
  WaitForExpectedCount(job_update_count, 2);

  ASSERT_TRUE(NotifyDriverExit(job_id));
  WaitForExpectedCount(job_update_count, 3);
}

TEST_F(ServiceBasedGcsClientTest, DISABLED_TestActorTableResubscribe) {
  ...
  // NOTE: GCS will not reply when actor registration fails, so when GCS restarts, gcs
  // client will register the actor again. When an actor is registered, the status in
  // GCS is `DEPENDENCIES_UNREADY`. When GCS finds that the owner of an actor is nil, it
  // will destroy the actor and the status of the actor will change to `DEAD`. The GCS
  // client fetch actor info from the GCS server, and the status of the actor may be
  ...
  // `DEAD`, we will fetch one record, so `num_subscribe_all_notifications` will
  // increase by 2.
...
TEST_F(ServiceBasedGcsClientTest, TestNodeTableResubscribe) {
  ...
  resources->set_node_id(node_info->basic_gcs_node_info().node_id());
  ...
  ASSERT_TRUE(RegisterNode(*node_info));
  node_id = NodeID::FromBinary(node_info->basic_gcs_node_info().node_id());
  ASSERT_TRUE(UpdateResources(node_id, key));
  resources->set_node_id(node_info->basic_gcs_node_info().node_id());
  ASSERT_TRUE(ReportResourceUsage(resources));
...
/// ANT Internally disable this case as we added retry in redis client.
TEST_F(ServiceBasedGcsClientTest, DISABLED_TestGcsRedisFailureDetector) {
  // Stop redis.
...
TEST_F(ServiceBasedGcsClientTest, DISABLED_TestEvictExpiredDestroyedActors) {
...
TEST_F(ServiceBasedGcsClientTest, TestEvictExpiredDeadNodes) {
  ...
  for (const auto &node : nodes) {
    EXPECT_TRUE(
        node_ids.contains(NodeID::FromBinary(node.basic_gcs_node_info().node_id())));
  }
}
