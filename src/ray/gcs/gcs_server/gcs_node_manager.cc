...
GcsNodeManager::GcsNodeManager(boost::asio::io_service &main_io_service,
    ...
    : main_io_service_(main_io_service),
      ...
      send_batched_resources_timer_(io_service),
      gcs_pub_sub_(gcs_pub_sub),
      gcs_table_storage_(gcs_table_storage) {
  SendBatchedResources();
}
...
void GcsNodeManager::HandleReportResources(const rpc::ReportResourcesRequest &request,
                                           rpc::ReportResourcesReply *reply,
                                           rpc::SendReplyCallback send_reply_callback) {
  NodeID node_id = NodeID::FromBinary(request.resources_data().client_id());
  auto resources_data = std::make_shared<rpc::ResourcesData>();
  resources_data->CopyFrom(request.resources_data());
...
void GcsNodeManager::SendBatchedResources() {
  ...
  auto report_resources_period = boost::posix_time::milliseconds(
      RayConfig::instance().raylet_report_resources_period_milliseconds());
  send_batched_resources_timer.expires_from_now(report_resources_period);
  send_batched_resources_timer.async_wait([this](const boost::system::error_code &error) {
    if (error == boost::asio::error::operation_aborted) {
      // `operation_aborted` is set when `send_batched_resources_timer` is canceled or
      // destroyed. The Monitor lifetime may be short than the object who use it. (e.g.
      // gcs_server)
      return;
    }
    RAY_CHECK(!error) << "Periodically send resources batch failed with error: "
                      << error.message();
    SendBatchedResources();
  });
}
