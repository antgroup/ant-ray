...
Status GcsJobDataTable::PutData(const JobID &job_id, const std::string &key,
                                const std::string &data, const StatusCallback &callback) {
  return store_client_->AsyncPut(table_name_, job_id.Hex() + "_" + key, data, callback);
}

Status GcsJobDataTable::GetData(const JobID &job_id, const std::string &key,
  ...
  return store_client_->AsyncGet(table_name_, job_id.Hex() + "_" + key, on_done);
}
...
template class GcsTable<NodegroupID, rpc::NodegroupData>;
...
template class GcsTable<FrozenID, rpc::FrozenNodesNotification>;
template class GcsTable<UniqueID, GcsWorkerProcessRuntimeResourceTableData>;
template class GcsTableWithJobId<UniqueID, GcsWorkerProcessRuntimeResourceTableData>;
