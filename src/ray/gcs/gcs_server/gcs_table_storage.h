...
using rpc::GcsWorkerProcessRuntimeResourceTableData;
...
typedef NodegroupID FrozenID;
const FrozenID CONST_KEY_FROZEN_TABLE = FrozenID::FromBinary("FROZEN_TABLE_KEY");
typedef NodegroupID MigrationID;
const MigrationID CONST_KEY_ACTOR_MIGRATION_TABLE =
    FrozenID::FromBinary("ACTOR_MIGRATION_TABLE_KEY");
...
class GcsNodegroupTable : public GcsTable<NodegroupID, rpc::NodegroupData> {
 public:
  explicit GcsNodegroupTable(std::shared_ptr<StoreClient> &store_client)
      : GcsTable(store_client) {
    table_name_ = TablePrefix_Name(TablePrefix::NAMESPACE);
  }
};
...
class GcsJobDataTable : public GcsTable<JobID, JobTableData> {
 public:
  explicit GcsJobDataTable(std::shared_ptr<StoreClient> &store_client)
      : GcsTable(store_client) {
    table_name_ = TablePrefix_Name(TablePrefix::JOB_DATA);
  }

  Status PutData(const JobID &job_id, const std::string &key, const std::string &data,
                 const StatusCallback &callback);
  Status GetData(const JobID &job_id, const std::string &key,
                 const ItemCallback<std::string> &callback);
};

class GcsFrozenNodeTable : public GcsTable<FrozenID, rpc::FrozenNodesNotification> {
 public:
  explicit GcsFrozenNodeTable(std::shared_ptr<StoreClient> &store_client)
      : GcsTable(store_client) {
    table_name_ = TablePrefix_Name(TablePrefix::FROZEN_NODES_DATA);
  }
};

class GcsWorkerProcessRuntimeResourceTable
    : public GcsTableWithJobId<UniqueID, GcsWorkerProcessRuntimeResourceTableData> {
 public:
  explicit GcsWorkerProcessRuntimeResourceTable(
      std::shared_ptr<StoreClient> &store_client)
      : GcsTableWithJobId(store_client) {
    table_name_ = TablePrefix_Name(TablePrefix::WORKER_RUNTIME_RESOURCE);
  }

 private:
  JobID GetJobIdFromKey(const UniqueID &key) override {
    return ray::GetJobIdFromActorWokerAssignmentId(key);
  }
};
...
class GcsTableStorage {
 public:
  ...
  GcsNodegroupTable &NodegroupTable() {
    RAY_CHECK(nodegroup_table_ != nullptr);
    return *nodegroup_table_;
  }
  ...
  GcsJobDataTable &JobDataTable() {
    RAY_CHECK(job_data_table_ != nullptr);
    return *job_data_table_;
  }

  GcsFrozenNodeTable &FrozenNodeTable() {
    RAY_CHECK(frozen_node_table_ != nullptr);
    return *frozen_node_table_;
  }

  GcsWorkerProcessRuntimeResourceTable &WorkerProcessRuntimeResourceTable() {
    RAY_CHECK(worker_process_runtime_resource_table_ != nullptr);
    return *worker_process_runtime_resource_table_;
  }
 ...
 protected:
  ...
  std::unique_ptr<GcsNodegroupTable> nodegroup_table_;
  std::unique_ptr<GcsJobDataTable> job_data_table_;
  std::unique_ptr<GcsFrozenNodeTable> frozen_node_table_;
  std::unique_ptr<GcsWorkerProcessRuntimeResourceTable>
      worker_process_runtime_resource_table_;
};
...
class RedisGcsTableStorage : public GcsTableStorage {
 public:
  explicit RedisGcsTableStorage(std::shared_ptr<RedisClient> redis_client) {
    ...
    nodegroup_table_.reset(new GcsNodegroupTable(store_client_));
    job_data_table_.reset(new GcsJobDataTable(store_client_));
    frozen_node_table_.reset(new GcsFrozenNodeTable(store_client_));
    worker_process_runtime_resource_table_.reset(
        new GcsWorkerProcessRuntimeResourceTable(store_client_));
  }
};
...
class InMemoryGcsTableStorage : public GcsTableStorage {
 public:
  explicit InMemoryGcsTableStorage(instrumented_io_context &main_io_service)
      : InMemoryGcsTableStorage(std::make_shared<InMemoryStoreClient>(main_io_service)) {}

  explicit InMemoryGcsTableStorage(std::shared_ptr<InMemoryStoreClient> store_client) {
    RAY_CHECK(store_client != nullptr);
    store_client_ = store_client;
    job_table_.reset(new GcsJobTable(store_client_));
    actor_table_.reset(new GcsActorTable(store_client_));
    actor_task_spec_table_.reset(new GcsActorTaskSpecTable(store_client_));
