...
class GcsActorSchedulerTest : public ::testing::Test {
 public:
  void SetUp() override {
    ...
    gcs_label_manager_ = std::make_shared<gcs::GcsLabelManager>();
...
TEST_F(GcsActorSchedulerTest, TestScheduleActorSuccess) {
  ...
  ASSERT_TRUE(
      raylet_client_->GrantWorkerLease(node->basic_gcs_node_info().node_manager_address(),
                                       node->basic_gcs_node_info().node_manager_port(),
                                       worker_id, node_id, NodeID::Nil()));
...
TEST_F(GcsActorSchedulerTest, TestScheduleRetryWhenLeasing) {
  ...
  ASSERT_TRUE(raylet_client_->GrantWorkerLease(
      ...
      node->basic_gcs_node_info().node_manager_port(), WorkerID::FromRandom(), node_id,
      NodeID::Nil(), Status::IOError("")));
...
TEST_F(GcsActorSchedulerTest, TestNodeFailedWhenLeasing) {
  ...
  ASSERT_TRUE(
      raylet_client_->GrantWorkerLease(node->basic_gcs_node_info().node_manager_address(),
                                       node->basic_gcs_node_info().node_manager_port(),
                                       WorkerID::FromRandom(), node_id, NodeID::Nil()));
  ASSERT_EQ(1, raylet_client_->num_workers_requested);
  ASSERT_EQ(0, raylet_client_->callbacks.size());
  ASSERT_EQ(0, gcs_actor_scheduler_->num_retry_leasing_count_);
...
TEST_F(GcsActorSchedulerTest, TestSpillback) {
  ...
  auto node_id_1 = NodeID::FromBinary(node1->basic_gcs_node_info().node_id());
  ...
  auto node_id_2 = NodeID::FromBinary(node2->basic_gcs_node_info().node_id());
  ...
  auto invalid_node_id =
      NodeID::FromBinary(Mocker::GenNodeInfo()->basic_gcs_node_info().node_id());
  ASSERT_TRUE(raylet_client_->GrantWorkerLease(
      node2->basic_gcs_node_info().node_manager_address(),
      node2->basic_gcs_node_info().node_manager_port(), WorkerID::Nil(), node_id_1,
      invalid_node_id));
  ASSERT_EQ(2, raylet_client_->num_workers_requested);
  ASSERT_EQ(1, raylet_client_->callbacks.size());
  ASSERT_EQ(0, worker_client_->callbacks.size());
  ...
  ASSERT_TRUE(raylet_client_->GrantWorkerLease(
      node2->basic_gcs_node_info().node_manager_address(),
      node2->basic_gcs_node_info().node_manager_port(), worker_id, node_id_2,
      NodeID::Nil()));
  ASSERT_EQ(0, raylet_client_->callbacks.size());
  ASSERT_EQ(1, worker_client_->callbacks.size());
...
TEST_F(GcsActorSchedulerTest, TestReschedule) {
  ...
  // Grant a worker, then the actor creation request should be send to the worker.
  ASSERT_TRUE(raylet_client_->GrantWorkerLease(
      node1->basic_gcs_node_info().node_manager_address(),
      node1->basic_gcs_node_info().node_manager_port(), worker_id, node_id_1,
      NodeID::Nil()));
  ASSERT_EQ(0, raylet_client_->callbacks.size());
  ASSERT_EQ(1, worker_client_->callbacks.size());
