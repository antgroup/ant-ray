...
TEST_F(GcsPlacementGroupSchedulerTest, TestNodeDeadDuringPreparingResources) {
  ...
  scheduler_->ScheduleUnplacedBundles(placement_group, failure_handler, success_handler);
  ASSERT_TRUE(raylet_clients_[0]->GrantPrepareBundleResources());
  RemoveNode(NodeID::FromBinary(node1->basic_gcs_node_info().node_id()));
...
TEST_F(GcsPlacementGroupSchedulerTest, TestNodeDeadDuringCommittingResources) {
  ...
  ASSERT_EQ(placement_group->GetCreatedBundles().size(), 1);

  scheduler_->ScheduleUnplacedBundles(placement_group, failure_handler, success_handler);
  WaitPendingDone(raylet_clients_[0]->lease_callbacks, 1);
  ASSERT_TRUE(raylet_clients_[0]->GrantPrepareBundleResources());
...
TEST_F(GcsPlacementGroupSchedulerTest, TestNodeDeadDuringRescheduling) {
  ...
  auto bundles_on_node0 = scheduler_->GetBundlesOnNode(
      NodeID::FromBinary(node0->basic_gcs_node_info().node_id()));
  ASSERT_EQ(1, bundles_on_node0.size());
  auto bundles_on_node1 = scheduler_->GetBundlesOnNode(
      NodeID::FromBinary(node1->basic_gcs_node_info().node_id()));
...
TEST_F(GcsPlacementGroupSchedulerTest, TestWarnPlacementGroupSchedulingFailure) {
  auto create_placement_group_request = Mocker::GenCreatePlacementGroupRequest();
  auto placement_group =
      std::make_shared<gcs::GcsPlacementGroup>(create_placement_group_request, "");
  std::unordered_map<std::string, double> resource_map = {{"CPU", 5.0}, {"memory", 5.0}};
  auto bundles = placement_group->GetUnplacedBundles();
  absl::flat_hash_map<NodeID, std::shared_ptr<SchedulingResources>> available_resource = {
      {NodeID::FromRandom(), std::make_shared<SchedulingResources>(resource_map)},
      {NodeID::FromRandom(), std::make_shared<SchedulingResources>(resource_map)},
      {NodeID::FromRandom(), std::make_shared<SchedulingResources>(resource_map)}};
  scheduler_->WarnPlacementGroupSchedulingFailure(placement_group, available_resource);
}

namespace gcs {
TEST_F(GcsPlacementGroupSchedulerTest, TestResourceLeak) {
  AddNode(Mocker::GenNodeInfo(0));
  AddNode(Mocker::GenNodeInfo(1));
  auto failure_handler = [this](std::shared_ptr<gcs::GcsPlacementGroup> placement_group) {
    absl::MutexLock lock(&placement_group_requests_mutex_);
  ...
  auto request = Mocker::GenCreatePlacementGroupRequest(
      /*name=*/"", /*strategy=*/rpc::PlacementStrategy::STRICT_PACK, /*bundles_count=*/1);
  auto placement_group = std::make_shared<gcs::GcsPlacementGroup>(request, "");
  scheduler_->ScheduleUnplacedBundles(placement_group, failure_handler, success_handler);

  int node_index = !raylet_clients_[0]->lease_callbacks.empty() ? 0 : 1;
  auto raylet_client = raylet_clients_[node_index];
  ASSERT_TRUE(raylet_client->GrantPrepareBundleResources());

  WaitPendingDone(raylet_client->commit_callbacks, 1);
  ASSERT_TRUE(raylet_client->GrantCommitBundleResources(/*success=*/false));
  WaitPendingDone(raylet_client->commit_callbacks, 0);

  auto bundles = placement_group->GetUnplacedBundles();
  ASSERT_EQ(bundles.size(), 1);
  ASSERT_EQ(bundles[0]->NodeId(), NodeID::Nil());

  auto bundle_locations = scheduler_->committed_bundle_location_index_.GetBundleLocations(
      placement_group->GetPlacementGroupID());
  ASSERT_FALSE(bundle_locations.has_value());

  scheduler_->ScheduleUnplacedBundles(placement_group, failure_handler, success_handler);
  node_index = !raylet_clients_[0]->lease_callbacks.empty() ? 0 : 1;
  raylet_client = raylet_clients_[node_index];
  ASSERT_TRUE(raylet_client->GrantPrepareBundleResources());

  WaitPendingDone(raylet_client->commit_callbacks, 1);
  ASSERT_TRUE(raylet_client->GrantCommitBundleResources());
  WaitPlacementGroupPendingDone(1, GcsPlacementGroupStatus::SUCCESS);

  auto bundle_locations_2 =
      scheduler_->committed_bundle_location_index_.GetBundleLocations(
          placement_group->GetPlacementGroupID());
  ASSERT_TRUE(bundle_locations_2.has_value() && (*bundle_locations_2)->size() == 1);
}
}  // namespace gcs
}  // namespace ray
