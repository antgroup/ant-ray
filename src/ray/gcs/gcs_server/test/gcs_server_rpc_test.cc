...
class GcsServerTest : public ::testing::Test {
 public:
  ...
  bool NotifyDriverExit(const rpc::NotifyDriverExitRequest &request) {
    std::promise<bool> promise;
    client_->NotifyDriverExit(
        request,
        [&promise](const Status &status, const rpc::NotifyDriverExitReply &reply) {
          RAY_CHECK_OK(status);
          promise.set_value(true);
        });
    return WaitReady(promise.get_future(), timeout_ms_);
  ...
  std::vector<rpc::NodeInfo> GetJobDistribution() {
    rpc::GetJobDistributionRequest request;
    std::vector<rpc::NodeInfo> node_infos;
    std::promise<bool> promise;
    client_->GetJobDistribution(
        request, [&node_infos, &promise](const Status &status,
                                         const rpc::GetJobDistributionReply &reply) {
          RAY_CHECK_OK(status);
          for (int index = 0; index < reply.node_info_list_size(); ++index) {
            node_infos.push_back(reply.node_info_list(index));
          }
          promise.set_value(true);
        });
    EXPECT_TRUE(WaitReady(promise.get_future(), timeout_ms_));
  ...
  std::vector<rpc::GcsNodeInfo> GetAllNodeInfo() {
    ...
    rpc::GetAllFullNodeInfoRequest request;
    std::promise<bool> promise;
    client_->GetAllFullNodeInfo(
        request, [&node_info_list, &promise](const Status &status,
                                             const rpc::GetAllFullNodeInfoReply &reply) {
          RAY_CHECK_OK(status);
          for (int index = 0; index < reply.node_info_list_size(); ++index) {
...
TEST_F(GcsServerTest, TestJobDistribution) {
  std::vector<rpc::NodeInfo> node_info = GetJobDistribution();
  ASSERT_TRUE(node_info.size() == 0);
}
...
TEST_F(GcsServerTest, TestJobInfo) {
  ...
  // Notify driver exit and mark job finished
  rpc::NotifyDriverExitRequest notify_driver_exit_request;
  notify_driver_exit_request.set_job_id(job_table_data->job_id());
  ASSERT_TRUE(NotifyDriverExit(notify_driver_exit_request));
}
...
TEST_F(GcsServerTest, TestNodeInfo) {
  ...
  ASSERT_TRUE(node_info_list[0].basic_gcs_node_info().state() ==
              rpc::BasicGcsNodeInfo::ALIVE);

  // Report heartbeat
  rpc::ReportHeartbeatRequest report_heartbeat_request;
  report_heartbeat_request.mutable_heartbeat()->set_node_id(
      gcs_node_info->basic_gcs_node_info().node_id());
  ASSERT_TRUE(ReportHeartbeat(report_heartbeat_request));

  // Update node resources
  rpc::UpdateResourcesRequest update_resources_request;
  update_resources_request.set_node_id(gcs_node_info->basic_gcs_node_info().node_id());
