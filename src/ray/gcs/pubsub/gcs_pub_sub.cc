    ...
    auto callback = [this, channel_key, command_done_callback, command_subscribe_callback,
      ...
      if (reply->IsUnsubscribeCallback()) {
        ...
        bool need_callback = true;
        {
          ...
          auto channel = channels_.find(channel_key);
          if (channel == channels_.end()) {
            RAY_LOG(INFO) << "Duplicated subscribe notification is received.";
            return;
          }
          if (!channel->second.pending_reply) {
            RAY_LOG(INFO)
                << "The connection is reestablished, no need to call callback function.";
            need_callback = false;
          }
        ...
        if (need_callback && command_done_callback) {
