...
Status InMemoryStoreClient::AsyncPut(const std::string &table_name,
  ...
  Post([callback]() { callback(Status::OK()); }, "GcsInMemoryStore.Put");
  return Status::OK();
}
...
Status InMemoryStoreClient::AsyncPutWithIndex(const std::string &table_name,
  ...
  Post([callback]() { callback(Status::OK()); }, "GcsInMemoryStore.PutWithIndex");
  return Status::OK();
}
...
Status InMemoryStoreClient::AsyncGet(const std::string &table_name,
  ...
  if (iter != table->records_.end()) {
    ...
    Post([callback, data]() { callback(Status::OK(), data); }, "GcsInMemoryStore.Get");
  } else {
    Post([callback]() { callback(Status::OK(), boost::none); },
...
Status InMemoryStoreClient::AsyncGetAll(
  ...
  Post([result, callback]() { callback(std::move(*result)); }, "GcsInMemoryStore.GetAll");
  return Status::OK();
}
...
Status InMemoryStoreClient::AsyncBatchDelete(const std::string &table_name,
  ...
  Post([callback]() { callback(Status::OK()); }, "GcsInMemoryStore.BatchDelete");
  return Status::OK();
}
...
Status InMemoryStoreClient::AsyncGetByIndex(
  ...
  Post([result, callback]() { callback(std::move(*result)); },
...
Status InMemoryStoreClient::AsyncDeleteByPattern(const std::string &table_name,
                                                 const std::string &pattern,
                                                 const ItemCallback<int> &callback) {
  return Status::NotImplemented(
      "AsyncDeleteByPattern is not supported for InMemoryStoreClient.");
}
...
void InMemoryStoreClient::Post(std::function<void()> task, std::string name) {
  RAY_CHECK(task != nullptr);
  main_io_service_.post(task, name);
}
