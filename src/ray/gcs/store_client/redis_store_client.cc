...
Status RedisStoreClient::AsyncDeleteByPattern(const std::string &table_name,
                                              const std::string &pattern,
                                              const ItemCallback<int> &callback) {
  auto scanner = std::make_shared<RedisScanner>(redis_client_, table_name);
  auto on_done = [this, callback, scanner, table_name](
                     const Status &status, const std::vector<std::string> &result) {
    if (!result.empty()) {
      int count = result.size();
      RAY_CHECK_OK(DeleteByKeys(result, [count, callback](Status status) {
        RAY_CHECK_OK(status);
        if (callback) {
          callback(count);
        }
  ...
  return scanner->ScanKeys(pattern, on_done);
}
