...
Status PlasmaClient::Impl::Release(const ObjectID &object_id) {
  RAY_LOG(DEBUG) << "called plasma_release on conn " << store_conn_ << " with object id "
                 << object_id;
  std::lock_guard<std::recursive_mutex> guard(client_mutex_);
...
Status PlasmaClient::Impl::Seal(const ObjectID &object_id) {
  RAY_LOG(DEBUG) << "called plasma_seal on conn " << store_conn_ << " with object id "
                 << object_id;
  std::lock_guard<std::recursive_mutex> guard(client_mutex_);
...
Status PlasmaClient::Impl::Abort(const ObjectID &object_id) {
  RAY_LOG(DEBUG) << "called plasma_abort on conn " << store_conn_ << " with object id "
                 << object_id;
  std::lock_guard<std::recursive_mutex> guard(client_mutex_);
  auto object_entry = objects_in_use_.find(object_id);
  RAY_CHECK(object_entry != objects_in_use_.end())
...
Status PlasmaClient::Impl::Delete(const std::vector<ObjectID> &object_ids) {
  if (RAY_LOG_ENABLED(DEBUG)) {
    ...
    oss << "called plasma_release on conn " << store_conn_ << " with object ids ";
...
Status PlasmaClient::Impl::Evict(int64_t num_bytes, int64_t &num_bytes_evicted) {
  RAY_LOG(DEBUG) << "called plasma_evict on conn " << store_conn_ << " with num_bytes "
                 << num_bytes;
  std::lock_guard<std::recursive_mutex> guard(client_mutex_);
