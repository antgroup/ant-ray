...
class PlasmaStore {
 ...
 private:
  ...
  /// Return true if the given entry has only one reference.
  /// Only one reference means there's only a raylet that pins the object
  /// so it is safe to spill the object.
  bool IsObjectSpillableInternal(ObjectTableEntry *entry);
  ...
  std::unique_ptr<plasma::PlasmaStats> plasma_stats_;
