...
class MockServer {
 public:
  MockServer(instrumented_io_context &main_service,
  ...
  ~MockServer() { RAY_CHECK_OK(gcs_client_->FullNodeInfo().UnregisterSelf()); }

 private:
  ray::Status RegisterGcs(instrumented_io_context &io_service) {
    auto object_manager_port = object_manager_.GetServerPort();
    GcsNodeInfo node_info;
    node_info.set_node_id(node_id_.Binary());
    node_info.mutable_basic_gcs_node_info()->set_node_manager_address("127.0.0.1");
    node_info.mutable_basic_gcs_node_info()->set_node_manager_port(object_manager_port);
    node_info.mutable_basic_gcs_node_info()->set_object_manager_port(object_manager_port);

    ray::Status status = gcs_client_->FullNodeInfo().RegisterSelf(node_info, nullptr);
...
class StressTestObjectManager : public TestObjectManagerBase {
 public:
  ...
  void WaitConnections() {
    node_id_1 = gcs_client_1->FullNodeInfo().GetSelfId();
    node_id_2 = gcs_client_2->FullNodeInfo().GetSelfId();
    RAY_CHECK_OK(gcs_client_1->FullNodeInfo().AsyncSubscribeToNodeChange(
        ...
        nullptr));
    RAY_CHECK_OK(gcs_client_2->FullNodeInfo().AsyncSubscribeToNodeChange(
        [this](const NodeID &node_id, const GcsNodeInfo &data) {
          if (node_id == node_id_1 || node_id == node_id_2) {
            num_connected_clients += 1;
          }
  ...
  void TransferTestExecute(int num_trials, int64_t data_size,
    ...
    NodeID node_id_1 = gcs_client_1->FullNodeInfo().GetSelfId();
    NodeID node_id_2 = gcs_client_2->FullNodeInfo().GetSelfId();
  ...
  void TestConnections() {
    ...
    RAY_LOG(DEBUG) << "\n"
                   << "All connected nodes:"
                   << "\n";
    auto data = gcs_client_1->FullNodeInfo().Get(node_id_1);
    RAY_LOG(DEBUG) << "NodeID="
                   << NodeID::FromBinary(data->basic_gcs_node_info().node_id()) << "\n"
                   << "NodeIp=" << data->basic_gcs_node_info().node_manager_address()
                   << "\n"
                   << "NodePort=" << data->basic_gcs_node_info().node_manager_port();
    auto data2 = gcs_client_1->FullNodeInfo().Get(node_id_2);
    RAY_LOG(DEBUG) << "NodeID="
                   << NodeID::FromBinary(data2->basic_gcs_node_info().node_id()) << "\n"
                   << "NodeIp=" << data2->basic_gcs_node_info().node_manager_address()
                   << "\n"
                   << "NodePort=" << data2->basic_gcs_node_info().node_manager_port();
  }
};

TEST_F(StressTestObjectManager, StartStressTestObjectManager) {
  std::thread loop_thread([&]() {
    RAY_LOG(INFO) << "main_service run";
    main_service.run();
  });
  boost::asio::deadline_timer t(main_service, boost::posix_time::seconds(10));
  t.async_wait([this](const boost::system::error_code & /*e*/) { WaitConnections(); });
  loop_thread.join();
}
