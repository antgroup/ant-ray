...
class LocalObjectManager {
 public:
  ...
  /// Dumping objects to high-available external storage
  ///
  /// \param object_ids Object IDs to dump
  /// \param object_sizes Size in bytes of each object, must be in the same order with
  /// object_ids
  /// \param owner_address the owner_address of dumping objects
  /// \param callback Called after dumped all objects or error occurred
  void DumpObjects(const std::vector<ObjectID> &object_ids,
                   const std::vector<uint64_t> &object_sizes,
                   const rpc::Address &owner_address,
                   std::function<void(const ray::Status &)> callback);

  /// Restore a dumped HA object from external storage back into local memory.
  /// Note: This is no-op if the same load request is in flight or the requested
  /// object wasn't dumped yet. The caller should ensure to retry object restoration in
  /// this case.
  ///
  /// \param object_id The ID of the object to restore.
  /// \param object_url The URL where the object is dumped.
  /// \param callback A callback to call when the load is done.
  /// Status will contain the error during load process, if any.
  void LoadDumpedObject(const ObjectID &object_id, const std::string &object_url,
                        std::function<void(const ray::Status &)> callback);
  ...
  /// Return the dumped object URL or the empty string.
  std::string GetDumpedObjectURL(const ObjectID &object_id);
 ...
 private:
  ...
  /// Add objects' dumped URLs to the global object directory. Call the
  /// callback once all URLs have been added.
  void AddDumpedUrls(const std::vector<ObjectID> &object_ids,
                     const rpc::DumpObjectsReply &worker_reply,
                     std::function<void(const ray::Status &)> callback);

  /// Called after get the reply of sending `AddDumpedUrlRequest`.
  /// \param object_id The dumped object's id
  /// \param object_url dump url with offset, can be used to retreive the object
  /// \param num_remaining The number of dumped objects left to add their urls.
  /// Since it will be modified inside each invocation, this should be shared_ptr.
  /// \param callback Called after adding ALL dumped objects' url
  /// \param status Reply status of the request
  void AddDumpedUrlsCallback(const ObjectID &object_id, const std::string &object_url,
                             std::shared_ptr<size_t> num_remaining,
                             std::function<void(const ray::Status &)> callback,
                             ray::Status status);
  ...
  // Objects that were pinned on this node but that are being dumped.
  // These objects will be released once dumping is complete and the URL is
  // written to the object directory.
  absl::flat_hash_map<ObjectID, std::pair<int, rpc::Address>> objects_pending_dump_;

  /// Objects that were dumped on this node but that are being loaded.
  /// The field is used to dedup the same load request while loading is in
  /// progress.
  ...
  /// The total size of the objects that are currently being
  /// dumped from this node, in bytes.
  size_t num_bytes_pending_dump_;

  /// Mapping from object id to url_with_offsets. We cannot reuse pinned_objects_ because
  /// pinned_objects_ entries are deleted when dumping happens.
  absl::flat_hash_map<ObjectID, std::string> dumped_objects_url_;
