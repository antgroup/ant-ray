...
void ReplyCancelled(ray::rpc::RequestWorkerLeaseReply *reply,
                    ray::rpc::SendReplyCallback send_reply_callback,
                    ray::rpc::RequestWorkerLeaseReply::SchedulingFailureType failure_type,
...
void HeartbeatSender::Heartbeat() {
  ...
  RAY_CHECK_OK(
      gcs_client_->Nodes().AsyncReportHeartbeat(heartbeat_data, [](Status status) {
        RAY_LOG(DEBUG) << "Heartbeat return status=" << status;
        if (status.IsDisconnected()) {
          RAY_LOG(FATAL) << "This node has beem marked as dead.";
        } else if (status.IsExitNode()) {
          std::string message = "Raylet intentionally exits to release resources.";
          RAY_LOG(WARNING) << message;
          RAY_EVENT(INFO, EVENT_LABEL_RELEASE_NODE) << message;
          _exit(kExitNodeCode);
        } else if (status.IsExitAndReplaceNode()) {
          std::string message = "Raylet intentionally exits to replace a new one.";
          RAY_LOG(WARNING) << message;
          RAY_EVENT(INFO, EVENT_LABEL_PROCESS_EXIT) << message;
          _exit(kExitAndReplaceNodeCode);
        } else if (status.IsExitAndRestartNode()) {
          std::string message = "Raylet intentionally exits to restart.";
          RAY_LOG(WARNING) << message;
          RAY_EVENT(INFO, EVENT_LABEL_PROCESS_EXIT) << message;
          _exit(kExitAndRestartNodeCode);
        }
      }));
}
...
NodeManager::NodeManager(instrumented_io_context &io_service, const NodeID &self_node_id,
    ...
    : self_node_id_(self_node_id),
      ...
      actor_worker_assignment_manager_(std::make_shared<ActorWorkerAssignmentManager>(
          io_service, RayConfig::instance().gcs_task_scheduling_enabled())),
      worker_pool_(io_service, self_node_id_, config.node_manager_address,
                   ...
                   config.per_job_python_env_path, config.job_dir_template,
                   actor_worker_assignment_manager_, config.log_dir),
      ...
      client_call_manager_(io_service, /*num_threads=*/1,
      ...
      // === ANT-INTERNAL below ===
      timer_interval_(boost::posix_time::milliseconds(1000)),
      pending_tasks_reschedule_timer_(io_service),
      memory_monitor_(std::make_unique<MemoryMonitor>(
          io_service, RayConfig::instance().memory_usage_threshold(),
  ...
  cluster_resource_scheduler_ =
      std::shared_ptr<ClusterResourceScheduler>(new ClusterResourceScheduler(
          ...
          *gcs_client_, [this]() { return object_manager_.GetUsedMemory(); },
          ...
          [this](auto &binary_node_id, auto &schedule_context) {
            return this->IsNodeSchedulable(binary_node_id, schedule_context);
          },
          /*is_overcommit_enabled_fn=*/
          [this]() { return this->overcommit_enabled_; }));
  ...
  cluster_task_manager_ = std::make_shared<ClusterTaskManager>(
      ...
      max_task_args_memory, actor_worker_assignment_manager_);
  placement_group_resource_manager_ = std::make_shared<NewPlacementGroupResourceManager>(
      ...
      [this](const ray::gcs::NodeResourceInfoAccessor::ResourceMap &resources) {
        // NOTE(loushang.ls): In gcs scheduling, we will update the resource view in the
        // GCS process directly, and if we do the same operation here, it will make the
        // resource view conflict.
  ...
  timer_callback_ = [this](const boost::system::error_code &error) {
    if (error != boost::asio::error::operation_aborted) {
      ray::stats::DependencyLocalObjectCount().Record(
          dependency_manager_.GetLocalObjectsCount());

      timer_->expires_from_now(timer_interval_);
      timer_->async_wait(timer_callback_);
    } else {
      RAY_LOG(INFO) << "timer abort";
    }
  };
  ...
  agent_manager_.reset(new AgentManager(
      ...
      [this](const std::string &ip_address, int port) {
        if (ip_address.empty() || port == 0) {
          return std::shared_ptr<rpc::JobClientInterface>(nullptr);
        }
        return std::shared_ptr<rpc::JobClientInterface>(
            new rpc::JobClient(ip_address, port, client_call_manager_));
      },
      /*event_client_factory=*/
      [this](const std::string &ip_address, int port) {
        if (ip_address.empty() || port == 0) {
          return std::shared_ptr<rpc::EventClientInterface>(nullptr);
        }
        return std::shared_ptr<rpc::EventClientInterface>(
            new rpc::EventClient(ip_address, port, client_call_manager_));
      },
      /*reporter_client_factory=*/
      [this](const std::string &ip_address, int port) {
        if (ip_address.empty() || port == 0) {
          return std::shared_ptr<rpc::ReporterClientInterface>(nullptr);
        }
        return std::shared_ptr<rpc::ReporterClientInterface>(
            new rpc::ReporterClient(ip_address, port, client_call_manager_));
      },
      /*delay_executor=*/
      ...
      [this](const JobID &job_id, const Status &status) {
        OnJobEnvInitialized(job_id, status);
      },
      job_manager_, std::make_unique<PeriodicalRunner>(io_service_),
      /*fill_workers_info=*/
      [this](rpc::GetWorkersInfoReply *reply) {
        ...
        for (const auto &worker : workers) {
          if (worker->GetWorkerType() == rpc::WorkerType::WORKER) {
            auto job_data = job_manager_->GetJobData(worker->GetAssignedJobId());
            auto mutable_worker_info = reply->mutable_worker_info_list()->Add();
            if (job_data) {
              mutable_worker_info->set_job_name(job_data->job_name());
            }
      ...
      [this](const rpc::ReportLocalRuntimeResourcesRequest &request) {
        io_service_.post(
            [this, request]() {
              absl::flat_hash_map<int, ResourceSet> worker_runtime_resources;
              for (const auto &worker : request.worker_stat_list()) {
                auto &worker_resource_set = worker_runtime_resources[worker.pid()];
                worker_resource_set.AddOrUpdateResource(
                    kMemory_ResourceLabel, FractionalResourceQuantity(ToMemoryUnits(
                                               double(worker.memory_tail()))));
                worker_resource_set.AddOrUpdateResource(
                    kCPU_ResourceLabel,
  ...
  agent_manager_->StartAgent();
}

ray::Status NodeManager::RegisterGcs() {
  // subscribe frozen node change
  RAY_UNUSED(gcs_client_->FrozenNodes().AsyncSubscribeFrozenNode(
      [this](const auto &frozen_node_set) { this->OnFrozenNodeUpdated(frozen_node_set); },
      [](Status status) { RAY_LOG(INFO) << "AsyncSubscribeFrozenNode done."; }));
  ...
  auto on_node_change = [this](const NodeID &node_id, const BasicGcsNodeInfo &data) {
    if (data.state() == rpc::BasicGcsNodeInfo::ALIVE) {
      NodeAdded(data);
    } else {
      RAY_CHECK(data.state() == rpc::BasicGcsNodeInfo::DEAD);
      NodeRemoved(node_id);
    }
  };
  ...
  RAY_RETURN_NOT_OK(gcs_client_->Nodes().AsyncSubscribeInternalConfig(
      [](const std::string &config_json_str) {
        RAY_LOG(INFO) << "Received update internal config notification, config_json_str="
                      << config_json_str;
        RayConfig::instance().update(config_json_str);
      },
      [](Status status) {
        RAY_CHECK_OK(status);
        RAY_LOG(INFO) << "AsyncSubscribeInternalConfig done.";
      }));
  ...
  // Subscribe to all nodegroup update notification.
  const auto nodegroup_update_notification_handler =
      [this](const NodegroupID &nodegroup_id, const rpc::NodegroupData &nodegroup_data) {
        NodegroupUpdated(nodegroup_data);
      };
  RAY_RETURN_NOT_OK(gcs_client_->Nodegroups().AsyncSubscribeAll(
      nodegroup_update_notification_handler, [](const ray::Status &status) {
        RAY_CHECK_OK(status);
        RAY_LOG(INFO) << "Finished subscribing all nodegroups infos.";
      }));
  ...
  // subscribe to placement group bundles changed event.
  // Why we need this? Image the following steps.
  // 1. Worker submit remove bundle index request.
  // 2. GCS publish bundles removed message but delayed.
  // 3. Raylet receives cancel resource request from GCS.
  // 4. Another worker submits tasks with the removed bundle.
  // 5. Raylet receives these tasks and queued.
  // 6. Worker and Raylet reveive the publish message.
  const auto bundles_changed_notification_handler =
      [this](const PlacementGroupID &updated_placement_group_id,
             const rpc::PlacementGroupBundlesChangedNotification &notification) {
        OnPlacementGroupBundlesChanged(notification);
      };
  RAY_CHECK_OK(gcs_client_->PlacementGroups().AsyncSubscribeAllBundlesChangedEvent(
      bundles_changed_notification_handler, [](const ray::Status &status) {
        RAY_CHECK_OK(status);
        RAY_LOG(INFO)
            << "Finished subscribing the change of bundles for all placement groups";
      }));
...
void NodeManager::HandleJobSubmitted(const JobID &job_id, const JobTableData &job_data) {
  ...
  if ((job_data.pre_initialize_job_runtime_env_enabled() ||
       job_data.raylet_id() == self_node_id_.Binary()) &&
      IsHostInNodegroup(initial_config_.node_manager_hostname, job_data.nodegroup_id())) {
    // If the nodegroup of the node is match with the job's nodegroup then int the job
    // env.
...
void NodeManager::HandleJobStarted(const JobID &job_id, const JobTableData &job_data) {
  ...
  if (job_id.IsSubmittedFromDashboard() &&
      job_data.pre_initialize_job_runtime_env_enabled() &&
      IsHostInNodegroup(initial_config_.node_manager_hostname, job_data.nodegroup_id())) {
    // Maybe this is a new node and missed the `JobSubmitted` envent. So if the
    // nodegroup of the node is match with the job's nodegroup we need to initialize the
    // job env.
    InitializeJobEnv(job_data);
  }
...
void NodeManager::HandleJobFinished(const JobID &job_id, const JobTableData &job_data) {
  ...
  if (!job_manager_->OnJobFailedOrFinished(job_id)) {
    RAY_LOG(INFO) << "Job " << job_id
                  << " is already failed or finished, just try to clean up runtime "
                     "environment to avoid disk leaks.";
    // Clean job env.
    agent_manager_->ClearJobEnv(job_data);
  ...
  cluster_task_manager_->CancelAndRemoveTasksForJob(job_id);
}

void NodeManager::FillResourceReport(rpc::ResourcesData &resources_data) {
  uint64_t now_ms = current_time_ms();
  auto need_whole_report = false;
  if (now_ms - last_whole_resource_usage_report_at_ms_ >=
      RayConfig::instance().raylet_whole_report_resources_period_milliseconds()) {
    need_whole_report = true;
    last_whole_resource_usage_report_at_ms_ = now_ms;
  }
  ...
  cluster_resource_scheduler_->FillResourceUsage(resources_data, need_whole_report);
  cluster_task_manager_->FillResourceUsage(
      resources_data, gcs_client_->NodeResources().GetLastResourceUsage(),
      need_whole_report);
  FillNormalTaskResourceUsage(resources_data, need_whole_report);
...
void NodeManager::ReportResourceUsage() {
  ...
  if (resources_data->resources_total_size() > 0 ||
    ...
    RAY_LOG(DEBUG) << "ReportResourceUsage, resources_total_size="
                   << resources_data->resources_total_size()
                   << ", resources_available_changed="
                   << resources_data->resources_available_changed()
                   << ", resource_load_changed="
                   << resources_data->resource_load_changed()
                   << ", should_global_gc=" << resources_data->should_global_gc()
                   << ", resources_normal_task_changed="
                   << resources_data->resources_normal_task_changed();
...
void NodeManager::WarnResourceDeadlock() {
  ...
  if (any_pending && resource_deadlock_warned_++ > 0) {
    ...
    // ANT-INTERNAL: the following TriggerGlobalGC() will trigger all workers to
    // perform a GC every local_gc_min_interval_ns_ (10s), when some tasks cannot be
    // scheduled. This can cause perf issues to some applications (espacially Java).
    // TriggerGlobalGC();
...
void NodeManager::NodeAdded(const BasicGcsNodeInfo &node_info) {
  ...
  remote_node_info_[node_id] = std::make_shared<BasicGcsNodeInfo>(node_info);
  ...
  RAY_CHECK_OK(gcs_client_->NodeResources().AsyncGetResources(
      ...
      [this, node_id](
        ...
        if (data) {
          ...
          if (ResourceCreateUpdated(node_id, resource_set)) {
            AsyncScheduleAndDispatchTasks();
          }
        }
      }));
...
void NodeManager::NodeRemoved(const NodeID &node_id) {
  ...
  const auto node_entry = remote_node_info_.find(node_id);
  if (node_entry != remote_node_info_.end()) {
    remote_node_info_.erase(node_entry);
  }
...
void NodeManager::HandleUnexpectedWorkerFailure(const rpc::WorkerDeltaData &data) {
  ...
  for (const auto &pair : leased_workers_) {
    ...
    if (!worker->IsDetachedActor()) {
      // TODO (Alex): Cancel all pending child tasks of the tasks whose owners have
      // failed because the owner could've submitted lease requests before failing.
      if (!worker_id.IsNil()) {
...
void NodeManager::ResourceUsageBatchReceived(
  ...
  if (updated) {
    // If light resource usage report enabled, we update remote resources only when
    // related
...
void NodeManager::ProcessClientMessage(const std::shared_ptr<ClientConnection> &client,
  ...
  RAY_LOG(DEBUG) << "[NodeManager] Receive message "
...
void NodeManager::ProcessRegisterClientRequestMessage(
  ...
  auto send_reply_callback = [this, client, job_id](Status status, int assigned_port) {
    ...
    auto reply = ray::protocol::CreateRegisterClientReply(
        ...
        fbb.CreateString(initial_config_.node_name));
    fbb.Finish(reply);
  ...
  if (worker_type == rpc::WorkerType::WORKER ||
    ...
    RAY_LOG(INFO) << "ProcessRegisterClientRequestMessage 2";
    // Register the new driver.
    RAY_CHECK(pid >= 0);
    const JobID job_id = from_flatbuf<JobID>(*message->job_id());
    if (worker_pool_.IsJobFinished(job_id)) {
      std::ostringstream ostr;
      ostr << "Job " << job_id << " is already finished.";
    ...
    if (job_id.IsSubmittedFromDashboard()) {
      ...
      job_data_ptr->set_timestamp(current_sys_time_seconds());
      *job_data_ptr->mutable_config() = job_config;
    } else {
      job_data_ptr = gcs::CreateJobTableData(
          job_id, /*is_dead*/ false, current_sys_time_seconds(), worker_ip_address, pid,
          boost::asio::ip::host_name(), language, self_node_id_, job_config);
    }

    Status status = worker_pool_.RegisterDriver(
        worker, job_config,
        [this, send_reply_callback, job_data_ptr](Status status, int assigned_port) {
          if (!status.ok()) {
          ...
          RAY_CHECK_OK(gcs_client_->Jobs().AsyncAdd(
              job_data_ptr, [send_reply_callback, assigned_port](Status status) {
                if (status.ok()) {
...
void NodeManager::ProcessAnnounceWorkerPortMessage(
  ...
  if (worker->IsForActor() &&
      !actor_worker_assignment_manager_->IsValid(worker->GetShimProcess())) {
    // This is a leaked worker, just kill it.
    RAY_LOG(WARNING) << "The worker " << worker->WorkerId() << " with process id "
                     << worker->GetProcess().GetId()
                     << " is a leaked worker, which should be killed and mark as dead.";
    worker->MarkDead();
    worker_pool_.KillWorker(worker);
    return;
  }
...
void NodeManager::DisconnectClient(
  ...
  if (is_worker && worker->IsDead() && !worker->ActorNeedsRestart()) {
  ...
  auto worker_failure_data_ptr = gcs::CreateWorkerFailureData(
      ...
      time(nullptr), disconnect_type, worker->GetAssignedJobId(),
      worker->GetAssignmentId(), creation_task_exception);
  RAY_CHECK_OK(
  ...
  if (is_worker) {
    ...
    if ((!task_id.IsNil() || !actor_id.IsNil()) && !worker->IsDead()) {
      ...
      if (disconnect_type == rpc::WorkerExitType::SYSTEM_ERROR_EXIT) {
        const auto is_job_finished =
            worker_pool_.IsJobFinished(worker->GetAssignedJobId());
        const auto actor_needs_restart = worker->ActorNeedsRestart();
        RAY_LOG(INFO) << "Worker " << worker->WorkerId()
                      << " failed. is_job_finished=" << is_job_finished
                      << ", actor_needs_restart=" << actor_needs_restart;

        if (!is_job_finished && !actor_needs_restart) {
          RAY_EVENT(ERROR, "WorkerFailure")
                  .WithField("job_id", worker->GetAssignedJobId().Hex())
              << "Worker " << worker->WorkerId()
              << " died. Address: " << worker->IpAddress() << ":" << worker->Port()
              << ", Pid: " << worker->GetProcess().GetId()
              << ", JobId: " << worker->GetAssignedJobId()
              << ", ActorId: " << worker->GetActorId()
              << ", TaskId: " << worker->GetAssignedTaskId();
          if (worker->GetLanguage() == ray::Language::JAVA) {
            agent_manager_->CheckJavaHsErrLog(
                GetInitialConfig().log_dir, worker->IpAddress(),
                worker->GetProcess().GetId(), false, worker->GetAssignedJobId(),
                worker->WorkerId(), worker->GetActorId(), worker->GetAssignedTaskId());
          }
        }
    ...
    // Cancel all tasks in the specified worker process that are leasing workers.
    auto canceled_actor_creation_tasks =
        actor_worker_assignment_manager_->CancelActorsInProcess(
            worker->GetProcess(), /*mark_assignment_as_invalid=*/true);
    for (auto &task_id : canceled_actor_creation_tasks) {
      cluster_task_manager_->CancelTask(
          task_id, rpc::RequestWorkerLeaseReply::SCHEDULING_FAILED,
          "The worker process that the actor associated is disconnected.");
    }
    ...
    RAY_CHECK(!job_id.IsNil());
    bool exit_with_error = (disconnect_type != rpc::WorkerExitType::INTENDED_EXIT);
    if (!exit_with_error) {
      RAY_EVENT(INFO, EVENT_LABEL_JOB_PIPELINE_DRIVER_EXIT)
              .WithField("job_id", job_id.Hex())
          << "Driver intentional disconnected.";
    } else {
      RAY_EVENT(ERROR, EVENT_LABEL_JOB_PIPELINE_DRIVER_EXIT)
              .WithField("job_id", job_id.Hex())
      ...
      if (worker->GetLanguage() == ray::Language::JAVA) {
        agent_manager_->CheckJavaHsErrLog(
            GetInitialConfig().log_dir, worker->IpAddress(), worker->GetProcess().GetId(),
            true, worker->GetAssignedJobId(), worker->WorkerId(), worker->GetActorId(),
            worker->GetAssignedTaskId());
      }
    }
    RAY_CHECK_OK(
        gcs_client_->Jobs().AsyncNotifyDriverExit(job_id, exit_with_error, nullptr));
...
void NodeManager::ProcessPushErrorRequestMessage(const uint8_t *message_data) {
  ...
  RAY_LOG(DEBUG) << "ProcessPushErrorRequestMessage, error_message=" << error_message
                 << ", type=" << type;
...
void NodeManager::HandleRequestWorkerLease(const rpc::RequestWorkerLeaseRequest &request,
                                           ...
                                           rpc::SendReplyCallback send_reply_callback) {
  /// An overloaded function for the HandleRequestWorkerLease above with an additional
  /// params. We use an overloadded function so we don't need to change the base
  /// interface.
  HandleRequestWorkerLease(request, reply, send_reply_callback,
                           /*waiting_for_job_env_initialized=*/true);
}

void NodeManager::HandleRequestWorkerLease(const rpc::RequestWorkerLeaseRequest &request,
                                           rpc::RequestWorkerLeaseReply *reply,
                                           rpc::SendReplyCallback send_reply_callback,
                                           bool waiting_for_job_env_initialized) {
  ...
  // Actor's job is not running. Let gcs retry.
  if (!worker_pool_.IsJobRunning(task_id.JobId())) {
    std::string scheduling_failure_message =
        "Env of job " + task_id.JobId().Hex() + " is not ready.";
    RAY_LOG(INFO) << scheduling_failure_message << " task_id = " << task_id;
    ReplyCancelled(reply, send_reply_callback,
                   rpc::RequestWorkerLeaseReply::SCHEDULING_FAILED,
                   scheduling_failure_message);
    return;
  }

  // Actor's job is forbidden to submit new tasks. Let gcs retry.
  if (l1_handler.IsJobBanningNewTasks(task_id.JobId())) {
    std::string scheduling_failure_message =
        "Job " + task_id.JobId().Hex() + " is forbidden to submit new tasks.";
    RAY_LOG(INFO) << scheduling_failure_message << " task_id = " << task_id;
    ReplyCancelled(reply, send_reply_callback,
                   rpc::RequestWorkerLeaseReply::SCHEDULING_FAILED,
                   scheduling_failure_message);
  ...
  // Return invalid directly if the request resource in this task contains the invalid
  // bundles of the placement group. Why we need this? Image the following steps. Place
  // NOTE don't be confused with the comment from the `RegisterGcs` method.
  // 1. Worker submit remove bundle index request.
  // 2. GCS publish bundles removed message but delayed.
  // 3. Raylet receives cancel resource request from GCS.
  // 4. Another worker submits tasks with the removed bundle.
  // 5. Worker and Raylet reveive the publish message.
  // 6. Raylet receives these tasks and queued.
  const auto &required_resources = task.GetTaskSpecification().GetRequiredResources();
  for (const auto &group_invalid_bundles : placement_group_invalid_bundle_index_) {
    for (const auto &bundle_index : group_invalid_bundles.second) {
      if (required_resources.ContainsPlacementGroup(group_invalid_bundles.first,
                                                    bundle_index)) {
        std::ostringstream error_info;
        error_info << "The task: " << task_id
                   << " was interruptted because using a invalid bundle: " << bundle_index
                   << " of the placement group: " << group_invalid_bundles.first;
        RAY_LOG(WARNING) << error_info.str();
        send_reply_callback(Status::Invalid(error_info.str()), nullptr, nullptr);
      }
    }
  }

  // If job env is not initialized, we will stash this worker request away for a while,
  // waiting for job env to be initialized instead of scheduling and rejecting it
  // immediately. If we don't wait for a while, the request will be sent back and froth
  // between the caller and raylet, which will cause big network traffic.
  //
  // If timeout is reached and job env is still not initialized. We'll reject this worker
  // leasing request.
  auto job_id = task.GetTaskSpecification().JobId();
  if (waiting_for_job_env_initialized && job_id.IsSubmittedFromDashboard() &&
      !agent_manager_->IsJobEnvInitialized(job_id)) {
    // Add this task to a queue, reschedule it when job env is ready.
    WorkerRequestContext context{request, reply, send_reply_callback};
    waiting_for_env_worker_requests_.emplace(task_id, std::move(context));
    RAY_LOG(INFO) << "Env of job " << job_id
                  << "is not ready now, stash this worker request, task_id=" << task_id
                  << ", stashed map size is " << waiting_for_env_worker_requests_.size();

    bool need_warm_up = true;
    if (auto job_data = job_manager_->GetJobData(job_id)) {
      // In order to achieve eager task model, if it's normal task, we set a timeout to
      // wait InitializeJobEnv.
      need_warm_up = job_data->pre_initialize_job_runtime_env_enabled() ||
                     task.GetTaskSpecification().IsNormalTask();
      // This method ensures that the job is initialized only once.
      InitializeJobEnv(*job_data);
    }

    if (need_warm_up) {
      // Set a timeout, when reached, force reschedule it.
      // If it's a normal task, it will be spilled over to another node.
      // If it's an actor task, it will be spilled back to GCS.
      execute_after(
          io_service_,
          [this, job_id, task_id]() {
            auto iter = waiting_for_env_worker_requests_.find(task_id);
            if (iter != waiting_for_env_worker_requests_.end()) {
              const WorkerRequestContext &context = iter->second;
              agent_manager_->WarnJobEnvUninitialized(job_id);
              // Just reschedule it regardless of job env. Task scheduler will
              // find that local node is not feasible because job env is not
              // ready, and spill over/back this task.
              this->HandleRequestWorkerLease(context.request, context.reply,
                                             context.send_reply_callback,
                                             /* waiting_for_job_env_initialized=*/false);
              waiting_for_env_worker_requests_.erase(iter);
            }
          },
          RayConfig::instance().raylet_check_job_env_initialization_timeout_ms());
    }
  ...
  if (!(RayConfig::instance().gcs_task_scheduling_enabled() &&
    ...
    if (RayConfig::instance().max_pending_lease_requests_per_scheduling_category() <= 1) {
      cluster_task_manager_->QueueTask(task, request.grant_or_reject(), reply,
                                       send_reply_callback);
      AsyncScheduleAndDispatchTasks();
    } else {
      // When scheduling parallelism is large, we have to queue and schedule task
      // atomically. Otherwise, we may accept too many lease requests, because resources
      // are not allocated until dispatch. Even though excessive requests will eventually
      // be spilled when running into resource allocation failures (during dispatch), the
      // hybrid or spread policy is violated.
  ...
  // --------- GCS actor scheduling blow ----------

  auto actor_id = task.GetTaskSpecification().ActorCreationId();
  auto assignment_id = UniqueID::FromBinary(request.worker_process_id());

  RAY_CHECK(!assignment_id.IsNil())
      << "The assignment id leased by GCS should not be NIL.";
  RAY_LOG(INFO) << "Leased assignment id is " << assignment_id;
  if (!actor_worker_assignment_manager_->IsAssignable(assignment_id)) {
    std::string scheduling_failure_message =
        "The assignment id " + assignment_id.Hex() + " is invalid.";
    RAY_LOG(INFO) << scheduling_failure_message << " actor_id = " << actor_id;
    send_reply_callback(Status::OK(), nullptr, nullptr);
    ReplyCancelled(reply, send_reply_callback,
                   rpc::RequestWorkerLeaseReply::SCHEDULING_FAILED,
                   scheduling_failure_message);
  ...
  auto send_reply_callback_wrapper =
      [this, actor_id, task_id, assignment_id, reply, send_reply_callback](
        ...
        if (!reply->rejected()) {
          if (reply->canceled()) {
            // Cancel actor.
            RAY_UNUSED(actor_worker_assignment_manager_->CancelActor(task_id));
          } else {
            RAY_CHECK(actor_worker_assignment_manager_->OnActorAssigned(task_id));
          }
  ...
  RAY_CHECK(actor_worker_assignment_manager_->AssignActor(task_id, assignment_id));
  cluster_task_manager_->QueueTask(task, request.grant_or_reject(), reply,
...
void NodeManager::HandleCommitBundleResources(
  ...
  auto bundle_spec = std::make_shared<BundleSpecification>(request.bundle_spec());
  RAY_LOG(INFO) << "Request to commit bundle resources is received, "
                << bundle_spec->DebugString();

  const auto &job_id = bundle_spec->PlacementGroupId().JobId();
  auto callback = [this, job_id, bundle_spec, send_reply_callback](bool success) {
    if (worker_pool_.IsJobFinished(job_id)) {
      send_reply_callback(Status::Invalid("Job " + job_id.Hex() + " is finished."),
                          nullptr, nullptr);
      return;
    }

    if (!success) {
      std::string message = "Env of job " + job_id.Hex() + "is not ready now.";
      RAY_LOG(INFO) << message;
      // Return bundle resources.
      send_reply_callback(Status::Invalid(message), nullptr, nullptr);
  ...
  auto job_data = job_manager_->GetJobData(job_id);
  if (job_id.IsSubmittedFromDashboard() && !agent_manager_->IsJobEnvInitialized(job_id) &&
      job_data->pre_initialize_job_runtime_env_enabled()) {
    auto timer = execute_after(
        io_service_,
        /*callback=*/
        [this, bundle_spec] {
          auto iter = commit_bundle_resources_requests_.find(bundle_spec->BundleId());
          if (iter != commit_bundle_resources_requests_.end()) {
            iter->second->callback(/*success=*/false);
            commit_bundle_resources_requests_.erase(iter);
          }
        },
        RayConfig::instance().raylet_commit_bundle_resources_timeout_ms());
    auto context = std::make_shared<CommitBundleResourcesRequestContext>();
    context->timer = timer;
    context->callback = callback;
    commit_bundle_resources_requests_[bundle_spec->BundleId()] = context;
  } else {
...
void NodeManager::HandleCancelResourceReserve(
  ...
  // Remove pending requests from `commit_bundle_resources_requests_`
  auto iter = commit_bundle_resources_requests_.find(bundle_spec.BundleId());
  if (iter != commit_bundle_resources_requests_.end()) {
    iter->second->callback(/*success=*/false);
    commit_bundle_resources_requests_.erase(iter);
  ...
  // NOTE(loushang.ls): This method will be invoked in three kinds of cases, first is
  // removing placement groups when reserving resources failed, second is that removing
  // specified bundles, third is that removing placement groups when job dead.
  ...
  std::vector<std::shared_ptr<WorkerInterface>> workers_associated_with_bundle;
  for (const auto &worker_it : leased_workers_) {
    auto &worker = worker_it.second;
    if (worker->GetBundleId().first == bundle_spec.PlacementGroupId() &&
        worker->GetBundleId().second == bundle_spec.Index()) {
      if (bundle_spec.IsValid()) {
        workers_associated_with_pg.emplace_back(worker);
      } else {
        workers_associated_with_bundle.emplace_back(worker);
      }
    }
  }
  ...
  for (const auto &worker : workers_associated_with_bundle) {
    RAY_LOG(WARNING) << "Destroying worker since its bundle was removed or job dead. "
                        "Placement group id: "
                     << worker->GetBundleId().first
                     << ", bundle index: " << bundle_spec.BundleId().second
                     << ", task id: " << worker->GetAssignedTaskId()
                     << ", actor id: " << worker->GetActorId()
                     << ", worker id: " << worker->WorkerId();
    // Make the worker no restart as it's bundle has been removed.
    DestroyWorker(worker, rpc::WorkerExitType::INTENDED_EXIT);
  }
...
void NodeManager::HandleReleaseUnusedWorkers(
  ...
  std::unordered_map<WorkerID, Process> worker_id_to_process;
  for (const auto &worker : worker_pool_.GetAllRegisteredWorkers()) {
    worker_id_to_process.emplace(worker->WorkerId(), worker->GetProcess());
  }

  std::unordered_set<WorkerID> in_use_worker_ids;
  std::unordered_set<Process> in_use_worker_processes;
  for (int index = 0; index < request.worker_ids_in_use_size(); ++index) {
    ...
    auto iter = worker_id_to_process.find(worker_id);
    if (iter != worker_id_to_process.end()) {
      in_use_worker_processes.emplace(iter->second);
    }
  }

  // Remove leaked tasks to prevent memory leaking.
  auto canceled_actor_creation_tasks =
      actor_worker_assignment_manager_->CancelLeakedActors(in_use_worker_processes);
  for (auto &task_id : canceled_actor_creation_tasks) {
    cluster_task_manager_->CancelTask(task_id);
  }

  for (auto iter = leased_workers_.begin(); iter != leased_workers_.end(); ++iter) {
    // We need to exclude workers used by common tasks.
    // Because they are not used by GCS.
    if (!in_use_worker_ids.count(iter->first)) {
      const auto &assigned_task_id = iter->second->GetAssignedTaskId();
      if (IsActorCreationTask(assigned_task_id)) {
        cluster_task_manager_->CancelTask(assigned_task_id);
      }
    }
  }

  std::vector<std::shared_ptr<WorkerInterface>> to_be_killed_workers;
  std::vector<std::shared_ptr<WorkerInterface>> to_be_released_workers;
  for (auto iter = leased_workers_.begin(); iter != leased_workers_.end(); ++iter) {
    // We need to exclude workers used by common tasks.
    // Because they are not used by GCS.
    if (!in_use_worker_ids.count(iter->first)) {
      const auto &assigned_task_id = iter->second->GetAssignedTaskId();
      if (IsActorCreationTask(assigned_task_id)) {
        to_be_released_workers.emplace_back(iter->second);
        if (in_use_worker_processes.count(iter->second->GetProcess()) == 0) {
          to_be_killed_workers.emplace_back(iter->second);
        }
      }
    }
  }
  ...
  for (auto &released_worker : to_be_released_workers) {
    RAY_LOG(INFO) << "Release leaked worker " << released_worker->WorkerId()
                  << ", assigned_task_id = " << released_worker->GetAssignedTaskId();
    // Return the resources that were being used by this worker.
    cluster_task_manager_->ReleaseWorkerResources(released_worker);

    released_worker->AssignTaskId(TaskID::Nil());
    released_worker->ResetActorId();
    released_worker->SetOwnerAddress(rpc::Address());
    worker_pool_.PushWorker(released_worker);

    leased_workers_.erase(released_worker->WorkerId());
    resources_changed = true;
  }

  for (auto &worker : to_be_killed_workers) {
    RAY_LOG(INFO) << "Kill the leaked worker " << worker->WorkerId()
                  << ", process = " << worker->GetProcess().GetId();
    // TODO(Shanly): How to process common tasks?
    dependency_manager_.CancelWaitRequest(worker->WorkerId());
...
void NodeManager::HandleCancelWorkerLease(const rpc::CancelWorkerLeaseRequest &request,
  ...
  // Check whether this task was stashed because its job env is not ready.
  auto iter = waiting_for_env_worker_requests_.find(task_id);
  if (iter != waiting_for_env_worker_requests_.end()) {
    std::string msg = "Cancel leasing worker for task " + task_id.Hex();
    RAY_LOG(INFO) << msg;
    // Cancel worker lease request.
    const auto &worker_request = iter->second;
    ReplyCancelled(worker_request.reply, worker_request.send_reply_callback,
...
std::string NodeManager::DebugString() const {
  ...
  for (const auto &entry : remote_node_info_) {
    result << "\n" << entry.first;
  }
...
void NodeManager::HandlePinObjectIDs(const rpc::PinObjectIDsRequest &request,
  ...
  send_reply_callback(Status::OK(), nullptr, nullptr);
  ray::stats::PlasmaPinnedObjectRequestCount().Record(1);
}
...
void NodeManager::HandleFormatGlobalMemoryInfo(
  ...
  unsigned int num_nodes = remote_node_info_.size() + 1;
  ...
  for (const auto &entry : remote_node_info_) {
    auto client = std::make_unique<rpc::NodeManagerClient>(
        entry.second->node_manager_address(), entry.second->node_manager_port(),
...
void NodeManager::RecordMetrics() {
  ...
  // Record leased worker metrices.
  size_t leased_actor_workers = 0;
  size_t leased_normal_workers = 0;
  for (auto &entry : leased_workers_) {
    std::shared_ptr<WorkerInterface> worker = entry.second;
    if (worker->IsForActor()) {
      ++leased_actor_workers;
    } else {
      ++leased_normal_workers;
    }
  }
  stats::CurrentLeasedWorkers().Record(leased_actor_workers,
                                       {{stats::LeasedWorkerTypeKey, "Actor"}});
  stats::CurrentLeasedWorkers().Record(leased_normal_workers,
                                       {{stats::LeasedWorkerTypeKey, "NormalTask"}});
}
...
void NodeManager::NodegroupUpdated(const NodegroupData &nodegroup_data) {
  RAY_LOG(INFO) << "Update nodegroup " << nodegroup_data.nodegroup_id();
  if (nodegroup_data.nodegroup_updated_timestamp() <
      nodegroup_updated_timestamps_[nodegroup_data.nodegroup_id()]) {
    RAY_LOG(INFO) << "Receiving an outdated nodegroup update. Ignore it.";
    return;
  }

  auto local_host_name = boost::asio::ip::host_name();

  nodegroup_updated_timestamps_[nodegroup_data.nodegroup_id()] =
      nodegroup_data.nodegroup_updated_timestamp();
  // Nodegroup was removed.
  if (nodegroup_data.is_removed()) {
    RAY_LOG(INFO) << "Remove nodegroup " << nodegroup_data.nodegroup_id();
    // After removing nodegroup, node which is divided into this nodegroup previously need
    // to clear.
    auto iter = nodegroup_hosts_.find(nodegroup_data.nodegroup_id());
    if (iter != nodegroup_hosts_.end()) {
      if (iter->second.find(local_host_name) != iter->second.end()) {
        // Only clean job env. we don't remove node from nodegroup, because nodegroup will
        // be deleted.
        RAY_LOG(INFO) << "Because the node" << local_host_name
                      << " belongs to the nodegroup" << nodegroup_data.nodegroup_id()
                      << " which is removed soon. Job env will be clean.";
        for (const auto &entry : job_manager_->GetAllJobData()) {
          agent_manager_->ClearJobEnv(*entry.second);
          runtime_env_manager_.RemoveURIReference(entry.first.Hex());
        }
      }
    }
    nodegroup_hosts_.erase(nodegroup_data.nodegroup_id());
    return;
  }

  bool is_local_node_already_in_nodegroup =
      nodegroup_hosts_.count(nodegroup_data.nodegroup_id()) &&
      nodegroup_hosts_[nodegroup_data.nodegroup_id()].count(local_host_name);

  // Update hosts of this nodegroup.
  std::unordered_set<std::string> hosts;
  for (const auto &entry : nodegroup_data.host_to_node_spec()) {
    hosts.emplace(entry.first);
  }
  nodegroup_hosts_[nodegroup_data.nodegroup_id()] = hosts;

  bool is_local_node_in_nodegroup = hosts.count(local_host_name);

  // Local node is removed from the nodegroup
  if (is_local_node_already_in_nodegroup && !is_local_node_in_nodegroup) {
    // It is well-known that local_host_name is deleted from current nodegroup.
    // We should only clear job env because hosts of nodegroup have been update.
    RAY_LOG(INFO) << "The node" << local_host_name
                  << "which belongs to the nodegroup, but now is removed because it is "
                     "Shrunk. job env on the node should be cleaned.";
    for (const auto &entry : job_manager_->GetAllJobData()) {
      agent_manager_->ClearJobEnv(*entry.second);
      runtime_env_manager_.RemoveURIReference(entry.first.Hex());
    }
  ...
  // Local node is newly added to the nodegroup.
  if (!is_local_node_already_in_nodegroup && is_local_node_in_nodegroup) {
    for (const auto &entry : job_manager_->GetAllJobData()) {
      if (entry.first.IsSubmittedFromDashboard() &&
          entry.second->pre_initialize_job_runtime_env_enabled() &&
          entry.second->nodegroup_id() == nodegroup_data.nodegroup_id()) {
        InitializeJobEnv(*entry.second);
      }
    }
  }

  if (nodegroup_data.nodegroup_id() != NODEGROUP_RESOURCE_DEFAULT &&
      is_local_node_in_nodegroup) {
    auto iter =
        nodegroup_data.schedule_options().find("runtime_resource_scheduling_enabled");
    if (iter != nodegroup_data.schedule_options().end()) {
      overcommit_enabled_ = (iter->second == "true");
    }
    if (schedule_options_.UpdateFromMapProto(nodegroup_data.schedule_options())) {
      agent_manager_->OnRuntimeResourceScheduleOptionsUpdated(schedule_options_);
    }
  }

  // Reschedule infesible tasks.
  // When nodegroups are updated, some infesible tasks might be able to be scheduled.
  cluster_task_manager_->ScheduleInfeasibleTasks();
  AsyncScheduleAndDispatchTasks();
}

void NodeManager::OnPlacementGroupBundlesChanged(
    const rpc::PlacementGroupBundlesChangedNotification &notification) {
  const PlacementGroupID &placement_group_id =
      PlacementGroupID::FromBinary(notification.placement_group_id());
  LogBundlesChangedEventDebugInfo(notification);
  std::vector<int32_t> invalid_bundel_indexes;
  for (int index = 0; index < notification.bundles_size(); index++) {
    if (!notification.bundles(index)) {
      cluster_task_manager_->CancelTaskWithPlacementGroup(placement_group_id, index);
      invalid_bundel_indexes.push_back(index);
    }
  }
  // Update the local placement group invalid bundles view.
  if (!invalid_bundel_indexes.empty()) {
    placement_group_invalid_bundle_index_[placement_group_id] =
        std::move(invalid_bundel_indexes);
  }
}

void NodeManager::InitializeJobEnv(const rpc::JobTableData &job_data) {
  auto job_id = JobID::FromBinary(job_data.job_id());
  RAY_CHECK(!job_data.is_dead())
      << "Failed to initialize env for a dead job, job_id = " << job_id;
  if (agent_manager_->IsJobEnvInitialized(job_id)) {
    RAY_LOG(INFO) << "The job env has been initialized, job_id = " << job_id;
    return;
  }
  RAY_LOG(INFO) << "Initializing job env, job_id = " << job_id;
  rpc::JobEnvInitNotification job_env_init_notification;
  job_env_init_notification.mutable_job_table_data()->CopyFrom(job_data);
  if (job_data.raylet_id() == self_node_id_.Binary()) {
    job_env_init_notification.set_start_driver(true);
  }
  agent_manager_->OnJobEnvInitNotification(job_env_init_notification);
}

void NodeManager::OnJobEnvInitialized(const JobID &job_id, const Status &status) {
  for (auto iter = commit_bundle_resources_requests_.begin();
       iter != commit_bundle_resources_requests_.end();) {
    auto current = iter++;
    if (current->first.first.JobId() == job_id) {
      current->second->callback(/*success=*/status.ok());
      commit_bundle_resources_requests_.erase(current);
    }
  }

  for (auto iter = waiting_for_env_worker_requests_.begin();
       iter != waiting_for_env_worker_requests_.end();) {
    const auto &worker_request = iter->second;
    JobID request_job_id =
        JobID::FromBinary(worker_request.request.resource_spec().job_id());
    if (request_job_id == job_id) {
      if (!status.ok()) {
        auto task_id =
            TaskID::FromBinary(worker_request.request.resource_spec().task_id());
        std::ostringstream ostr;
        ostr << "Env of job " << job_id << " is not ready, task_id = " << task_id;
        if (IsActorCreationTask(task_id)) {
          ostr << ", actor_id = " << task_id.ActorId();
        }
        auto scheduling_failure_message = ostr.str();
        ReplyCancelled(worker_request.reply, worker_request.send_reply_callback,
                       rpc::RequestWorkerLeaseReply::SCHEDULING_FAILED,
                       scheduling_failure_message);
      } else {
        HandleRequestWorkerLease(worker_request.request, worker_request.reply,
                                 worker_request.send_reply_callback, false);
      }
      iter = waiting_for_env_worker_requests_.erase(iter);
      continue;
    }
    iter++;
  }
...
void NodeManager::FillNormalTaskResourceUsage(rpc::ResourcesData &resource_data,
  ...
  if (need_whole_report || !last_heartbeat_resources->GetNormalTaskResources().IsEqual(
    ...
    resource_data.set_resources_normal_task_changed(true);
    auto &normal_task_map = *(resource_data.mutable_resources_normal_task());
    auto normal_task_resource_map = normal_task_resources.GetResourceMap();
    normal_task_map = {normal_task_resource_map.begin(), normal_task_resource_map.end()};
    resource_data.set_resources_normal_task_timestamp(current_sys_time_ns());
    last_heartbeat_resources->SetNormalTaskResources(normal_task_resources);
  }
}

bool NodeManager::IsNodeFrozen(const NodeID &node_id) const {
  if (node_id == self_node_id_) {
    return frozen_node_set_.count(initial_config_.node_name);
  }
  auto iter = remote_node_info_.find(node_id);
  if (iter == remote_node_info_.end()) {
    return false;
  }
  return frozen_node_set_.count(iter->second->node_name()) > 0;
}

void NodeManager::OnFrozenNodeUpdated(
    const std::unordered_set<std::string> &frozen_node_set) {
  std::string frozen_node_set_str =
      StrJoin(frozen_node_set.begin(), frozen_node_set.end(),
              [](auto &iter) { return *iter; }, ",");
  RAY_LOG(INFO) << "FrozenNodesNotification received, set=[" << frozen_node_set_str
                << "], self node name=" << initial_config_.node_name;

  std::unordered_set<std::string> to_be_unfrozen;
  for (auto &entry : frozen_node_set_) {
    if (!frozen_node_set.count(entry)) {
      to_be_unfrozen.emplace(entry);
    }
  }

  frozen_node_set_ = frozen_node_set;

  // Try to reschedule infesiable tasks.
  if (to_be_unfrozen.count(initial_config_.node_name)) {
    AsyncScheduleAndDispatchTasks();
  }
  for (auto &pair : remote_node_info_) {
    if (!to_be_unfrozen.count(pair.second->node_name())) {
      continue;
    }
    AsyncScheduleAndDispatchTasks();
  }

  if (frozen_node_set.count(initial_config_.node_name)) {
    RAY_UNUSED(gcs_client_->FrozenNodes().ReportFreezeDone(
        initial_config_.node_name, self_node_id_, [](const auto status) {
          RAY_LOG(INFO) << "FrozenNodesNotification handle done.";
        }));
  }
}

bool NodeManager::IsHostInNodegroup(const std::string &host_name,
                                    const std::string &nodegroup_id) const {
  if (nodegroup_id == NODEGROUP_RESOURCE_DEFAULT) {
    return true;
  }
  auto iter = nodegroup_hosts_.find(nodegroup_id);
  return iter != nodegroup_hosts_.end() && iter->second.count(host_name);
}

bool NodeManager::IsNodeSchedulable(const std::string &binary_node_id,
                                    const ScheduleContext &schedule_context) const {
  NodeID node_id = NodeID::FromBinary(binary_node_id);
  const JobID &job_id = schedule_context.task_spec.JobId();
  // Find hostname.
  std::string node_host_name;
  auto node_iter = remote_node_info_.find(node_id);
  if (node_iter != remote_node_info_.end()) {
    node_host_name = node_iter->second->node_manager_hostname();
  } else if (node_id == self_node_id_) {
    node_host_name = initial_config_.node_manager_hostname;
  }
  // Find nodegroup.
  auto job_data = job_manager_->GetJobData(job_id);
  const std::string &nodegroup_id =
      job_data == nullptr || job_data->nodegroup_id().empty() ? NODEGROUP_RESOURCE_DEFAULT
                                                              : job_data->nodegroup_id();

  if (!IsHostInNodegroup(node_host_name, nodegroup_id)) {
    return false;
  }
  // Check job env initialization.
  if (node_id == self_node_id_ && job_id.IsSubmittedFromDashboard() &&
      !agent_manager_->IsJobEnvInitialized(job_id)) {
    return false;
  }
  // Frozen node checking.
  if (IsNodeFrozen(node_id)) {
    return false;
  }
  return true;
}

void NodeManager::DumpNodeMetaData() const {
  rapidjson::Document doc(rapidjson::kObjectType);
  rapidjson::Document::AllocatorType &allocator = doc.GetAllocator();
  rapidjson::Document doc_nodegroup_ids(rapidjson::kArrayType);
  for (const auto &entry : nodegroup_hosts_) {
    if (entry.second.count(initial_config_.node_manager_hostname)) {
      doc_nodegroup_ids.PushBack(rapidjson::Value(entry.first, allocator), allocator);
    }
  }
  doc.AddMember("namespaces", doc_nodegroup_ids, allocator);

  auto file_name = initial_config_.session_dir + "/ray_node_meta.txt";

  // Write to tmp file.
  auto tmp_file_name = file_name + ".tmp";
  std::fstream fs;
  fs.open(tmp_file_name, std::fstream::out | std::fstream::trunc);
  fs << rapidjson::to_string(doc, true);
  fs.close();

  // Rename tmp file to target file.
  if (std::rename(tmp_file_name.c_str(), file_name.c_str())) {
    RAY_LOG(WARNING) << "Failed to rename " << tmp_file_name << " to " << file_name;
  }
}
...
MemoryUsageRefreshCallback NodeManager::CreateMemoryUsageRefreshCallback() {
  return [this](bool is_usage_above_threshold, MemorySnapshot system_memory,
                float usage_threshold) {
    if (high_memory_eviction_target_ != nullptr) {
      if (!Process::IsAlive(high_memory_eviction_target_->GetProcess().GetId())) {
    ...
    if (is_usage_above_threshold) {
      if (high_memory_eviction_target_ != nullptr) {
        ...
        if (schedule_options_.runtime_resource_scheduling_enabled_) {  /// ANT-INTERNAL
          ResourceViolationWorkerKillingPolicy worker_killing_policy;
        ...
        if (worker_to_kill == nullptr) {
          ...
          DestroyWorker(high_memory_eviction_target_,
                        rpc::WorkerExitType::NODE_OUT_OF_MEMORY, true /* force */);

          stats::WorkerEvictionCount.Record(
              1, {{ray::stats::WorkerNodeAddressKey, worker_to_kill->IpAddress()}});
        }
      }
    }
  };
...
const std::string NodeManager::CreateOomKillMessageDetails(
    const std::shared_ptr<WorkerInterface> &worker, const NodeID &node_id,
    const MemorySnapshot &system_memory, float usage_threshold) const {
