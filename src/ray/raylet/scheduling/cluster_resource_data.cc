...
std::string NodeResources::DebugString(const StringIdMap &string_to_in_map) const {
  std::stringstream buffer;
  buffer << " {\n";
  for (size_t i = 0; i < this->predefined_resources.size(); i++) {
    buffer << "\t" << ResourceEnumToString(PredefinedResources(i)) << ": "
...
std::string NodeResources::DictString(const StringIdMap &string_to_in_map) const {
  std::stringstream buffer;
  bool first = true;
  buffer << "{";
  for (size_t i = 0; i < this->predefined_resources.size(); i++) {
...
std::string NodeResourceInstances::DebugString(
    const StringIdMap &string_to_int_map) const {
  std::stringstream buffer;
  buffer << "{\n";
  for (size_t i = 0; i < this->predefined_resources.size(); i++) {
    buffer << "\t" << ResourceEnumToString(PredefinedResources(i)) << ": "
           << "(" << VectorToString(predefined_resources[i].total) << ":"
...
std::string TaskRequest::DebugString(const StringIdMap &string_to_int_map) const {
  std::stringstream buffer;
  buffer << " {";
  for (size_t i = 0; i < this->predefined_resources.size(); i++) {
  ...
  buffer << "  [";
  for (size_t i = 0; i < this->custom_resources.size(); i++) {
    buffer << string_to_int_map.Get(this->custom_resources[i].id) << ":"
