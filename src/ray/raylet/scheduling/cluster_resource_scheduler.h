...
class ClusterResourceScheduler : public ClusterResourceSchedulerInterface {
 public:
  ...
  ClusterResourceScheduler(
      ...
      const std::function<bool(const std::string &binary_node_id,
          ...
          &is_node_schedulable_fn = [](auto &, auto &) { return true; });
  ClusterResourceScheduler(
      ...
      const std::function<bool(const std::string &binary_node_id,
                               const ScheduleContext &schedule_context)>
          &is_node_schedulable_fn = [](auto &, auto &) { return true; },
      std::function<bool()> is_overcommit_enabled_fn = []() { return false; });
  ...
  /// \param node_id Node ID of the given node
  /// \param resources Node's resources.
  bool IsFeasible(const TaskRequest &task_req, int64_t node_id,
  ...
  int64_t GetBestSchedulableNode(const TaskRequest &task_request,
  ...
  /// Ant-Internal.
  /// Check whether the total resources are empty.
  ///
  /// \param resource_name: Resource which we want to check.
  bool IsTotalResourceEmpty(const std::string &resource_name);
  ...
  bool RemoveResourceByInstance(const std::string &resource_name,
                                const std::vector<FixedPoint> &instances);
  ...
  ///
  /// \param task_resources The resources to be allocated.
  ...
  /// at the instance granularity.
  ///
  /// \param task_spec The specification of the task for which we allocate resources.
  /// \param task_allocation Resources allocated to the task at instance granularity.
  /// This is a return parameter.
  ///
  /// \return True if local node has enough resources to satisfy the task request.
  /// False otherwise.
  bool AllocateLocalTaskResources(const TaskSpecification &task_spec,
                                  std::shared_ptr<TaskResourceInstances> task_allocation);

  bool AllocateLocalTaskResources(const BundleSpecification &bundle_spec,
                                  std::shared_ptr<TaskResourceInstances> task_allocation);
  ...
  void FillResourceUsage(rpc::ResourcesData &resources_data,
                         bool need_whole_report = false) override;
  ...
  bool ContainsResource(const std::string &resource_name) const override;
 ...
 private:
  ...
  /// === ANT-INTERNAL below ===
  /// Additional filter function to check whether a node is schedulable
  const std::function<bool(const std::string &binary_node_id,
                           const ScheduleContext &schedule_context)>
      is_node_schedulable_fn_;

  bool ContainsRareResources(const TaskRequest &task_req) const;

  bool ContainsRareResources(const NodeResources &resources) const;

  bool IsNodeUsable(int64_t node_id, const ScheduleContext &schedule_context) const;

  std::function<bool()> is_overcommit_enabled_fn_;
  FRIEND_TEST(ClusterResourceSchedulerTest, SchedulingTaskRequestTest);
  /// === ANT-INTERNAL above ===
};
