...
class ClusterResourceSchedulerTest : public ::testing::Test {
 public:
  ...
  rpc::BasicGcsNodeInfo node_info;
...
TEST_F(ClusterResourceSchedulerTest, NodeAffinitySchedulingStrategyTest) {
  std::unordered_map<std::string, double> resource_total({{"CPU", 10}});
  ...
  scheduling_strategy.mutable_node_affinity_scheduling_strategy()->add_nodes(
      remote_node_id);
  ...
  scheduling_strategy.clear_node_affinity_scheduling_strategy();
  scheduling_strategy.mutable_node_affinity_scheduling_strategy()->add_nodes(
      local_node_id);
  scheduling_strategy.mutable_node_affinity_scheduling_strategy()->set_soft(false);
...
TEST_F(ClusterResourceSchedulerTest, SpreadSchedulingStrategyTest) {
  ...
  std::string node_id_1 = resource_scheduler.GetBestSchedulableNode(
      resource_request, scheduling_strategy, false, false, &violations, &is_infeasible,
      mock_task_spec);
  std::unordered_map<std::string, double> resource_available({{"CPU", 9}});
  resource_scheduler.AddOrUpdateNode(node_id_1, resource_total, resource_available);
...
#define PRED_CUSTOM_LEN 2
    ...
    int64_t node_id = resource_scheduler.GetBestSchedulableNode(
        task_req, scheduling_strategy, false, false, &violations, &is_infeasible,
        mock_task_spec);
    ASSERT_EQ(node_id, 1);
...
TEST_F(ClusterResourceSchedulerTest, DeadNodeTest) {
  ...
  ASSERT_EQ(node_id.Binary(), resource_scheduler.GetBestSchedulableNode(
                                  resource, scheduling_strategy, false, false,
                                  &violations, &is_infeasible, mock_task_spec));
  ...
  ASSERT_EQ("", resource_scheduler.GetBestSchedulableNode(
                    resource, scheduling_strategy, false, false, &violations,
...
TEST_F(ClusterResourceSchedulerTest, TestAlwaysSpillInfeasibleTask) {
  ...
  ASSERT_EQ(remote_feasible, resource_scheduler.GetBestSchedulableNode(
                                 ...
                                 &total_violations, &is_infeasible, mock_task_spec));
    ...
    for (int i = 0; i < 3; i++) {
      ...
      for (int j = 0; j < num_slots_available; j++) {
        ASSERT_EQ(remote, resource_scheduler.GetBestSchedulableNode(
                              task_spec, scheduling_strategy, false, false, &t,
                              &is_infeasible, mock_task_spec));
      ...
      ASSERT_EQ("", resource_scheduler.GetBestSchedulableNode(
                        task_spec, scheduling_strategy, false, true, &t, &is_infeasible,
                        mock_task_spec));
      ASSERT_FALSE(
...
TEST_F(ClusterResourceSchedulerTest, DynamicResourceTest) {
  ...
  std::string result = resource_scheduler.GetBestSchedulableNode(
      task_request, scheduling_strategy, false, false, &t, &is_infeasible,
      mock_task_spec);
  ASSERT_TRUE(result.empty());

  resource_scheduler.AddLocalResourceInstances("custom123", {0., 1.0, 1.0});

  result = resource_scheduler.GetBestSchedulableNode(task_request, scheduling_strategy,
...
TEST_F(ClusterResourceSchedulerTest, TestForceSpillback) {
  ...
  ASSERT_EQ(
      resource_scheduler.GetBestSchedulableNode(
          resource_spec, scheduling_strategy, false,
          /*force_spillback=*/false, &total_violations, &is_infeasible, mock_task_spec),
      "local");
  ...
  ASSERT_EQ(
      resource_scheduler.GetBestSchedulableNode(
          resource_spec, scheduling_strategy, false,
          /*force_spillback=*/true, &total_violations, &is_infeasible, mock_task_spec),
      "");
...
TEST_F(ClusterResourceSchedulerTest, TaskResourceInstancesSerializedStringTest) {
  ...
  std::string expected_serialized_string = R"({"CPU":2,"memory":4,"GPU":[1, 1]})";
