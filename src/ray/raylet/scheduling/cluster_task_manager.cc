...
#include "ray/event/event_label.internal.h"
...
ClusterTaskManager::ClusterTaskManager(
    ...
    : self_node_id_(self_node_id),
      ...
      actor_worker_assignment_manager_(actor_worker_assignment_manager) {}

namespace {
void ReplyCancelled(const Work &work,
                    rpc::RequestWorkerLeaseReply::SchedulingFailureType failure_type,
                    const std::string &scheduling_failure_message) {
  auto reply = std::get<2>(work);
  auto callback = std::get<3>(work);
  ...
  for (auto shapes_it = tasks_to_schedule_.begin();
    ...
    for (auto work_it = work_queue.begin(); work_it != work_queue.end();) {
      ...
      std::string node_id_string =
          GetBestSchedulableNode(work, /*force_spillback=*/false, &is_infeasible);
      ...
      if (node_id_string.empty()) {
        ...
        if (task.GetTaskSpecification().IsNodeAffinitySchedulingStrategy() &&
          ...
          ReplyCancelled(
              work, rpc::RequestWorkerLeaseReply::SCHEDULING_CANCELLED_UNSCHEDULABLE,
...
void ClusterTaskManager::DispatchScheduledTasksToWorkers(
  ...
  for (auto shapes_it = tasks_to_dispatch_.begin();
    ...
    for (auto work_it = dispatch_queue.begin(); work_it != dispatch_queue.end();) {
      ...
      // Check if a task (normal task only for now) should be rescheduled.
      if (spec.IsNormalTask() &&
          RayConfig::instance().rescheduling_by_worker_registration_timeouts() &&
          worker_pool_.GetPastWorkerRegistrationTimeoutsCount() >
              RayConfig::instance().worker_registration_timeouts_limit() &&
          !spec.GetRequiredPlacementResources().ContainsPlacementGroup()) {
        // The worker pool has detected too many worker registration timeouts, maybe it is
        // because of system overloading. Try spill the task to another node.
        bool grant_or_reject = std::get<1>(work);
        if (grant_or_reject) {
          // We are going to spillback this task to a remote node. Set `grant_or_reject`
          // false so that the task creator will trust the spillback decision made here.
          std::get<1>(work) = false;
        }
        bool did_spill = TrySpillback(work, is_infeasible, /*force_spillback=*/true);
        if (!did_spill) {
          // Recover the task's original `grant_or_reject` setup (if it has been changed
          // above).
          if (grant_or_reject) {
            std::get<1>(work) = true;
          }
          if (is_infeasible) {
            // Because cluster resource scheduler sets `require_available =
            // force_spillback` as a hack in order to remain bug compatible with the
            // legacy scheduling algorithms, `is_infeasible` here does not truely mean
            // this task infeasible. So do the scheduling procedure again with
            // `force_spillback=false`. Then if `is_infeasible` is still true, we move
            // this task and the rest of the shape to `infeasible_tasks` below (out of
            // the inner loop).
            is_infeasible = false;
            GetBestSchedulableNode(work, /*force_spillback=*/false, &is_infeasible);
          }
        ...
        RAY_LOG(DEBUG)
            << "Task " << task_id
            << " has been rescheduled to a remote node, because worker pool has detected "
            << worker_pool_.GetPastWorkerRegistrationTimeoutsCount()
            << " worker registration timeouts recently.";
        // This task has been rescheduled successfully, try next one.
        work_it = dispatch_queue.erase(work_it);
        continue;
      }

      // If the task has a RuntimeEnv set and the creation of the worker fails multiple
      // times, the task will be canceled。
      int64_t worker_register_timeout_num = 0;
      if (actor_worker_assignment_manager_) {
        worker_register_timeout_num =
            actor_worker_assignment_manager_->GetAssignmentWorkRegisterTimeoutNum(
                task_id);
      }
      if (spec.HasRuntimeEnv() &&
          worker_register_timeout_num >=
              RayConfig::instance().worker_register_timeout_max_retries()) {
        std::ostringstream ostr;
        ostr << "This task will be cancelled because the worker creation failed or the "
                "runtime env container creation failed "
             << worker_register_timeout_num << " times >= "
             << RayConfig::instance().worker_register_timeout_max_retries()
             << " times, task id:" << task_id
             << ". You can check the detail error by looking at the log of the node "
             << self_node_id_;
        RAY_LOG(WARNING) << ostr.str();
        RAY_EVENT(ERROR, EVENT_LABEL_RUNTIME_ENV_SETUP_FAILED)
                .WithField("host_name", boost::asio::ip::host_name())
                .WithField("job_id", spec.JobId().Hex())
            << ostr.str();
      ...
      if (!schedulable) {
        ...
        if (RayConfig::instance().worker_resource_limits_enabled()) {
          RAY_LOG(DEBUG)
              << "Worker resource limits enabled, will serialize the task resources.";
        ...
        RAY_LOG(DEBUG) << "Dispatching task " << task_id << " to worker "
                       << worker->WorkerId();
        auto reply = std::get<2>(*work_it);
        auto callback = std::get<3>(*work_it);
...
bool ClusterTaskManager::TrySpillback(const Work &work, bool &is_infeasible,
  ...
  std::string node_id_string =
      GetBestSchedulableNode(work, force_spillback, &is_infeasible);
...
void ClusterTaskManager::CancelTaskWithPlacementGroup(
    const PlacementGroupID &placement_group_id, int64_t bundle_index) {
  std::vector<RayTask> killing_tasks = {};
  for (auto shapes_it = tasks_to_schedule_.begin(); shapes_it != tasks_to_schedule_.end();
       shapes_it++) {
    auto &work_queue = shapes_it->second;
    for (auto work_it = work_queue.begin(); work_it != work_queue.end(); work_it++) {
      const auto &task = std::get<0>(*work_it);
      const auto &resources = task.GetTaskSpecification().GetRequiredPlacementResources();
      if (resources.ContainsPlacementGroup(placement_group_id, bundle_index)) {
        killing_tasks.push_back(task);
      }
    }
  }
  ...
  std::ostringstream ostr;
  ostr << "The bundle " << bundle_index << " of " << placement_group_id << " is removed.";
  std::string scheduling_failure_message = ostr.str();

  for (const auto &task : killing_tasks) {
    RAY_CHECK(
        CancelTask(task.GetTaskSpecification().TaskId(),
                   rpc::RequestWorkerLeaseReply::SCHEDULING_CANCELLED_BUNDLE_REMOVED,
...
void ClusterTaskManager::RecordMetrics() {
  ...
  stats::NumTasksToSchedule().Record(tasks_to_schedule_.size());
  stats::NumTasksToDispatch().Record(tasks_to_dispatch_.size());
  stats::NumWaitingTasksQueueLength().Record(waiting_task_queue_.size());
  stats::NumExecutingTasks().Record(executing_task_args_.size());
  stats::NumPinnedTasks().Record(pinned_task_arguments_.size());
}
...
void ClusterTaskManager::Spillback(const NodeID &spillback_to, const Work &work) {
  if (std::get<1>(work)) {
    auto reply = std::get<2>(work);
    auto send_reply_callback = std::get<3>(work);
...
void ClusterTaskManager::CancelAndRemoveTasksForJob(
    const JobID &job_id, rpc::RequestWorkerLeaseReply::SchedulingFailureType failure_type,
    const std::string &scheduling_failure_message) {
  auto filter_job_task =
      [](const JobID &job_id,
         const std::unordered_map<SchedulingClass, std::deque<Work>> &shaped_task_queue,
         absl::flat_hash_set<TaskID> *task_ids) {
        for (auto shapes_it = shaped_task_queue.begin();
             shapes_it != shaped_task_queue.end(); shapes_it++) {
          auto &work_queue = shapes_it->second;
          for (auto work_it = work_queue.begin(); work_it != work_queue.end();
               work_it++) {
            const auto &task = std::get<0>(*work_it);
            if (task.GetTaskSpecification().TaskId().JobId() == job_id) {
              task_ids->emplace(task.GetTaskSpecification().TaskId());
            }
          }
        }
      };

  absl::flat_hash_set<TaskID> related_task_ids;
  filter_job_task(job_id, tasks_to_schedule_, &related_task_ids);
  filter_job_task(job_id, tasks_to_dispatch_, &related_task_ids);
  filter_job_task(job_id, infeasible_tasks_, &related_task_ids);
  for (auto &task_id : related_task_ids) {
    CancelTask(task_id, failure_type, scheduling_failure_message);
  }
}

bool ClusterTaskManager::IsLocallySchedulable(const RayTask &task) {
  const auto &spec = task.GetTaskSpecification();
  return cluster_resource_scheduler_->IsLocallySchedulable(
      spec.GetRequiredResources().GetResourceMap(), spec);
}
...
std::string ClusterTaskManager::GetBestSchedulableNode(const Work &work,
                                                       bool force_spillback,
                                                       bool *is_infeasible) {
  RayTask task = std::get<0>(work);
  ...
  if (std::get<1>(work) && !force_spillback && IsLocallySchedulable(task)) {
    *is_infeasible = false;
  ...
  return cluster_resource_scheduler_->GetBestSchedulableNode(
      ...
      task.GetTaskSpecification().GetMessage().scheduling_strategy(),
      task.GetTaskSpecification().IsActorCreationTask(), force_spillback, &_unused,
      is_infeasible, task.GetTaskSpecification());
}
