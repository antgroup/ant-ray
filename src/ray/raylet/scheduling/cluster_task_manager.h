...
typedef std::tuple<RayTask, bool, rpc::RequestWorkerLeaseReply *,
                   std::function<void(void)>>
    Work;

typedef std::function<const rpc::BasicGcsNodeInfo *(const NodeID &node_id)>
    NodeInfoGetter;
...
class ClusterTaskManager : public ClusterTaskManagerInterface {
 public:
  ...
  void CancelAndRemoveTasksForJob(
      const JobID &job_id,
      rpc::RequestWorkerLeaseReply::SchedulingFailureType failure_type =
          rpc::RequestWorkerLeaseReply::SCHEDULING_CANCELLED_INTENDED,
      const std::string &scheduling_failure_message = "") override;
  ...
  bool IsLocallySchedulable(const RayTask &task) override;

  /// Calculate normal task resources.
  ResourceSet CalcNormalTaskResources() const override;

  /// Queue task. This hanppens when processing the worker lease request.
  ///
  /// \param task: The incoming task to be queued and scheduled.
  /// \param reply: The reply of the lease request.
  /// \param send_reply_callback: The function used during dispatching.
  void QueueTask(const RayTask &task, const bool grant_or_reject,
                 rpc::RequestWorkerLeaseReply *reply,
                 rpc::SendReplyCallback send_reply_callback) override;
  /// === ANT-INTERNAL above ===
 ...
 private:
  ...
  bool TrySpillback(const Work &spec, bool &is_infeasible, bool force_spillback = false);
  ...
  std::string GetBestSchedulableNode(const Work &work, bool force_spillback,
                                     bool *is_infeasible);
