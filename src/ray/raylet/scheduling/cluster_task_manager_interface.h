...
class ClusterTaskManagerInterface {
 public:
  ...
  virtual void FillResourceUsage(
      ...
      bool need_whole_report = false) = 0;
  ...
  /// placement_group_id.
  virtual void CancelTaskWithPlacementGroup(const PlacementGroupID &placement_group_id,
                                            int64_t bundle_index) = 0;
  ...
  virtual void CancelAndRemoveTasksForJob(
      const JobID &job_id,
      rpc::RequestWorkerLeaseReply::SchedulingFailureType failure_type =
          rpc::RequestWorkerLeaseReply::SCHEDULING_CANCELLED_INTENDED,
      const std::string &scheduling_failure_message = "") = 0;
  ...
  virtual bool IsLocallySchedulable(const RayTask &task) = 0;
  ...
  /// \param reply: The reply of the lease request.
  /// \param send_reply_callback: The function used during dispatching.
  virtual void QueueTask(const RayTask &task, const bool grant_or_reject,
                         rpc::RequestWorkerLeaseReply *reply,
