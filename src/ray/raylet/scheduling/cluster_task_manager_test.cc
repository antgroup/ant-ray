...
class MockWorkerPool : public WorkerPoolInterface {
 public:
  ...
  uint32_t GetPastWorkerRegistrationTimeoutsCount() {
    return past_worker_registration_timeouts_count;
  }

  std::list<std::shared_ptr<WorkerInterface>> workers;
  int num_pops;
  uint32_t past_worker_registration_timeouts_count = 0;
};

std::shared_ptr<ClusterResourceScheduler> CreateSingleNodeScheduler(
    const std::string &id, gcs::GcsClient &gcs_client) {
...
RayTask CreateTask(const std::unordered_map<std::string, double> &required_resources,
  ...
  spec_builder.SetCommonTaskSpec(
      ...
      std::unordered_map<std::string, std::string>(), ActorID::Nil(), true);
 ...
 public:
  ClusterTaskManagerTest()
      : gcs_client_(std::make_unique<gcs::MockGcsClient>()),
        ...
        scheduler_(CreateSingleNodeScheduler(id_.Binary(), *gcs_client_)),
        ...
        task_manager_(id_, scheduler_, dependency_manager_,
                      ...
                      /* get_node_info= */
                      [this](const NodeID &node_id) -> const rpc::BasicGcsNodeInfo * {
                        node_info_calls_++;
                      ...
                      /*actor_worker_assignment_manager=*/nullptr) {}

  void SetUp() {
    static rpc::BasicGcsNodeInfo node_info;
  ...
  absl::flat_hash_map<NodeID, rpc::BasicGcsNodeInfo> node_info_;
...
TEST_F(ClusterTaskManagerTest, TaskUnschedulableTest) {
  ...
  task_spec.GetMutableMessage()
      ...
      ->add_nodes(NodeID::FromRandom().Binary());
  task_spec.GetMutableMessage()
      .mutable_scheduling_strategy()
      ->mutable_node_affinity_scheduling_strategy()
      ->set_soft(false);
...
TEST_F(ClusterTaskManagerTest, ReschedulingByWorkerRegistrationTimeoutsTest) {
  RayConfig::instance().initialize(
      R"({"rescheduling_by_worker_registration_timeouts": true})");
  RayConfig::instance().initialize(R"({"worker_registration_timeouts_limit": 1})");
  // Assume worker pool has detected 2 worker registration timeouts in the current time
  // window.
  pool_.past_worker_registration_timeouts_count = 2;
  ...
  // Because worker pool has detected too many worker registration timeouts, this task
  // should fail to be dispatched, even though the local node has enough resources. But
  // because there is no available remote node, rescheduling should fail and the task
  // still stays at the local node.
  ASSERT_FALSE(callback_occurred);
  ASSERT_EQ(leased_workers_.size(), 0);
  ASSERT_EQ(pool_.workers.size(), 1);

  // Add a remote node with enough resources.
  auto remote_node_id = NodeID::FromRandom();
  AddNode(remote_node_id, /*num_cpus=*/4, /*num_gpus=*/0, /*memory=*/64);

  // Try scheduling the pending task.
  task_manager_.ScheduleAndDispatchTasks();

  // This task was rescheduled to the remote node.
  ASSERT_TRUE(callback_occurred);
  ASSERT_EQ(leased_workers_.size(), 0);
  ASSERT_EQ(pool_.workers.size(), 1);

  // Now assume the worker pool detects zero timeout.
  pool_.past_worker_registration_timeouts_count = 0;

  // Queue and schedule a new task.
  RayTask task2 = CreateTask({{ray::kCPU_ResourceLabel, 4}});
  reply.Clear();
  callback_occurred = false;
  task_manager_.QueueAndScheduleTask(task2, false, &reply, callback);

  // This task can be dispatched to a local worker.
  ASSERT_TRUE(callback_occurred);
  ASSERT_EQ(leased_workers_.size(), 1);
  ASSERT_EQ(pool_.workers.size(), 0);
  ASSERT_EQ(node_info_calls_, 0);
