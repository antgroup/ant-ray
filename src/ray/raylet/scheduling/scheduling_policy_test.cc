...
TEST_F(SchedulingPolicyTest, AvoidSchedulingCPURequestsOnGPUNodes) {
...

  {
    ...
    const TaskRequest req = ResourceMapToTaskRequest(map, {{"CPU", 1}});
    const int to_schedule =
        raylet_scheduling_policy::SchedulingPolicy(local_node, nodes)
            .HybridPolicy(ResourceMapToTaskRequest(map, {{"CPU", 1}}), 0.51, false, true,
                          [](auto) { return true; }, true);
    ASSERT_EQ(to_schedule, remote_node);
  }
  {
    // A GPU request should be scheduled on a GPU node.
    const TaskRequest req = ResourceMapToTaskRequest(map, {{"GPU", 1}});
    const int to_schedule =
        raylet_scheduling_policy::SchedulingPolicy(local_node, nodes)
            .HybridPolicy(req, 0.51, false, true, [](auto) { return true; }, true);
    ASSERT_EQ(to_schedule, local_node);
  }
  {
  ...
  }
  {
    ...
    const TaskRequest req = ResourceMapToTaskRequest(map, {{"CPU", 1}, {"GPU", 1}});
    const int to_schedule =
        raylet_scheduling_policy::SchedulingPolicy(local_node, nodes)
            .HybridPolicy(req, 0.51, false, true, [](auto) { return true; }, true);
    ASSERT_EQ(to_schedule, local_node);
...
TEST_F(SchedulingPolicyTest, NonGpuNodePreferredSchedulingTest) {
  ...
  TaskRequest req = ResourceMapToTaskRequest(map, {{"CPU", 1}});
  int to_schedule = raylet_scheduling_policy::SchedulingPolicy(local_node, nodes)
                        .HybridPolicy(req, 0.51, false, true, [](auto) { return true; },
                                      /*gpu_avoid_scheduling*/ true);
  ASSERT_EQ(to_schedule, remote_node_1);

  req = ResourceMapToTaskRequest(map, {{"CPU", 3}});
  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local_node, nodes)
                    .HybridPolicy(req, 0.51, false, true, [](auto) { return true; },
                                  /*gpu_avoid_scheduling*/ true);
  ASSERT_EQ(to_schedule, remote_node_2);

  req = ResourceMapToTaskRequest(map, {{"CPU", 1}, {"GPU", 1}});
  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local_node, nodes)
                    .HybridPolicy(req, 0.51, false, true, [](auto) { return true; },
                                  /*gpu_avoid_scheduling*/ true);
  ASSERT_EQ(to_schedule, local_node);

  req = ResourceMapToTaskRequest(map, {{"CPU", 2}});
  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local_node, nodes)
                    .HybridPolicy(req, 0.51, false, true, [](auto) { return true; },
                                  /*gpu_avoid_scheduling*/ true);
  ASSERT_EQ(to_schedule, remote_node_1);
...
TEST_F(SchedulingPolicyTest, NodeAffinityPolicyTest) {
  StringIdMap map;
  TaskRequest req = ResourceMapToTaskRequest(map, {{"CPU", 1}});
  auto spread_threshold = RayConfig::instance().scheduler_spread_threshold();
  auto is_node_available = [](int64_t) { return true; };

  absl::flat_hash_map<int64_t, Node> nodes;
  int64_t local = 0;
  int64_t unavailable = 1;
  int64_t infeasible = 2;
  int64_t not_exist = 3;

  nodes.emplace(local, CreateNodeResources(20, 20, 0, 0, 0, 0));
  // Unavailable node
  nodes.emplace(unavailable, CreateNodeResources(0, 20, 0, 0, 0, 0));
  // Infeasible node
  nodes.emplace(infeasible, CreateNodeResources(0, 0, 0, 0, 0, 0));

  auto to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                         .NodeAffinityPolicy(req, spread_threshold, false, false,
                                             is_node_available, {local}, false);
  ASSERT_EQ(to_schedule, local);

  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                    .NodeAffinityPolicy(req, spread_threshold, false, false,
                                        is_node_available, {unavailable}, false);
  // Prefer the specified node even if it's not available right now.
  ASSERT_EQ(to_schedule, -1);

  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                    .NodeAffinityPolicy(req, spread_threshold, false, false,
                                        is_node_available, {unavailable}, true);
  // Prefer the specified node even if it's not available right now.
  ASSERT_EQ(to_schedule, local);

  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                    .NodeAffinityPolicy(req, spread_threshold, false, false,
                                        is_node_available, {infeasible}, false);
  // The task is unschedulable since soft is false.
  ASSERT_TRUE(to_schedule == -1);

  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                    .NodeAffinityPolicy(req, spread_threshold, false, false,
                                        is_node_available, {infeasible}, true);
  // The task is scheduled somewhere else since soft is true.
  ASSERT_EQ(to_schedule, local);

  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                    .NodeAffinityPolicy(req, spread_threshold, false, false,
                                        is_node_available, {not_exist}, false);
  // The task is unschedulable since soft is false.
  ASSERT_TRUE(to_schedule == -1);

  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                    .NodeAffinityPolicy(req, spread_threshold, false, false,
                                        is_node_available, {not_exist}, true);
  // The task is scheduled somewhere else since soft is true.
  ASSERT_EQ(to_schedule, local);
  ...
  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                    .NodeAffinityPolicy(req, spread_threshold, false, false,
                                        is_node_available, {local}, false, true);
  ASSERT_EQ(to_schedule, -1);

  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                    .NodeAffinityPolicy(req, spread_threshold, false, false,
                                        is_node_available, {local}, true, true);
  ASSERT_EQ(to_schedule, local);

  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                    .NodeAffinityPolicy(req, spread_threshold, false, false,
                                        is_node_available, {unavailable}, false, true);
  ASSERT_EQ(to_schedule, local);

  to_schedule = raylet_scheduling_policy::SchedulingPolicy(local, nodes)
                    .NodeAffinityPolicy(req, spread_threshold, false, false,
                                        is_node_available, {not_exist}, false, true);
  ASSERT_EQ(to_schedule, local);
}
