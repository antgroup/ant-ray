...
class MockWorkerClient : public rpc::CoreWorkerClientInterface {
 public:
  ...
  void AddDumpedUrl(
      const rpc::AddDumpedUrlRequest &request,
      const rpc::ClientCallback<rpc::AddDumpedUrlReply> &callback) override {
    dumped_object_urls.emplace(ObjectID::FromBinary(request.object_id()),
                               request.dumped_url());
    dumped_url_callbacks.push_back(callback);
  }

  bool ReplyAddDumpedUrl(Status status = Status::OK()) {
    if (dumped_url_callbacks.empty()) {
      return false;
    }
    auto callback = dumped_url_callbacks.front();
    auto reply = rpc::AddDumpedUrlReply();
    callback(status, reply);
    dumped_url_callbacks.pop_front();
    return true;
  }
  ...
  std::unordered_map<ObjectID, std::string> dumped_object_urls;
  std::deque<rpc::ClientCallback<rpc::AddSpilledUrlReply>> spilled_url_callbacks;
  std::deque<rpc::ClientCallback<rpc::AddDumpedUrlReply>> dumped_url_callbacks;
};
...
class MockIOWorkerClient : public rpc::CoreWorkerClientInterface {
 public:
  ...
  void DumpObjects(const rpc::DumpObjectsRequest &request,
                   const rpc::ClientCallback<rpc::DumpObjectsReply> &callback) override {
    dump_callbacks.push_back(callback);
  }

  bool ReplyDumpObjects(std::vector<std::string> urls, Status status = Status::OK()) {
    if (dump_callbacks.size() == 0) {
      return false;
    }
    auto dump_callback = dump_callbacks.front();
    auto reply = rpc::DumpObjectsReply();
    if (status.ok()) {
      for (const auto &url : urls) {
        reply.add_dumped_objects_url(url);
      }
    }
    dump_callback(status, reply);
    dump_callbacks.pop_front();
    return true;
  }

  void LoadDumpedObjects(
      const rpc::LoadDumpedObjectsRequest &request,
      const rpc::ClientCallback<rpc::LoadDumpedObjectsReply> &callback) override {
    load_dumped_callbacks.push_back(callback);
  }

  bool ReplyLoadDumpedObjects(int64_t bytes_loaded, Status status = Status::OK()) {
    rpc::LoadDumpedObjectsReply reply;
    reply.set_bytes_loaded_total(bytes_loaded);
    if (load_dumped_callbacks.size() == 0) {
      return false;
    };
    auto callback = load_dumped_callbacks.front();
    callback(status, reply);
    load_dumped_callbacks.pop_front();
    return true;
  }
  ...
  std::list<rpc::ClientCallback<rpc::DumpObjectsReply>> dump_callbacks;
  std::list<rpc::ClientCallback<rpc::LoadDumpedObjectsReply>> load_dumped_callbacks;
...
class MockIOWorkerPool : public IOWorkerPoolInterface {
 public:
  ...
  MOCK_METHOD1(PushDumpWorker, void(const std::shared_ptr<WorkerInterface> &worker));

  MOCK_METHOD1(PushLoadWorker, void(const std::shared_ptr<WorkerInterface> &worker));
  ...
  void PopLoadWorker(
      std::function<void(std::shared_ptr<WorkerInterface>)> callback) override {
    loading_callbacks.push_back(callback);
  }
  ...
  bool LoadWorkerPushed() {
    if (loading_callbacks.size() == 0) {
      return false;
    }
    const auto callback = loading_callbacks.front();
    callback(io_worker);
    loading_callbacks.pop_front();
    return true;
  }

  std::list<std::function<void(std::shared_ptr<WorkerInterface>)>> restoration_callbacks;
  std::list<std::function<void(std::shared_ptr<WorkerInterface>)>> loading_callbacks;
...
TEST_F(LocalObjectManagerTest, TestExplicitDump) {
  std::vector<ObjectID> object_ids;
  ...
  for (size_t i = 0; i < free_objects_batch_size; i++) {
    ...
    auto object =
        std::make_unique<RayObject>(data_buffer, nullptr, std::vector<ObjectID>());
    object_sizes.push_back(object->GetSize());
    objects.push_back(std::move(object));
  }
  manager.PinObjects(object_ids, std::move(objects), owner_address);

  int num_times_fired = 0;
  // The test class has mocked the inner async calls to sync.
  manager.DumpObjects(object_ids, object_sizes, owner_address,
                      [&](const Status &status) mutable {
                        ASSERT_TRUE(status.ok());
                        num_times_fired++;
                      });
  // num_times_fired will be modified after getting the reply.
  ASSERT_EQ(num_times_fired, 0);

  // NOTE: EXPECT_CALL must be used before the calls are actually made.
  // The `PushDumpWorker` will actually be called exactly once in `ReplyDumpObjects`.
  EXPECT_CALL(worker_pool, PushDumpWorker(_)).Times(1);
  std::vector<std::string> urls;
  for (size_t i = 0; i < object_ids.size(); i++) {
    urls.push_back(BuildURL("url" + std::to_string(i)));
  }
  ASSERT_TRUE(worker_pool.io_worker_client->ReplyDumpObjects(urls));
  for (size_t i = 0; i < object_ids.size(); i++) {
    ASSERT_TRUE(owner_client->ReplyAddDumpedUrl());
  }
  ASSERT_EQ(num_times_fired, 1);
  for (size_t i = 0; i < object_ids.size(); i++) {
    ASSERT_EQ(owner_client->dumped_object_urls[object_ids[i]], urls[i]);
  }
}

TEST_F(LocalObjectManagerTest, TestDuplicateDump) {
  std::vector<ObjectID> object_ids;
  std::vector<uint64_t> object_sizes;
  std::vector<std::unique_ptr<RayObject>> objects;
  rpc::Address owner_address;
  ...
  // Spill the same objects the second time to test a dumping object won't be dumped
  // again. Because these objects are already in the dumping queue, the request will be
  // considered duplicated and denied.
  manager.DumpObjects(object_ids, object_sizes, owner_address,
                      [&](const Status &status) mutable { ASSERT_TRUE(!status.ok()); });
  ...
  // Dump dumplicate objects the third time after they're confirmed dumped to test that
  // a dumped object (instead of a dumping object) won't be dumped again.
  manager.DumpObjects(object_ids, object_sizes, owner_address,
                      [&](const Status &status) mutable { ASSERT_TRUE(!status.ok()); });
}

TEST_F(LocalObjectManagerTest, TestDumpError) {
  // Check that we can dump an object again if there was a transient error
  // during the first attempt.
  rpc::Address owner_address;
  owner_address.set_worker_id(WorkerID::FromRandom().Binary());
  ...
  auto object = std::make_unique<RayObject>(std::move(data_buffer), nullptr,
                                            std::vector<ObjectID>());
  const uint64_t object_size = object->GetSize();
  std::vector<std::unique_ptr<RayObject>> objects;
  objects.push_back(std::move(object));
  manager.PinObjects({object_id}, std::move(objects), owner_address);

  int num_times_fired = 0;
  manager.DumpObjects({object_id}, {object_size}, owner_address,
                      [&](const Status &status) mutable {
                        ASSERT_FALSE(status.ok());
                        num_times_fired++;
                      });

  // Return an error from the IO worker during dump.
  EXPECT_CALL(worker_pool, PushDumpWorker(_));
  ASSERT_TRUE(
      worker_pool.io_worker_client->ReplyDumpObjects({}, Status::IOError("error")));
  ASSERT_FALSE(owner_client->ReplyAddDumpedUrl());
  ASSERT_EQ(num_times_fired, 1);
  ASSERT_EQ(owner_client->dumped_object_urls.count(object_id), 0);

  // Try to dump the same object again.
  manager.DumpObjects({object_id}, {object_size}, owner_address,
                      [&](const Status &status) mutable {
                        ASSERT_TRUE(status.ok());
                        num_times_fired++;
  ...
  std::string url = BuildURL("url");
  EXPECT_CALL(worker_pool, PushDumpWorker(_));
  ASSERT_TRUE(worker_pool.io_worker_client->ReplyDumpObjects({url}));
  ASSERT_TRUE(owner_client->ReplyAddDumpedUrl());
  ASSERT_EQ(owner_client->dumped_object_urls[object_id], url);
  ASSERT_EQ(num_times_fired, 2);
}

TEST_F(LocalObjectManagerTest, TestLoadDumpedObject) {
  // First, dump objects.
  std::vector<ObjectID> object_ids;
  std::vector<std::unique_ptr<RayObject>> objects;
  std::vector<uint64_t> object_sizes;
  ...
  std::vector<std::string> urls;
  for (size_t i = 0; i < object_ids.size(); i++) {
    ASSERT_TRUE(manager.GetDumpedObjectURL(object_ids[i]).empty());
    urls.push_back(BuildURL("url" + std::to_string(i)));
  }
  ASSERT_TRUE(worker_pool.io_worker_client->ReplyDumpObjects(urls));
  for (size_t i = 0; i < object_ids.size(); i++) {
    ASSERT_FALSE(manager.GetDumpedObjectURL(object_ids[i]).empty());
  }
  for (size_t i = 0; i < object_ids.size(); i++) {
    ASSERT_TRUE(owner_client->ReplyAddDumpedUrl());
  }
  ...
  const auto url = urls[0];
  int num_times_fired = 0;
  EXPECT_CALL(worker_pool, PushLoadWorker(_));
  // Subsequent calls should be deduped, so that only one callback should be fired.
  for (int i = 0; i < 10; i++) {
    manager.LoadDumpedObject(object_id, url, [&](const Status &status) {
      ASSERT_TRUE(status.ok());
      num_times_fired++;
    });
  }
  ...
  // When restore workers are pushed, the request should be dedupped.
  for (int i = 0; i < 10; i++) {
    worker_pool.LoadWorkerPushed();
    ASSERT_EQ(num_times_fired, 0);
  }
  worker_pool.io_worker_client->ReplyLoadDumpedObjects(10);
  // The restore should've been invoked.
  ASSERT_EQ(num_times_fired, 1);
}
