...
using raylet::ActorWorkerAssignmentManager;
class MockActorWorkerAssignmentManager : public ActorWorkerAssignmentManager {
 public:
  using ActorWorkerAssignmentManager::ActorWorkerAssignmentManager;
  bool IsValid(const Process &process) override {
    if (mock_process_invalid_) {
      return false;
    }
    return ActorWorkerAssignmentManager::IsValid(process);
  }
  bool mock_process_invalid_ = false;
};
...
class WorkerPoolMock : public WorkerPool {
 public:
  explicit WorkerPoolMock(
      instrumented_io_context &io_service, const WorkerCommandMap &worker_commands,
      ...
      : WorkerPool(io_service, NodeID::FromRandom(), "", POOL_SIZE_SOFT_LIMIT, 0,
                   ...
                   actor_worker_assignment_manager, ""),
  ...
  int GetMaximumStartupConcurrency(const Language &language, bool for_actor) override {
    RAY_UNUSED(language);
    RAY_UNUSED(for_actor);
  ...
  int NumIdleWorkersForActorCreationTasks() const {
    int total = 0;
    for (auto &state_entry : states_by_lang_) {
      total += state_entry.second.idle_actor_workers.size();
    }
    return total;
  }

  int NumIdleWorkersForNormalTasks() const {
    int total = 0;
    for (auto &state_entry : states_by_lang_) {
      total += state_entry.second.idle_normal_task_workers.size();
    }
    return total;
  }
  ...
  bool SetStartingWorkerProcessExited(const Process &proc, bool exited) {
    auto &state = states_by_lang_.find(Language::PYTHON)->second;
    auto proc_iter = state.starting_worker_processes.find(proc);
    if (proc_iter != state.starting_worker_processes.end()) {
      proc_iter->second.exited = exited;
      return true;
    }
    return false;
  }
...
class WorkerPoolTest : public ::testing::Test {
 public:
  ...
  std::shared_ptr<WorkerInterface> CreateWorker(
      ...
      const JobID &job_id = JOB_ID, bool for_actor = false,
    ...
    worker->SetIsForActor(for_actor);
    return worker;
  }

  std::shared_ptr<WorkerInterface> CreateSpillWorker(Process proc) {
    return CreateWorker(proc, Language::PYTHON, JobID::Nil(), /*for_actor=*/false,
  ...
  void SetWorkerCommands(const WorkerCommandMap &worker_commands) {
    actor_worker_assignment_manager_ =
        std::make_shared<MockActorWorkerAssignmentManager>(io_service_);
  ...
  void TestStartupWorkerProcessCount(Language language, int num_workers_per_process) {
    ...
    for (int i = 0; i < desired_initial_worker_process_count; i++) {
      worker_pool_->StartWorkerProcess(language, rpc::WorkerType::WORKER, JOB_ID,
                                       num_workers_per_process, /*for_actor=*/false);
 ...
 protected:
  instrumented_io_context io_service_;
  std::shared_ptr<MockActorWorkerAssignmentManager> actor_worker_assignment_manager_;
...
TEST_F(WorkerPoolTest, HandleWorkerRegistration) {
  Process proc =
      worker_pool_->StartWorkerProcess(Language::JAVA, rpc::WorkerType::WORKER, JOB_ID,
                                       NUM_WORKERS_PER_PROCESS_JAVA, false);
...
TEST_F(WorkerPoolTest, StartWorkerWithDynamicOptionsCommand) {
  ...
  if (RayConfig::instance().gcs_task_scheduling_enabled()) {
    job_config.set_java_worker_process_default_memory_units(/*250m*/ 250 * 1024 * 1024);
    job_config.set_java_heap_fraction(0.8);
  }
  worker_pool_->HandleJobStarted(JOB_ID, job_config);

  actor_worker_assignment_manager_->AssignActor(task_spec.TaskId(),
                                                UniqueID::FromRandom());
  ...
  if (RayConfig::instance().gcs_task_scheduling_enabled()) {
    expected_command.insert(expected_command.end(), {"-Dray.maximum-memory-mb=250m"});
  }
  ...
  expected_command.push_back("--runtime-env-hash=0");
...
TEST_F(WorkerPoolTest, PopWorkerMultiTenancy) {
  if (RayConfig::instance().gcs_task_scheduling_enabled()) {
    // Skip this case when gcs task scheduling enabled since
    // it's hard to mock the task scheduling.
    return;
  }
  ...
  for (auto job_id : job_ids) {
    for (int i = 0; i < 2; i++) {
      const bool for_actor = (i == 0);
      auto worker =
          CreateWorker(Process::CreateNewDummy(), Language::PYTHON, job_id, for_actor);
      worker_pool_->PushWorker(worker);
    }
  }
...
TEST_F(WorkerPoolTest, HandleIOWorkersPushPop) {
  ...
  for (const auto &worker : spill_workers) {
    auto proc = worker_pool_->StartWorkerProcess(
        rpc::Language::PYTHON, rpc::WorkerType::SPILL_WORKER, JobID::Nil(),
  ...
  for (const auto &worker : restore_workers) {
    auto proc = worker_pool_->StartWorkerProcess(
        rpc::Language::PYTHON, rpc::WorkerType::RESTORE_WORKER, JobID::Nil(),
        /*workers_to_start=*/1, /*for_actor=*/false);
    worker_pool_->RegisterWorker(worker, proc.GetId(), proc.GetId(), [](Status, int) {});
    worker_pool_->OnWorkerStarted(worker);
...
TEST_F(WorkerPoolTest, NoPopOnCrashedWorkerProcess) {
  ...
  Process proc =
      worker_pool_->StartWorkerProcess(Language::JAVA, rpc::WorkerType::WORKER, JOB_ID,
                                       NUM_WORKERS_PER_PROCESS_JAVA, /*for_actor=*/false);
...
TEST_F(WorkerPoolTest, PopWorkerWhenGcsTaskSchedulingEnable) {
  /// This case is used to test the functionality that it pops workers from
  /// different idle queues. When gcs task scheduling enabled, a normal task
  /// shouldn't be assigned to an actor worker.
  if (!RayConfig::instance().gcs_task_scheduling_enabled()) {
    return;
  }

  for (int i = 0; i < 3; ++i) {
    auto worker_for_actor_creation =
        CreateWorker(Process::CreateNewDummy(), Language::PYTHON, JOB_ID,
                     /*for_actor=*/true);
    worker_pool_->PushWorker(worker_for_actor_creation);
    auto worker_for_normal_task =
        CreateWorker(Process::CreateNewDummy(), Language::PYTHON, JOB_ID,
                     /*for_actor=*/false);
    worker_pool_->PushWorker(worker_for_normal_task);
  }

  ASSERT_EQ(worker_pool_->NumIdleWorkersForActorCreationTasks(), 3);
  ASSERT_EQ(worker_pool_->NumIdleWorkersForNormalTasks(), 3);

  {
    // Pop a actor creation task worker.
    auto task_id = TaskID::ForDriverTask(JOB_ID);
    auto actor_id = ActorID::Of(JOB_ID, task_id, 1);
    auto actor_creation_task_spec =
        ExampleTaskSpec(ActorID::Nil(), Language::PYTHON, JOB_ID, actor_id, {}, task_id);
    actor_worker_assignment_manager_->AssignActor(actor_creation_task_spec.TaskId(),
                                                  UniqueID::FromRandom());
    auto worker = worker_pool_->PopWorker(actor_creation_task_spec);
    // This should be false since it's starting a worker process for
    // it under gcs new scheduling mode.
    ASSERT_FALSE(worker);
    ASSERT_EQ(worker_pool_->NumIdleWorkersForActorCreationTasks(), 3);
    ASSERT_EQ(worker_pool_->NumIdleWorkersForNormalTasks(), 3);
  }
  ...
  {
    // Pop a normal task worker.
    auto normal_task_spec =
        ExampleTaskSpec(ActorID::Nil(), Language::PYTHON, JOB_ID, ActorID::Nil());
    auto worker = worker_pool_->PopWorker(normal_task_spec);
    ASSERT_TRUE(worker);
    ASSERT_EQ(worker_pool_->NumIdleWorkersForActorCreationTasks(), 3);
    ASSERT_EQ(worker_pool_->NumIdleWorkersForNormalTasks(), 2);
  }
}
...
TEST_F(WorkerPoolTest, TestWorkerCapping) {
  ...
  for (int i = 0; i < num_workers; i++) {
    Process proc = worker_pool_->StartWorkerProcess(
        Language::PYTHON, rpc::WorkerType::WORKER, job_id, /*workers_to_start=*/1,
        /*for_actor=*/true);
  ...
  // Start two IO workers. These don't count towards the limit.
  {
    RAY_LOG(INFO) << "XXX";
    Process proc = worker_pool_->StartWorkerProcess(
        Language::PYTHON, rpc::WorkerType::SPILL_WORKER, job_id, 1, false);
    auto worker = CreateSpillWorker(Process());
    RAY_CHECK_OK(worker_pool_->RegisterWorker(worker, proc.GetId(), proc.GetId(),
                                              [](Status, int) {}));
    worker_pool_->OnWorkerStarted(worker);
  ...
  }
  {
    RAY_LOG(INFO) << "YYY";
    Process proc = worker_pool_->StartWorkerProcess(
        Language::PYTHON, rpc::WorkerType::RESTORE_WORKER, job_id, 1, false);
...
TEST_F(WorkerPoolTest, TestRegisterWorkerAfterJobFinished) {
  ...
  for (const auto &worker : workers) {
    auto status = worker_pool_->RegisterWorker(worker, proc.GetId(), proc.GetId(),
                                               [](Status, int) {});
    ASSERT_TRUE(status.IsInvalid());
  }

  // It shouldn't start a new worker process as job has finished.
  proc = worker_pool_->StartWorkerProcess(Language::JAVA, rpc::WorkerType::WORKER, JOB_ID,
                                          NUM_WORKERS_PER_PROCESS_JAVA, false);
  ASSERT_FALSE(proc.IsValid());
}

TEST_F(WorkerPoolTest, ProcessInvalidWhenRegisterWorker) {
  // Mock process invalid.
  actor_worker_assignment_manager_->mock_process_invalid_ = true;

  Process proc =
      worker_pool_->StartWorkerProcess(Language::JAVA, rpc::WorkerType::WORKER, JOB_ID,
                                       NUM_WORKERS_PER_PROCESS_JAVA, /*for_actor=*/true);
  ASSERT_TRUE(proc.IsValid());
  ...
  // These actor workers shouldn't be registered to the worker pool as the process state
  // inside scheduling policy is invalid.
  for (const auto &worker : workers) {
    auto status = worker_pool_->RegisterWorker(worker, proc.GetId(), proc.GetId(),
                                               [](Status, int) {});
    ASSERT_TRUE(status.IsInvalid());
  ...
  // These normal workers could be registered to the worker pool even if the process state
  // inside scheduling policy is invalid.
  for (const auto &worker : workers) {
    auto status = worker_pool_->RegisterWorker(worker, proc.GetId(), proc.GetId(),
                                               [](Status, int) {});
    ASSERT_FALSE(status.IsInvalid());
  }
}
...
TEST_F(WorkerPoolTest, StartWorkWithDifferentShimPid) {
  ...
  actor_jvm_options.insert(
      ...
      {"-Dmy-actor.hello=foo", "-Dmy-actor.world=bar", "-Xmx2g", "-Xms1g"});
  if (!RayConfig::instance().gcs_task_scheduling_enabled()) {
    // This test is for Ray community
    auto task_id = TaskID::ForDriverTask(JOB_ID);
    auto actor_id = ActorID::Of(JOB_ID, task_id, 1);
    ...
    TaskSpecification java_task_spec = ExampleTaskSpec(
        ActorID::Nil(), Language::JAVA, JOB_ID, actor_id, actor_jvm_options, task_id);
    // It is for gcs schedule
    UniqueID assignment_id = UniqueID::FromRandom();
    actor_worker_assignment_manager_->AssignActor(java_task_spec.TaskId(), assignment_id);
    ASSERT_EQ(worker_pool_->PopWorker(java_task_spec), nullptr);
    last_process = worker_pool_->LastStartedWorkerProcess();
    pid_t java_shim_pid = last_process.GetId();
    pid_t java_pid = java_shim_pid + 1000;
    ...
    RAY_CHECK_OK(worker_pool_->RegisterWorker(java_worker, java_pid, java_shim_pid,
                                              [](Status, int) {}));
    auto new_task_id = TaskID::FromRandom(JobID::FromInt(1));
    auto new_actor_id = ActorID::Of(JOB_ID, new_task_id, 1);
    TaskSpecification new_java_task_spec =
        ExampleTaskSpec(ActorID::Nil(), Language::JAVA, JOB_ID, new_actor_id,
                        actor_jvm_options, new_task_id);
    actor_worker_assignment_manager_->AssignActor(new_java_task_spec.TaskId(),
                                                  assignment_id);
    worker_pool_->PushWorker(java_worker);
    ASSERT_TRUE(worker_pool_->PopWorker(new_java_task_spec) != nullptr);
  }
}
...
TEST_F(WorkerPoolTest, TestIOWorkerFailureAndSpawn) {
  ...
  for (int i = 0; i < MAX_IO_WORKER_SIZE; i++) {
    processes.push_back(worker_pool_->StartWorkerProcess(
        rpc::Language::PYTHON, rpc::WorkerType::SPILL_WORKER, JobID::Nil(),
        /*workers_to_start=*/1, /*for_actor=*/false));
  }
  ...
  auto proc2 = worker_pool_->StartWorkerProcess(
      rpc::Language::PYTHON, rpc::WorkerType::SPILL_WORKER, JobID::Nil(),
      /*workers_to_start=*/1, /*for_actor=*/false);
  worker_pool_->RegisterWorker(worker2, proc2.GetId(), proc2.GetId(), [](Status, int) {});
  ...
  auto proc3 = worker_pool_->StartWorkerProcess(
      rpc::Language::PYTHON, rpc::WorkerType::SPILL_WORKER, JobID::Nil(),
      /*workers_to_start=*/1, /*for_actor=*/false);
  worker_pool_->RegisterWorker(worker3, proc3.GetId(), proc3.GetId(), [](Status, int) {});
...
/// ANT-INTERNAL below
// This test checks if worker pool records the worker registration timeouts correctly.
TEST_F(WorkerPoolTest, RecordWorkerRegistrationTimeouts) {
  RayConfig::instance().initialize(R"({"worker_register_timeout_seconds": 1})");
  RayConfig::instance().initialize(R"({"worker_registration_timeouts_window_s": 3})");

  // The test body and worker pool should not share the same thread. Otherwise, the async
  // timer (for worker registration timeouts) in worker pool can not be scheduled because
  // the test body never yields.
  std::unique_ptr<std::thread> thread_io_service;
  thread_io_service.reset(new std::thread([this] {
    instrumented_io_context::work work(io_service_);
    io_service_.run();
  }));

  // All operations on worker pool have to be executed by `thread_io_service`.
  io_service_.post([this]() {
    worker_pool_->StartWorkerProcess(Language::PYTHON, rpc::WorkerType::WORKER, JOB_ID, 1,
                                     false);
  });

  auto starting_worker_erased = [this]() {
    auto promise = std::make_shared<std::promise<bool>>();
    io_service_.post([promise, this]() {
      promise->set_value(worker_pool_->NumWorkerProcessesStarting() == 0);
    });
    return promise->get_future().get();
  };
  // Because the starting worker process has not registered by timeout, make sure it is
  // erased.
  EXPECT_TRUE(WaitForCondition(starting_worker_erased, 10 * 1000));

  auto get_timeouts_count = [this]() {
    auto promise = std::make_shared<std::promise<uint32_t>>();
    io_service_.post([promise, this]() {
      promise->set_value(worker_pool_->GetPastWorkerRegistrationTimeoutsCount());
    });
    return promise->get_future().get();
  };
  // Worker pool successfully recorded the above worker registration timeout.
  ASSERT_EQ(get_timeouts_count(), 1);

  io_service_.post([this]() {
    auto proc = worker_pool_->StartWorkerProcess(
        Language::PYTHON, rpc::WorkerType::WORKER, JOB_ID, 1, false);
    // Mock the scenario that the starting worker process exits before registration
    // timeout.
    ASSERT_TRUE(worker_pool_->SetStartingWorkerProcessExited(proc, true));
  });

  EXPECT_TRUE(WaitForCondition(starting_worker_erased, 10 * 1000));
  // Because worker pool only records registration timeouts caused by system behavior,
  // this second timeout is not recorded. There is only the first registration timeout
  // within the moving window.
  ASSERT_EQ(get_timeouts_count(), 1);

  auto zero_timeout = [&get_timeouts_count]() { return get_timeouts_count() == 0; };
  // After `worker_registration_timeouts_window_s`, the moving window now contains no
  // timeout.
  EXPECT_TRUE(WaitForCondition(zero_timeout, 3 * 1000));
