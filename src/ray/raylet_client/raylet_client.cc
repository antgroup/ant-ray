...
raylet::RayletClient::RayletClient(
    ...
    pid_t worker_shim_pid, std::string *node_name)
  ...
  auto message = protocol::CreateRegisterClientRequest(
      ...
      /*port=*/0, fbb.CreateString(*serialized_job_config), fbb.CreateString(cmdline));
  fbb.Finish(message);
  ...
  *node_name = reply_message->raylet_name()->str();
...
void raylet::RayletClient::RequestWorkerLease(
    ...
    const int64_t backlog_size, const UniqueID &worker_process_id) {
  rpc::RequestWorkerLeaseRequest request;
  request.set_worker_process_id(worker_process_id.Binary());
...
void raylet::RayletClient::StopTasksAndBanNewTasks(
    const JobID &job_id,
    const rpc::ClientCallback<rpc::StopTasksAndBanNewTasksReply> &callback) {
  rpc::StopTasksAndBanNewTasksRequest request;
  request.set_job_id(job_id.Binary());
  request.set_timestamp(current_time_ms());
  grpc_client_->StopTasksAndBanNewTasks(request, callback);
}

void raylet::RayletClient::LiftNewTasksBan(
    const JobID &job_id, const rpc::ClientCallback<rpc::LiftNewTasksBanReply> &callback) {
  rpc::LiftNewTasksBanRequest request;
  request.set_job_id(job_id.Binary());
  request.set_timestamp(current_time_ms());
  grpc_client_->LiftNewTasksBan(request, callback);
}
