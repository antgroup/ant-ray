...
class WorkerLeaseInterface {
 public:
  ...
  virtual void RequestWorkerLease(
      ...
      const UniqueID &worker_process_id = UniqueID::Nil()) = 0;
...
/// Interface for controlling task lifecycle.
/// Only used when L1 fault tolerance is enabled. Abstract for testing.
class TaskControlInterface {
 public:
  virtual void StopTasksAndBanNewTasks(
      const JobID &job_id,
      const rpc::ClientCallback<ray::rpc::StopTasksAndBanNewTasksReply> &callback) = 0;

  virtual void LiftNewTasksBan(
      const JobID &job_id,
      const rpc::ClientCallback<ray::rpc::LiftNewTasksBanReply> &callback) = 0;
...
class RayletClient : public RayletClientInterface {
 public:
  ...
  void RequestWorkerLease(
      const ray::TaskSpecification &resource_spec, bool grant_or_reject,
      const ray::rpc::ClientCallback<ray::rpc::RequestWorkerLeaseReply> &callback,
      const int64_t backlog_size, const UniqueID &worker_process_id) override;
  ...
  void StopTasksAndBanNewTasks(
      const JobID &job_id,
      const ray::rpc::ClientCallback<ray::rpc::StopTasksAndBanNewTasksReply> &callback)
      override;

  void LiftNewTasksBan(
      const JobID &job_id,
      const ray::rpc::ClientCallback<ray::rpc::LiftNewTasksBanReply> &callback) override;
