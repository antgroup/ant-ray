...
#define VOID_GCS_RPC_CLIENT_METHOD(SERVICE, METHOD, grpc_client, SPECS)                \
  void METHOD(const METHOD##Request &request,                                          \
    ...
    auto operation = [request, operation_callback](GcsRpcClient *gcs_rpc_client) {     \
      RAY_UNUSED(INVOKE_GRPC_CALL(SERVICE, METHOD, request, operation_callback,        \
...
class GcsRpcClient {
 public:
  ...
  void Reset(const std::string &address, const int port,
    ...
    nodegroup_grpc_client_ = std::make_unique<GrpcClient<NodegroupInfoGcsService>>(
        address, port, client_call_manager);

    frozen_node_grpc_client_ = std::make_unique<GrpcClient<FrozenNodeGcsService>>(
        address, port, client_call_manager);
  }
  ...
  /// Put job result to GCS server.
  VOID_GCS_RPC_CLIENT_METHOD(JobInfoGcsService, PutJobData, job_info_grpc_client_, )

  /// Get job result to GCS server.
  VOID_GCS_RPC_CLIENT_METHOD(JobInfoGcsService, GetJobData, job_info_grpc_client_, )
  ...
  VOID_GCS_RPC_CLIENT_METHOD(JobInfoGcsService, NotifyDriverExit, job_info_grpc_client_, )

  /// Mark job as finished to GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(JobInfoGcsService, MarkJobFailed, job_info_grpc_client_, )

  /// Get job info from GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(JobInfoGcsService, GetJob, job_info_grpc_client_, )
  ...
  /// Update job resource requirements to GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(JobInfoGcsService, UpdateJobResourceRequirements,
  ...
  /// Get nodegroups' available quota from GCS service.
  VOID_GCS_RPC_CLIENT_METHOD(JobInfoGcsService, GetAvailableQuotaOfAllNodegroups,
                             job_info_grpc_client_, )
  ...
  VOID_GCS_RPC_CLIENT_METHOD(ActorInfoGcsService, GetJobDistribution,
                             actor_info_grpc_client_, )
  ...
  VOID_GCS_RPC_CLIENT_METHOD(NodeInfoGcsService, GetAllBasicNodeInfo,
                             node_info_grpc_client_, )

  /// Get full information of all nodes from GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(NodeInfoGcsService, GetAllFullNodeInfo,
                             node_info_grpc_client_, )
  ...
  VOID_GCS_RPC_CLIENT_METHOD(NodeInfoGcsService, UpdateInternalConfig,
                             node_info_grpc_client_, )

  VOID_GCS_RPC_CLIENT_METHOD(FrozenNodeGcsService, GetAllFrozenNodes,
                             frozen_node_grpc_client_, )

  VOID_GCS_RPC_CLIENT_METHOD(FrozenNodeGcsService, ReportFreezeDone,
                             frozen_node_grpc_client_, )
  ...
  /// Get resource usage from GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(NodeResourceInfoGcsService, GetResourceUsage,
                             node_resource_info_grpc_client_, )

  /// Get cluster resources from GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(NodeResourceInfoGcsService, GetClusterResources,
                             node_resource_info_grpc_client_, )
  ...
  /// Remove placement group bundles via GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(PlacementGroupInfoGcsService, RemovePlacementGroupBundles,
                             placement_group_info_grpc_client_, )

  /// Add nodes to a nodegroup of GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(NodegroupInfoGcsService, CreateOrUpdateNodegroup,
                             nodegroup_grpc_client_, )

  /// Remove nodes from a nodegroup of GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(NodegroupInfoGcsService, RemoveNodesFromNodegroup,
                             nodegroup_grpc_client_, )

  /// Remove a nodegroup from GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(NodegroupInfoGcsService, RemoveNodegroup,
                             nodegroup_grpc_client_, )

  /// Get all the nodegroup list from GCS Service.
  VOID_GCS_RPC_CLIENT_METHOD(NodegroupInfoGcsService, GetAllNodegroups,
                             nodegroup_grpc_client_, )

  /// Release idle nodes from the specified nodegroups.
  VOID_GCS_RPC_CLIENT_METHOD(NodegroupInfoGcsService, ReleaseIdleNodes,
                             nodegroup_grpc_client_, )

  /// Pin nodes for cluster scaling down.
  VOID_GCS_RPC_CLIENT_METHOD(NodegroupInfoGcsService, PinNodesForClusterScalingDown,
                             nodegroup_grpc_client_, )
 ...
 private:
  ...
  std::unique_ptr<GrpcClient<FrozenNodeGcsService>> frozen_node_grpc_client_;
  ...
  std::unique_ptr<GrpcClient<PlacementGroupInfoGcsService>>
      placement_group_info_grpc_client_;
  std::unique_ptr<GrpcClient<NodegroupInfoGcsService>> nodegroup_grpc_client_;
