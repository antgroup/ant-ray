...
class ServerCallImpl : public ServerCall {
 public:
    ...
    if (!io_service_.stopped()) {
      io_service_.post(
          [this] {
            ...
            HandleRequestImpl();
            auto consumed = current_sys_time_ms() - start;
            if (consumed > RayConfig::instance().rpc_execution_alarm_time_ms()) {
              RAY_LOG(WARNING) << "It took " << consumed
                               << " ms to process the rpc request "
                               << request_.descriptor()->name();
            }
          },
          call_name_);
    } else {
  ...
  void HandleRequestImpl() {
    ...
    Reply *reply = google::protobuf::Arena::CreateMessage<Reply>(&arena_);
