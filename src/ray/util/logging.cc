...
long RayLog::log_rotation_max_size_ = 128 * 1024 * 1024;
long RayLog::log_rotation_file_num_ = 1;
...
std::function<void(std::string, std::string, std::string)>
    RayLog::report_event_callback_ = nullptr;
...
void WriteFailureMessage(const char *data, int size) {
  ...
  if (nullptr != data && size > 0) {
    ...
    if (RayLog::report_event_callback_ != nullptr) {
      RayLog::report_event_callback_("FATAL", "SIGNAL_ACTION_FAILED",
...
#endif
  if (is_exposed_) {
    osstream_ = new std::ostringstream();
    *osstream_ << app_name_ << " - " << file_name << ":" << line_number << ":] ";
  }
}
...
std::ostream &RayLog::ExposeStream() { return *osstream_; }
...
RayLog::~RayLog() {
  if (osstream_ != nullptr) {
    if (report_event_callback_ != nullptr) {
      report_event_callback_("FATAL", "RAY_CHECK_FAILED", osstream_->str());
    }
    delete osstream_;
    osstream_ = nullptr;
  }
