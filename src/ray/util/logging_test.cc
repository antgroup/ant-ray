...
TEST(PrintLogTest, LogDiagnoseTest) {
  // Test empty app name.
  RayLog::StartRayLog("", RayLogLevel::DEBUG);
  RAY_LOG(INFO) << "Log started.";
  RAY_LOG(INFO) << "Log ended";
  RayLog::ShutDownRayLog();
}

// This test will output large amount of logs to stderr, should be disabled in travis.
TEST(LogPerfTest, PerfTest) {
  RayLog::StartRayLog("/fake/path/to/appdire/LogPerfTest", RayLogLevel::INFO,
  ...
  std::cout << "Testing RAY_DIAGNOSE log for " << rounds << " rounds takes " << elapsed
            << " ms." << std::endl;
