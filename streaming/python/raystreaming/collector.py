...
from raystreaming.runtime.transfer import ChannelID
...
class CollectionCollector(Collector):
    ...
    def collect_by_index(self, index, value):
        self._collector_list[index].collect(message.Record(value))


class OutputCollector(Collector):
    def __init__(self, writer, channel_ids: typing.List[str],
                 target_actors: typing.List[ActorHandle],
                 partition_func: partition.Partition, edge_src_operator_id,
                 target_job_vertex_id):
        self._writer = writer
        self._channel_ids = [ChannelID(id_str) for id_str in channel_ids]
        self._target_languages = []
        self.downstream_operator_id = target_job_vertex_id
        self.id = edge_src_operator_id
        for actor in target_actors:
            if actor._ray_actor_language == Language.PYTHON:
                ...
                raise Exception("Unsupported language {}".format(
                    actor._ray_actor_language))
    ...
    def get_downstream_operator_id(self):
        return self.downstream_operator_id
