...
class Function(UDCAdapter):
    ...
    def forward_command(self, message):
        command_func = message.message.get(
            constants.OPERATOR_COMMAND_FUNC_NAME, None)
        if command_func is None:
            raise ValueError(f"Can't find function {command_func}.")
        return methodcaller(
            command_func,
            *message.message.get(constants.OPERATOR_COMMAND_FUNC_ARGS,
                                 ()))(self)
    ...
    def on_prepare(self, msg):
        logger.warning("This function is undefined.")
        return True
...
class ProcessFunction(Function):
    ...
    @abstractmethod
    def process(self, value, collector):
        """Takes an element from the input data set and process it.
...
class JoinFunction(Function):
    """
    Base interface for join functions. Join function combines
    two streams to a universal stream by a specific key.
    """

    @abstractmethod
    def join(self, left_value, right_value):
        """
        The core method of JoinFunction, combining two values into one value
        of the same type.

        Args:
            left_value: An element from left stream.
            right_value: An element from right stream.

        Returns:
            The join result value that will push to downstream.
        """
        pass
...
class MergeFunction(Function):
    """
    Base interface for merge functions. Merge function combines
    multiple streams to a universal stream by a specific key.
    """

    @abstractmethod
    def merge(self, values: List):
        """
        The core method of MergeFunction, combining
        multiple values into one value of the same type.

        Args:
            values: A data list that comes from upstreams.

        Returns:
            The merge result value that will push to downstream.
        """
        pass
...
class CollectionSourceFunction(SourceFunction):
    ...
    def fetch(self, ctx: SourceContext, checkpoint_id: int):
        for v in self.values:
            ctx.collect(v)
        self.values = []
        raise EndOfDataException("CollectionSourceFunction End")
    ...
    def fetch(self, ctx: SourceContext, checkpoint_id: int):
        ...
        with open(self.filename, "r") as f:
            ...
            while line != "":
                ...
                line = f.readline()
            self.done = True
        raise EndOfDataException("LocalFileSourceFunction End")
...
class SimpleMergeFunction(MergeFunction):
    def __init__(self, func):
        self.func = func
...
class ArrowToPandasFunction(MapFunction):
    def map(self, record_batch):
