...
class CallResult:
    ...
    @staticmethod
    def fail(payload=None, msg=None):
        return CallResult(False, CallResultEnum.FAILED, msg, payload)
...
class RemoteCallMst:
    ...
    def report_job_worker_commit(master: ActorHandle,
        ...
        result = ray.get(return_id)
        logger.info("Remote call mst: report job worker commit finish.")
        return result

    @staticmethod
    def report_job_worker_partial_barrier(
            master: ActorHandle, report: WorkerReceivePartialBarrierReport):
        logger.info("Remote call mst: report job worker commit start.")
        report_pb = remote_call_pb2.BaseWorkerCmd()
        report_pb.actor_id = report.from_actor_id
        report_pb.timestamp = int(time.time() * 1000.0)
        wk_commit = remote_call_pb2.WorkerReceivePartialBarrierReport()
        wk_commit.global_checkpoint_id = report.global_checkpoint_id
        wk_commit.partial_checkpoint_id = report.partial_checkpoint_id
        report_pb.detail.Pack(wk_commit)
        return_ids = master.reportJobWorkerReceivePartialBarrier\
            .remote(report_pb.SerializeToString())
        result = ray.get(return_ids)
        logger.info(
            "Remote call mst: report job worker partial barrier finish.")
        return result

    @staticmethod
    def report_job_worker_end_of_data(master: ActorHandle,
                                      report: EndOfDataReport):
        logger.info("Remote call mst: report job worker end of data")
        report_pb = remote_call_pb2.BaseWorkerCmd()
        report_pb.actor_id = report.from_actor_id
        end_request_pb = remote_call_pb2.EndOfDataReport()
        report_pb.detail.Pack(end_request_pb)

        return_ids = master.reportJobWorkerEndOfData\
            .remote(report_pb.SerializeToString())
        ray.get(return_ids)
        logger.info("Remote call mst: report job worker end of data finish.")

    @staticmethod
    def report_job_worker_udc_commit(
            master: ActorHandle,
            report: WorkerUnitedDistributedControllerCommitReport):
        logger.info("Remote call mst: report the start of job worker's united"
                    " distributed controller‘s commit phase to job master..")
        report_pb = remote_call_pb2.BaseWorkerCmd()
        report_pb.actor_id = report.from_actor_id
        report_pb.timestamp = int(time.time() * 1000.0)
        wk_udc_commit = common_pb2.\
            WorkerUnitedDistributedControllerCommitReport()
        # copy.
        wk_udc_commit.control_message.blocking_type = \
            report.control_message.blocking_type
        wk_udc_commit.control_message.data_payload = \
            report.control_message.data_payload
        wk_udc_commit.control_message.commit_result = \
            report.control_message.commit_result

        report_pb.detail.Pack(wk_udc_commit)
        return_ids = master.reportJobWorkerUnitedDistributedControllerCommit \
            .remote(report_pb.SerializeToString())
        result = ray.get(return_ids)
        logger.info("Remote call mst: report the start of job worker's united"
                    " distributed controller‘s commit phase to job master.")
