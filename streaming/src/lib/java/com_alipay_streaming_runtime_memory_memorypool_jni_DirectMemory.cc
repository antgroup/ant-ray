...
#include "util/streaming_memory_pool.h"
#include "util/utility.h"

JNIEXPORT jlong JNICALL
Java_com_alipay_streaming_runtime_memory_memorypool_jni_DirectMemory_createMemoryPool(
...
Java_com_alipay_streaming_runtime_memory_memorypool_jni_DirectMemory_releaseMemoryPool(
    JNIEnv *env, jobject obj, jlong pool_ptr) {
  delete reinterpret_cast<ray::streaming::StreamingMemoryPool *>(pool_ptr);
}

JNIEXPORT jobject JNICALL
Java_com_alipay_streaming_runtime_memory_memorypool_jni_DirectMemory_requestMemory(
    JNIEnv *env, jobject obj, jlong alloc_size, jlong pool_ptr) {
  jlong buff_ptr =
      Java_com_alipay_streaming_runtime_memory_memorypool_jni_DirectMemory_allocateMemory(
...
Java_com_alipay_streaming_runtime_memory_memorypool_jni_DirectMemory_allocateMemory(
  ...
  STREAMING_LOG(DEBUG) << "poo addr => " << sp << " alloc size => " << alloc_size
                       << " addr => " << reinterpret_cast<long>(buff);
  return reinterpret_cast<long>(buff);
}

JNIEXPORT void JNICALL
Java_com_alipay_streaming_runtime_memory_memorypool_jni_DirectMemory_releaseMemory__Ljava_nio_ByteBuffer_2J(
...
JNIEXPORT void JNICALL
Java_com_alipay_streaming_runtime_memory_memorypool_jni_DirectMemory_showMemoryBuffer(
    JNIEnv *env, jobject obj, jobject buffer_byte_obj, jint size) {
  uint8_t *buff = (uint8_t *)env->GetDirectBufferAddress(buffer_byte_obj);
  STREAMING_LOG(INFO) << " buffer => " << reinterpret_cast<long>(buff) << ", content =>"
                      << ray::streaming::StreamingUtility::Byte2hex(buff, size);
}
