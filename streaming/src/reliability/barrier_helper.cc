...
void StreamingBarrierHelper::SetMsgIdByBarrierId(const ObjectID &q_id,
                                                 ...
                                                 uint64_t message_id) {
  std::lock_guard<std::mutex> lock(global_barrier_mutex_);
  global_barrier_map_[barrier_id][q_id] = message_id;
}
...
StreamingStatus StreamingBarrierHelper::GetBarrierIdByLastMessageId(const ObjectID &q_id,
  ...
  if (message_queue_item->second->empty()) {
    STREAMING_LOG(WARNING) << "[Barrier] q id => " << q_id.Hex() << ", str num => "
                           << StreamingUtility::Hexqid2str(q_id.Hex()) << ", message id "
                           << message_id;
    return StreamingStatus::NoSuchItem;
  } else {
...
void StreamingBarrierHelper::AddLoggingModeBarrier(const ObjectID &q_id,
                                                   const uint64_t checkpoint_id,
                                                   const uint64_t bundle_id) {
  std::lock_guard<std::mutex> lock(logging_mode_barriers_mutex_);
  logging_mode_barriers_[q_id].push_back(std::make_pair(checkpoint_id, bundle_id));
}

std::pair<uint64_t, uint64_t> StreamingBarrierHelper::GetCurrentLoggingModeCheckpointId(
    const ObjectID &q_id) {
  std::lock_guard<std::mutex> lock(logging_mode_barriers_mutex_);
  auto it = logging_mode_barriers_.find(q_id);
  if (it == logging_mode_barriers_.end()) {
    return std::make_pair(INVALID_BARRIER_ID, INVALID_BARRIER_ID);
  }
  auto &barrier_ids = it->second;
  if (barrier_ids.empty()) {
    return std::make_pair(INVALID_BARRIER_ID, INVALID_BARRIER_ID);
  }
  return barrier_ids.back();
}

void StreamingBarrierHelper::ReleaseLoggingModeCheckpoints(
    const ObjectID &q_id, const uint64_t checkpoint_id,
    std::list<std::pair<uint64_t, uint64_t>> &removed_list) {
  std::lock_guard<std::mutex> lock(logging_mode_barriers_mutex_);
  STREAMING_LOG(INFO) << "ReleaseLoggingModeCheckpoints " << q_id;
  auto it = logging_mode_barriers_.find(q_id);
  if (it == logging_mode_barriers_.end()) {
    STREAMING_LOG(INFO) << "ReleaseLoggingModeCheckpoints 1 " << q_id;
    return;
  }
  auto &barrier_ids = it->second;
  auto target = std::find_if(barrier_ids.begin(), barrier_ids.end(),
                             [&checkpoint_id](std::pair<uint64_t, uint64_t> &pair) {
                               STREAMING_LOG(INFO) << "ReleaseLoggingModeCheckpoints 1.1 "
                                                   << checkpoint_id << " " << pair.first;
                               return checkpoint_id == pair.first;
                             });
  if (target == barrier_ids.end()) {
    STREAMING_LOG(INFO) << "ReleaseLoggingModeCheckpoints 2 " << q_id;
    return;
  }
  STREAMING_LOG(INFO) << "ReleaseLoggingModeCheckpoints 3 " << q_id;
  target++;
  removed_list.splice(removed_list.begin(), barrier_ids, barrier_ids.begin(), target);
}
}  // namespace streaming
}  // namespace ray
