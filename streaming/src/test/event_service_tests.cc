...
TEST(EventServiceTest, remove_delete_channel_event) {
  ...
  std::vector<std::shared_ptr<ProducerChannelInfo>> mock_channel_info_vec;
  channel_vec.push_back(ObjectID::FromRandom());
  std::shared_ptr<ProducerChannelInfo> mock_channel_info1 =
      std::make_shared<ProducerChannelInfo>();
  mock_channel_info1->channel_id = channel_vec.back();
  mock_channel_info_vec.push_back(mock_channel_info1);
  std::shared_ptr<ProducerChannelInfo> mock_channel_info2 =
      std::make_shared<ProducerChannelInfo>();
  channel_vec.push_back(ObjectID::FromRandom());
  ...
  for (auto &id : mock_channel_info_vec) {
    Event empty_event{id.get(), EventType::EmptyEvent, true};
    Event user_event{id.get(), EventType::UserEvent, true};
    Event flow_event{id.get(), EventType::FlowEvent, true};
