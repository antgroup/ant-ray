#include <chrono>
#include <future>

#include "data_reader.h"
#include "data_writer.h"
#include "gtest/gtest.h"

using namespace ray;
using namespace ray::streaming;

namespace ray {
namespace streaming {
class MockDataWriter : public DataWriter {
 public:
  MockDataWriter(std::shared_ptr<RuntimeContext> &runtime_context)
      : DataWriter(runtime_context) {}
  uint64_t GetSendEmptyCnt(const ObjectID &queue_id) {
    return channel_info_map_[queue_id].sent_empty_cnt;
  }
};

TEST(StreamingMockTransfer, mock_produce_consume) {
  StreamingConfig config;
  std::shared_ptr<Config> transfer_config = std::make_shared<Config>();
  STREAMING_LOG(INFO) << "config.GetStreamingBufferPoolSize(): "
                      << config.GetStreamingBufferPoolSize();
  transfer_config->Set(ConfigEnum::BUFFER_POOL_SIZE, config.GetStreamingBufferPoolSize());
  transfer_config->Set(ConfigEnum::BUFFER_POOL_MIN_BUFFER_SIZE,
                       config.GetStreamingBufferPoolMinBufferSize());
  ObjectID channel_id = ObjectID::FromRandom();
  ProducerChannelInfo producer_channel_info;
  producer_channel_info.channel_id = channel_id;
  producer_channel_info.current_bundle_id = 0;
  MockProducer producer(transfer_config, producer_channel_info);

  ConsumerChannelInfo consumer_channel_info;
  consumer_channel_info.channel_id = channel_id;
  MockConsumer consumer(transfer_config, consumer_channel_info);

  producer.CreateTransferChannel();
  uint8_t data[3] = {1, 2, 3};
  producer.ProduceItemToChannel(data, 3, 1, 1);
  uint8_t *data_consumed;
  uint32_t data_size_consumed;
  uint64_t data_seq_id;
  uint64_t ts = 0;
  consumer.ConsumeItemFromChannel(data_seq_id, data_consumed, data_size_consumed, -1, ts);
  EXPECT_EQ(data_size_consumed, 3);
  EXPECT_EQ(data_seq_id, 1);
  EXPECT_EQ(std::memcmp(data_consumed, data, 3), 0);
  consumer.NotifyChannelConsumed(1);

  auto status = consumer.ConsumeItemFromChannel(data_seq_id, data_consumed,
                                                data_size_consumed, -1, ts);
  EXPECT_EQ(status, StreamingStatus::NoSuchItem);
}

class StreamingTransferTest : public ::testing::Test {
 public:
  StreamingTransferTest() : runtime_context_(new RuntimeContext()) {
    runtime_context_->config_.SetQueueType("mock_queue");
    runtime_context_->config_.SetStreamingBufferPoolMinBufferSize(1 << 10);
    writer = std::make_shared<MockDataWriter>(runtime_context_);
    reader = std::make_shared<DataReader>(runtime_context_);
  }
  virtual ~StreamingTransferTest() { DestroyTransfer(); };

  void InitTransfer(int channel_num = 1) {
    STREAMING_LOG(INFO) << "StreamingTransferTest InitTransfer";
    for (int i = 0; i < channel_num; ++i) {
      queue_vec.push_back(ObjectID::FromRandom());
    }
    std::vector<uint64_t> channel_id_vec(queue_vec.size(), 0);
    std::vector<uint64_t> queue_size_vec(queue_vec.size(), 100000);
    std::vector<StreamingQueueInitialParameter> params(queue_vec.size());
    writer->Init(queue_vec, params, channel_id_vec, queue_size_vec);
    reader->Init(queue_vec, params, timer_interval);
  }
  void DestroyTransfer() {
    writer.reset();
    reader.reset();
  }

 protected:
  std::shared_ptr<MockDataWriter> writer;
  std::shared_ptr<DataReader> reader;
  std::vector<ObjectID> queue_vec;
  std::shared_ptr<RuntimeContext> runtime_context_;
  uint64_t min_size = 1 << 16;
  int timer_interval = -1;
};

TEST_F(StreamingTransferTest, exchange_single_channel_test) {
  InitTransfer();
  writer->Run();
  uint8_t data[4] = {1, 2, 3, 0xff};
  uint32_t data_size = 4;
  ray::streaming::MemoryBuffer buffer;
  STREAMING_CHECK(writer->GetBufferPool(queue_vec[0]));
  writer->GetBufferPool(queue_vec[0])->GetBufferBlockedTimeout(min_size, &buffer);
  std::memcpy(buffer.Data() + kMessageHeaderSize, data, data_size);
  writer->WriteMessageToBufferRing(
      queue_vec[0],
      ::ray::streaming::MemoryBuffer(buffer.Data(), kMessageHeaderSize + data_size),
      StreamingMessageType::Message);
  std::shared_ptr<StreamingReaderBundle> msg;
  reader->GetBundle(5000, msg);
  StreamingMessageBundlePtr bundle_ptr =
      StreamingMessageBundle::FromBytes(msg->MetaBuffer(), msg->DataBuffer());
  auto &message_list = bundle_ptr->GetMessageList();
  auto &message = message_list.front();
  EXPECT_EQ(std::memcmp(message->Payload(), data, data_size), 0);
}

TEST_F(StreamingTransferTest, exchange_multichannel_test) {
  int channel_num = 4;
  InitTransfer(4);
  writer->Run();
  for (int i = 0; i < channel_num; ++i) {
    uint8_t data[4] = {1, 2, 3, (uint8_t)i};
    uint32_t data_size = 4;
    writer->WriteMessageToBufferRing(queue_vec[i], data, data_size);
    std::shared_ptr<StreamingReaderBundle> msg;
    reader->GetBundle(5000, msg);
    EXPECT_EQ(msg->from, queue_vec[i]);
    StreamingMessageBundlePtr bundle_ptr =
        StreamingMessageBundle::FromBytes(msg->MetaBuffer(), msg->DataBuffer());
    auto &message_list = bundle_ptr->GetMessageList();
    auto &message = message_list.front();
    EXPECT_EQ(std::memcmp(message->Payload(), data, data_size), 0);
  }
}

TEST_F(StreamingTransferTest, exchange_consumed_test) {
  InitTransfer();
  writer->Run();
  uint32_t data_size = 8196;
  std::shared_ptr<uint8_t> data(new uint8_t[data_size]);
  auto func = [data, data_size](int index) { std::fill_n(data.get(), data_size, index); };

  size_t num = 1000;
  std::thread write_thread([this, data, data_size, &func, num]() {
    for (size_t i = 0; i < num; ++i) {
      func(i);
      writer->WriteMessageToBufferRing(queue_vec[0], data.get(), data_size);
    }
  });

  std::list<StreamingMessagePtr> read_message_list;
  while (read_message_list.size() < num) {
    std::shared_ptr<StreamingReaderBundle> msg;
    reader->GetBundle(5000, msg);
    StreamingMessageBundlePtr bundle_ptr =
        StreamingMessageBundle::FromBytes(msg->MetaBuffer(), msg->DataBuffer());
    auto &message_list = bundle_ptr->GetMessageList();
    std::copy(message_list.begin(), message_list.end(),
              std::back_inserter(read_message_list));
  }

  int index = 0;
  for (auto &message : read_message_list) {
    func(index++);
    EXPECT_EQ(std::memcmp(message->Payload(), data.get(), data_size), 0)
        << " message payload: " << StreamingUtility::Byte2hex(data.get(), data_size)
        << " message read payload: "
        << StreamingUtility::Byte2hex(message->Payload(), message->PayloadSize())
        << " message buffer "
        << StreamingUtility::Byte2hex(message->Data(), message->Size()) << " message id "
        << message->GetMessageId();
  }
  write_thread.join();
}

TEST_F(StreamingTransferTest, empty_message_flow_control) {
  // NOTE(lingxuan.zlx): No empty message will be sent after version 2.1,
  // so we choose exectly once strategy to fit unit test.
  auto &config = runtime_context_->config_;
  config.SetReliabilityLevel(ReliabilityLevel::EXACTLY_ONCE);
  config.SetEmptyMessageTimeInterval(5);
  config.SetStreamingWriterConsumedStep(5);
  config.SetStreamingBundleConsumedStep(5);
  config.SetStreamingWriterFlowControlSeqIdScale(1);
  config.SetStreamingReaderConsumedStep(2);
  timer_interval = 10;
  InitTransfer();
  writer->Run();
  size_t reader_target_empty_cnt = 10;
  for (size_t i = 0; i < reader_target_empty_cnt; ++i) {
    std::shared_ptr<StreamingReaderBundle> msg;
    while (StreamingStatus::OK != reader->GetBundle(12, msg)) {
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    EXPECT_TRUE(msg && msg->meta && msg->meta->IsEmptyMsg());
  }
  STREAMING_LOG(INFO) << writer->GetSendEmptyCnt(queue_vec[0]) << " "
                      << reader_target_empty_cnt;
  EXPECT_TRUE(writer->GetSendEmptyCnt(queue_vec[0]) < 10 * reader_target_empty_cnt);
}

TEST_F(StreamingTransferTest, no_empty_message_flow_control) {
  auto &config = runtime_context_->config_;
  config.SetReliabilityLevel(ReliabilityLevel::EXACTLY_ONCE);
  config.SetEmptyMessageTimeInterval(5);
  config.SetStreamingWriterConsumedStep(5);
  config.SetStreamingBundleConsumedStep(0xfffff);
  config.SetStreamingWriterFlowControlSeqIdScale(1);
  config.SetStreamingReaderConsumedStep(2);
  timer_interval = 10;
  InitTransfer();
  writer->Run();
  size_t reader_target_empty_cnt = 10;
  for (size_t i = 0; i < reader_target_empty_cnt; ++i) {
    std::shared_ptr<StreamingReaderBundle> msg;
    while (StreamingStatus::OK != reader->GetBundle(12, msg))
      ;
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    EXPECT_TRUE(msg && msg->meta && msg->meta->IsEmptyMsg());
  }
  STREAMING_LOG(INFO) << writer->GetSendEmptyCnt(queue_vec[0]) << " "
                      << reader_target_empty_cnt;
  EXPECT_TRUE(writer->GetSendEmptyCnt(queue_vec[0]) > 20 * reader_target_empty_cnt);
}

class CyclicChannelFlowControlTest : public StreamingTransferTest {
 public:
  CyclicChannelFlowControlTest() {}
  virtual ~CyclicChannelFlowControlTest() { DestroyTransfer(); };
  void InitTransfer(int channel_num = 1) {
    STREAMING_LOG(INFO) << "CyclicChannelFlowControlTest InitTransfer";
    std::vector<StreamingQueueInitialParameter> params;
    for (int i = 0; i < channel_num; ++i) {
      queue_vec.push_back(ObjectID::FromRandom());
      StreamingQueueInitialParameter param;
      param.cyclic = true;
      params.push_back(param);
    }
    std::vector<uint64_t> channel_id_vec(queue_vec.size(), 0);
    std::vector<uint64_t> queue_size_vec(queue_vec.size(), 100000);
    writer->Init(queue_vec, params, channel_id_vec, queue_size_vec);
    reader->Init(queue_vec, params, timer_interval);
  }
  void DestroyTransfer() {
    writer.reset();
    reader.reset();
  }
};

TEST_F(CyclicChannelFlowControlTest, cyclic_no_data_message_flow_control) {
  const int writer_consumed_step = 5;
  auto &config = runtime_context_->config_;
  config.SetReliabilityLevel(ReliabilityLevel::EXACTLY_ONCE);
  config.SetEmptyMessageTimeInterval(5);
  config.SetStreamingRingBufferCapacity(5);
  config.SetStreamingWriterConsumedStep(writer_consumed_step);
  config.SetStreamingBundleConsumedStep(0xfffff);
  config.SetStreamingWriterFlowControlSeqIdScale(1);
  config.SetStreamingReaderConsumedStep(2);
  config.SetStreamingWriterFlowControlSeqIdScale(1);
  timer_interval = 10;
  InitTransfer();
  writer->Run();

  std::future<int> future = std::async(std::launch::async, [this]() {
    // Without flow control, more data message can be write. > writer_consumed_step
    for (int i = 0; i < writer_consumed_step * 10; i++) {
      uint8_t data[4] = {1, 2, 3, 0xff};
      uint32_t data_size = 4;
      ray::streaming::MemoryBuffer buffer;
      STREAMING_CHECK(writer->GetBufferPool(queue_vec[0]));
      writer->GetBufferPool(queue_vec[0])->GetBufferBlockedTimeout(min_size, &buffer);
      std::memcpy(buffer.Data() + kMessageHeaderSize, data, data_size);
      EXPECT_EQ(i + 1, writer->WriteMessageToBufferRing(
                           queue_vec[0],
                           ::ray::streaming::MemoryBuffer(buffer.Data(),
                                                          kMessageHeaderSize + data_size),
                           StreamingMessageType::Message));
      std::this_thread::sleep_for(std::chrono::milliseconds(10));
    }
    return 0;
  });

  std::future_status st = future.wait_for(std::chrono::seconds(10));
  EXPECT_EQ(st, std::future_status::ready);
}

TEST_F(CyclicChannelFlowControlTest, DISABLED_empty_message_flow_control) {
  auto &config = runtime_context_->config_;
  config.SetReliabilityLevel(ReliabilityLevel::EXACTLY_ONCE);
  config.SetEmptyMessageTimeInterval(5);
  config.SetStreamingWriterConsumedStep(5);
  config.SetStreamingBundleConsumedStep(5);
  config.SetStreamingWriterFlowControlSeqIdScale(1);
  config.SetStreamingReaderConsumedStep(2);
  timer_interval = 10;
  InitTransfer();
  writer->Run();
  size_t reader_target_empty_cnt = 10;
  for (size_t i = 0; i < reader_target_empty_cnt; ++i) {
    std::shared_ptr<StreamingReaderBundle> msg;
    while (StreamingStatus::OK != reader->GetBundle(12, msg)) {
    }
    std::this_thread::sleep_for(std::chrono::milliseconds(100));
    EXPECT_TRUE(msg && msg->meta && msg->meta->IsEmptyMsg());
  }
  STREAMING_LOG(INFO) << writer->GetSendEmptyCnt(queue_vec[0]) << " "
                      << reader_target_empty_cnt;
  EXPECT_TRUE(writer->GetSendEmptyCnt(queue_vec[0]) < 10 * reader_target_empty_cnt);
}
}  // namespace streaming
}  // namespace ray

int main(int argc, char **argv) {
  ::testing::InitGoogleTest(&argc, argv);
  return RUN_ALL_TESTS();
}
