...
std::string StreamingUtility::Byte2hex(const uint8_t *data, uint32_t data_size) {
  STREAMING_LOG(DEBUG) << "data_size=" << data_size;
  constexpr char hex[] = "0123456789abcdef";
  std::string result;
  for (uint32_t i = 0; i < data_size; i++) {
    unsigned short val = data[i];
    int a = (val >> 4), b = (val & 0xf);
    if (a < 0 || a >= 16 || b < 0 || b >= 16) {
      STREAMING_LOG(DEBUG) << "i=" << i << "val=" << val << ", val>>4 = " << a
                           << ", val&0xf=" << b;
    }
...
bool StreamingUtility::IsOnlineEnv() {
  auto env_ptr = std::getenv("ALIPAY_APP_ENV");
  if (env_ptr) {
    std::string env_str(env_ptr);
    return env_str == std::string("prod") || env_str == std::string("prepub") ||
           env_str == std::string("gray");
  }
  return false;
}
}  // namespace streaming
