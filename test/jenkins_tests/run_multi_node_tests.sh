...
docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/train.py \
...
docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/test/test_checkpoint_restore.py

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/test/test_policy_evaluator.py

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/test/test_serving_env.py

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/test/test_lstm.py

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/test/test_multi_agent_env.py

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/test/test_supported_spaces.py

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/tune/examples/tune_mnist_ray.py \
    --smoke-test

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/tune/examples/pbt_example.py \
    --smoke-test

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/tune/examples/hyperband_example.py \
    --smoke-test

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/tune/examples/async_hyperband_example.py \
    --smoke-test

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/tune/examples/tune_mnist_ray_hyperband.py \
    --smoke-test

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/tune/examples/tune_mnist_async_hyperband.py \
    --smoke-test

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/tune/examples/hyperopt_example.py \
    --smoke-test

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/tune/examples/tune_mnist_keras.py \
    --smoke-test

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/examples/legacy_multiagent/multiagent_mountaincar.py

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/examples/legacy_multiagent/multiagent_pendulum.py

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/examples/multiagent_cartpole.py --num-iters=2

docker run --rm --shm-size=10G --memory=10G $DOCKER_SHA \
    RAY_USE_XRAY=1 python /ray/python/ray/rllib/examples/multiagent_two_trainers.py --num-iters=2
